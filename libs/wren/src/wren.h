...
// that contains the import. Typically, this is used to implement relative imports.
typedef CS (*WrenResolveModuleFn)(WrenVM* vm, CS importer, CS name);
...
typedef void (*WrenLoadModuleCompleteFn)(WrenVM* vm, CS name, WrenLoadModuleResult result);
...
typedef void (*WrenWriteFn)(WrenVM* vm, CS text);
...
typedef void (*WrenErrorFn)( WrenVM* vm, WrenErrorType type, CS module, int line, CS message);
...
typedef WrenForeignClassMethods (*WrenBindForeignClassFn)( WrenVM* vm, CS module, CS className);
...
typedef struct {
   ...
   //imports of the same module are detected, and how the module is reported in stack traces.
   //
   ...
   //number, and an error message. If this is `NULL`, Wren doesn't report any errors.
   WrenErrorFn errorFn;
   ...
   //to shrink its memory usage automatically after reclaiming a large amount of memory.
   //
...
//freed after calling this. If [configuration] is `NULL`, uses a default configuration.
...
//The following functions are intended to be called from foreign methods or finalizers. The 
//interface Wren provides to a foreign method is like a register machine: you are given a numbered 
//array of slots that values can be read from and written to. Values always live in a slot 
//(unless explicitly captured using wrenGetSlotHandle(), which ensures the garbage collector can
//find them.
//
//When your foreign function is called, you are given one slot for the receiver and each argument 
//to the method. The receiver is in slot 0 and the arguments are in increasingly numbered slots 
//after that. You are free to read and write to those slots as you want. If you want more slots 
//to use as scratch space, you can call wrenEnsureSlots() to add more.
//
//When your function returns, every slot except slot zero is discarded and the value in slot zero 
//is used as the return value of the method. If you don't store a return value in that slot 
//yourself, it will retain its previous value, the receiver.
//
//While Wren is dynamically typed, C is not. This means the C interface has to support the 
//various types of primitive values a Wren variable can hold: bool, double, string, etc. If we 
//supported this for every operation in the C API, there would be a combinatorial explosion of 
//functions, like "get a double-valued element from a list", "insert a string key and double value
//into a map", etc.
//
...
CS wrenGetSlotBytes(WrenVM* vm, int slot, int* length);
...
//function returns, since the garbage collector may reclaim it.
//It is an error to call this if the slot does not contain a string.
CS wrenGetSlotString(WrenVM* vm, int slot);
...
void wrenInsertIntoList(WrenVM* vm, int listSlot, int index, int elementSlot);
...
CS wrenMetaSource();
WrenForeignMethodFn wrenMetaBindForeignMethod(
    WrenVM* vm,
    CS className,
    Bool isStatic,
    CS signature
