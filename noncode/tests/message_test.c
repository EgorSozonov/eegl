...
{
    Byte  *buf; /*allocated every time to find uninit errors */
    Byte  *s;

    // Should not write anything to destination if buflen is 0.
    trunc_string((Byte *)"", NULL, 1, 0);

    // Truncating an empty string does nothing.
    buf = alloc(1);
    trunc_string((Byte *)"", buf, 1, 1);
    assert(buf[0] == NUL);
    vim_free(buf);
    ...
    buf = alloc(40);
    s = vim_strsave((Byte *)"a text that fits");
    ...
    buf = alloc(40);
    s = vim_strsave((Byte *)"a short text");
    trunc_string(s, buf, 20, 40);
    assert(STRCMP(buf, "a short text") == 0);
    vim_free(buf);
    vim_free(s);
    ...
    buf = alloc(40);
    s = vim_strsave((Byte *)"a text tha just fits");
    trunc_string(s, buf, 20, 40);
    assert(STRCMP(buf, "a text tha just fits") == 0);
    vim_free(buf);
    vim_free(s);
    ...
    buf = alloc(40);
    s = vim_strsave((Byte *)"a text that nott fits");
    trunc_string(s, buf, 20, 40);
    assert(STRCMP(buf, "a text t...nott fits") == 0);
    vim_free(buf);
    vim_free(s);
...
test_trunc_string_mbyte(void)
{
    ...
    buf = alloc(40);
    s = vim_strsave((Byte *)"Ä text tha just fits");
    trunc_string(s, buf, 20, 40);
    ...
    buf = alloc(40);
    s = vim_strsave((Byte *)"a text ÄÖÜä nott fits");
    trunc_string(s, buf, 20, 40);
    ...
    buf = alloc(40);
    s = vim_strsave((Byte *)"a text that not fitsÄ");
    trunc_string(s, buf, 20, 40);
...
test_vim_snprintf(void)
{
	...
	n = vim_snprintf(buf, bsize, fmt_06b, (UVarNumber)12);
	assert(n == 6);
...
test_vim_snprintf_positional(void)
{
	...
	n = vim_snprintf(buf, bsize, fmt_06pb2, 6, (UVarNumber)12);
	assert(n == 6);
	assert(bsize == 0 || STRNCMP(buf, "001100", bsize_int) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize_int)] == '\0');

	n = vim_snprintf(buf, bsize, fmt_06pb, (UVarNumber)12, 6);
	assert(n == 6);
	assert(bsize == 0 || STRNCMP(buf, "001100", bsize_int) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize_int)] == '\0');
...
main(int argc, char **argv)
{
    ...
    optionSetAndReportError((Byte *)"encoding", 0, (Byte *)"utf-8", 0);
    init_chartab();
    test_trunc_string();
    ...
    optionSetAndReportError((Byte *)"encoding", 0, (Byte *)"latin1", 0);
    init_chartab();
    test_trunc_string();
    test_vim_snprintf();
    test_vim_snprintf_positional();
