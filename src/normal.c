...
private int foldMoveTo(int      updown, int      dir, long   count);
...
getcount:
   ...
   if (c == K_CURSORHOLD) {
    ...
    // Only set v:count when called from main() and not a stuffed action.
    // Do set it for redo.
   if (toplevel && readbuf1_empty())
...
normal_end:
   ...
   if (prev_finish_op || cmdArg.cmdchar == 'r'
                 || (cmdArg.cmdchar == 'g' && cmdArg.nchar == 'r')) {
       ui_cursor_shape();      // may show different cursor shape
   }
   ...
   int      find_type)
{
   ...
   ptr = memGetLine(wp->book, lnum, FALSE);
...
prepareForRedo(ActionArg* aArg) {
   prep_redo(aArg->oper->regname, aArg->count0,
                 ZERO, aArg->cmdchar, ZERO, ZERO, aArg->nchar);
}
   ...
   if (check && curPor->bookOpts.scrollBind) {
      ...
      ei (curPor == old_curPor) {
        ...
        && (curPor->topLine != old_topline
          ...
          {
         check_scrollbind(curPor->topLine - old_topline,
            (long)(curPor->leftCol - old_leftcol));
          }
...
find_decl(
   ...
   // and "~" causes trouble.
   patlen = eeSnprintf(pat, len + 7, eeIsWordPtr(ptr) ? "\\V\\<%.*s\\>" : "\\V%.*s", len, ptr);
...
get_visual_text(
   ActionArg   *aArg,
   Byte   **pp,       // return: start of selected text
...
nv_screengo(Operator *oper, Unt dir, long dist) {
   ...
   col_off1 = curPor_col_off();
   width1 = curPor->width - col_off1;
   width2 = curPor->width - col_off1;
   if (width2 == 0)
...
nv_zet(ActionArg* aArg) {
   ...
   case 'D':   
      if (foldManualAllowed(FALSE)) {
         ...
         deleteFold(curPor->cursor.lnum,
              curPor->cursor.lnum, nchar == 'D', FALSE);
      }
      break;
...
nv_colon(ActionArg* aArg) {
   int   isCmdkey = aArg->cmdchar == K_COMMAND || aArg->cmdchar == K_SCRIPT_COMMAND;
...
nv_ident(ActionArg* aArg) {
   ...
   if (cmdchar == ']' || cmdchar == Ctrl_RSB || cmdchar == 'K') {
      if (VIsual_active && get_visual_text(aArg, &ptr, &n) == FAIL)
          return;
      if (checkclearopq(aArg->oper))
          return;
   }
   ...
   kp_help = (*kp == ZERO || STRCMP(kp, ":he") == 0 || STRCMP(kp, ":help") == 0);
...
nv_right(ActionArg* aArg) {
   ...
   for (n = aArg->count1; n > 0; --n) {
      if ((!past_line && oneright() == FAIL) || (past_line && *ml_get_cursor() == ZERO)) {
         ...
         if (       ((aArg->cmdchar == ' '
                && firstOccurrence(p_ww, 's') != NULL)
            || (aArg->cmdchar == 'l'
                && firstOccurrence(p_ww, 'l') != NULL)
            || (aArg->cmdchar == K_RIGHT
                && firstOccurrence(p_ww, '>') != NULL))
             && curPor->cursor.lnum < curBook->mem.lineCount
         ) {
...
nv_left(ActionArg* aArg) {
   ...
   for (n = aArg->count1; n > 0; --n) {
      if (oneleft() == FAIL) {
          ...
          if (       (((aArg->cmdchar == K_BS
               || aArg->cmdchar == Ctrl_H)
                && firstOccurrence(p_ww, 'b') != NULL)
            || (aArg->cmdchar == 'h'
                && firstOccurrence(p_ww, 'h') != NULL)
            || (aArg->cmdchar == K_LEFT
                && firstOccurrence(p_ww, '<') != NULL))
             && curPor->cursor.lnum > 1
         ) {
      ...
      while (n > 0) {
         for (;;) {
            ...
            if (c == '{' || c == '}') {
               ...
               ei ((pos = findmatchlimit(aArg->oper, findc,
                     (aArg->cmdchar == '[') ? FM_BACKWARD : FM_FORWARD,
                                 0)) == NULL)
                  n = 0;
               else
                  curPor->cursor = *pos;
...
nv_findpar(ActionArg* aArg) {
   ...
   aArg->oper->use_reg_one = TRUE;
   curPor->setCursWant = TRUE;
   if (!findpar(&aArg->oper->inclusive, aArg->arg, aArg->count1, ZERO, FALSE)) {
      clearopbeep(aArg->oper);
      return;
   }
...
nv_pcmark(ActionArg* aArg) {
   ...
   if (aArg->cmdchar == 'g')
      pos = movechangelist((int)aArg->count1);
   else
      pos = movemark((int)aArg->count1);
...
nv_wordcmd(ActionArg* aArg) {
   ...
   if (aArg->cmdchar == 'e' || aArg->cmdchar == 'E')
       word_end = TRUE;
   else
       word_end = FALSE;
...
n_swapchar(ActionArg* aArg) {
   ...
   if (LINEEMPTY(curPor->cursor.lnum) && firstOccurrence(p_ww, '~') == NULL) {
      clearopbeep(aArg->oper);
      return;
   }
   ...
   curPor->setCursWant = TRUE;
   if (did_change) {
      changed_lines(startpos.lnum, startpos.col, curPor->cursor.lnum + 1,
                                0L);
...
curs_rows(Portal *po) {
   ...
   i = 0;
   po->cursorLineRow = 0;
   for (lnum = po->topLine; lnum < po->cursor.lnum; ++i) {
      valid = FALSE;
   ...
   int   *ecolp)   // end screen column
{
   ...
   if (pos->lnum >= po->topLine && pos->lnum <= po->bottomLine) {
      ...
      is_folded = getFoldsPortal(po, lnum, &lnum, NULL, TRUE, NULL);
...
void
f_screenpos(Var *argvars UNUSED, Var *returnVar) {
   Pos   pos;
   int      row = 0;
...
virtcol2col(Portal *po, LineNr lnum, int vcol) {
   ...
   Byte   *line = memGetLine(po->book, lnum, FALSE);
   Byte   *p = line + offset;
...
void
f_virtcol2col(Var *argvars UNUSED, Var *returnVar) {
   returnVar->number = -1;
...
topline_back_winheight(
   LineOffset   *lp,
   int      winheight)   // when TRUE limit to portal height
{
...
scroll_cursor_top(int min_scroll, int always) {
   ...
   if (getFolds(curPor->cursor.lnum, &top, &bot)) {
      --top;
      ++bot;
   } else {
...
scroll_cursor_halfway(int atend, int prefer_above) {
   ...
   loff.lnum = boff.lnum = curPor->cursor.lnum;
...
private int
getMapMode(Byte **cmdp, int forceit) {
   int      mode;
...
private void
mapClear(CS cmdp, CS arg, int forceit, int abbr) {
...
void
f_maplist(Var *argvars UNUSED, Var *returnVar) {
   Bag* d;
   MapBlock   *mp;
   int const flags = REPTERM_FROM_PART | REPTERM_DO_LT;
   int abbr = FALSE;
...
f_mapset(Var *argvars, Var *returnVar UNUSED) {
   ...
   sid = bagGetNumber(bag,tConst("sid"));
   lnum = bagGetNumber(bag, tConst("lnum"));
   int isBook = bagGetNumber(bag, tConst("buffer"));
   nowait = bagGetNumber(bag, tConst("nowait")) != 0;
...
CS
setLangmap(OptionChange *args UNUSED) {
   Byte  *p2;

   for (CS p = p_langmap; p[0] != ZERO; ) {
...
checkSmall(
    Portal   *wp,
    Fold   *fp,
    LineNr   lnum_off)   // offset for fp->fd_top
{
   int      count;
   int      n;
...
foldAddMarker(LineNr lnum, CS marker, int markerlen) {
   Byte   *cms = curBook->o.commentString;
...
private void
deleteFoldMarkers(Fold* fp, int recursive, LineNr lnum_off
         ...
         while (!gotInterruptG) {
                  ...
                  || (lvl >= level && fp->fd_top <= flp->lnum_save)))))
            {
               ...
               if (fp->fd_top >= startlnum) {
                ...
                // Insert new fold.  Careful: c may be NULL and it
               // may change!
               if (gap->len == 0)
                  i = 0;
               else
...
foldlevelMarker(FoldLine *flp) {
   ...
   s = memGetLine(flp->po->book, flp->lnum + flp->off, FALSE);
   while (*s) {
...
// "foldlevel()" function
void
f_foldlevel(Var *argvars UNUSED, Var *returnVar UNUSED) {
   LineNr lnum = tv_get_lnum(argvars);
   if (lnum >= 1 && lnum <= curBook->mem.lineCount)
