...
void transchar_hex(Byte *buf, int c);
int eeglStrNsize(Byte *s, int len);
int eeglStrSize(CS s);
int eeIsFnameChar_or_wc(int c);
CS skipwhite(CS q);
CS skipSpace(CS q);
CS skipwhite_and_nl(CS q);
int getwhitecols_curline(void);
int getwhitecols(Byte *p);
Byte *skipdigits(Byte *q);
CS skipbin(CS q);
CS skiphex(CS q);
CS skiptobin(CS q);
CS skiptodigit(CS q);
CS skiptohex(CS q);
int eeIsDigit(int c);
int eeIsXDigit(int c);
Boole eeIsLower(Unt c);
int eeIsUpper(int c);
int eeglIsAlfa(int c);
int eeglToUpper(int c);
int eeglToLower(int c);
CS skiptowhite(CS p);
CS skiptowhite_esc(CS p);
CS skipToLastSpace(CS p);
long getdigits(CS *pp);
long getdigits_quoted(CS *pp);
...
int hex2nr(int c);
int hexhex2nr(Byte *p);
int rem_backslash(CS filename);
void backslash_halve(Byte *p);
...
Byte *toStringChunky(ChunkyString *chunky);
Text mbText(CS b);
Text text(CS b);
...
CS eeStrbyte(CS string, int c);
CS lastOccurrence(CS string, int c);
void sortStrings(Byte **files, int count);
int has_non_ascii(Byte *s);
...
void string_filter_map(Byte *str, FilterMap filtermap, Var *expr, Var *returnVar);
...
long char_idx2byte(Byte *str, Unt str_len, Long idx);
CS concatStrArray(CS *arr, Unt len, Text separator, Arena *a);
CS fileExtension(Text fName);
Boole startsWith(Byte *longerStr, Byte *shorterStr);
...
int fuzzyMatchStr(Byte *str, Byte *pat);
ArrayList *fuzzyMatchStr_with_pos(Byte *str, Byte *pat);
...
CS remove_tail(Byte *p, Byte *pend, Byte *name);
...
UiColor decodeHexColorLengthGuaranteed(Byte *hex);
