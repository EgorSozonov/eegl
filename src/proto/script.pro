...
Byte *set_context_in_user_cmd(Expand *xp, Byte *arg_in);
CS set_context_in_user_cmdarg(CS comm, CS arg, long argFlags, Unt context, Expand *xp, int forceit);
...
CS uc_fun_cmd(void);
int parse_compl_arg(Byte *value, int vallen, int *context, long *argFlags, Byte **compl_arg);
...
Unt add_win_cmd_modifiers(Byte *builder, CommandModifier *cmod, int *multi_mods);
...
void func_name_with_sid(Byte *name, int sid, CS builder);
...
int add_defer(Byte *name, int argcount_arg, Var *argvars);
...
int event_ignored(AutoEvent event, Byte *evIgn);
int check_ei(Byte *evIgn);
...
void auCommRestoreBuf(AutocommSave *aco);
...
int has_autocmd(AutoEvent event, Byte *sfname, Book *book);
CS get_event_name(Expand *xp, int idx);
Byte *get_event_name_no_group(Expand *xp, int idx, int win);
int has_tabclosedpre(void);
int autocmd_supported(Byte *name);
