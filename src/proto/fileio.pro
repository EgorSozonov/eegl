/* src/fileio.c */
int modify_fname(Byte *src, int tilde_file, Unt *usedlen, Byte **fnamep, Byte **bufp, Unt *fnamelen);
void init_homedir(void);
void shorten_dir(CS str);
int file_is_readable(CS fname);
void f_chdir(Var *argvars, Var *returnVar);
void f_delete(Var *argvars, Var *returnVar);
void f_executable(Var *argvars, Var *returnVar);
void f_exepath(Var *argvars, Var *returnVar);
void f_filereadable(Var *argvars, Var *returnVar);
void f_filewritable(Var *argvars, Var *returnVar);
void f_finddir(Var *argvars, Var *returnVar);
void f_findfile(Var *argvars, Var *returnVar);
void f_fnamemodify(Var *argvars, Var *returnVar);
void f_getcwd(Var *argvars, Var *returnVar);
CS getfpermst(FileStat *st, CS perm);
void f_getfperm(Var *argvars, Var *returnVar);
void f_getfsize(Var *argvars, Var *returnVar);
void f_getftime(Var *argvars, Var *returnVar);
CS getftypest(FileStat *st);
void f_getftype(Var *argvars, Var *returnVar);
void f_glob(Var *argvars, Var *returnVar);
void f_glob2regpat(Var *argvars, Var *returnVar);
void f_globpath(Var *argvars, Var *returnVar);
void f_isdirectory(Var *argvars, Var *returnVar);
void f_isabsolutepath(Var *argvars, Var *returnVar);
void f_mkdir(Var *argvars, Var *returnVar);
void f_pathshorten(Var *argvars, Var *returnVar);
void f_readdir(Var *argvars, Var *returnVar);
void f_readdirex(Var *argvars, Var *returnVar);
void f_readblob(Var *argvars, Var *returnVar);
void f_readfile(Var *argvars, Var *returnVar);
void f_resolve(Var *argvars, Var *returnVar);
void f_tempname(Var *argvars, Var *returnVar);
void f_writefile(Var *argvars, Var *returnVar);
void f_browse(Var *argvars, Var *returnVar);
void f_browsedir(Var *argvars, Var *returnVar);
void f_filecopy(Var *argvars, Var *returnVar);
void home_replace(Book *buf, CS src, CS dst, int dstlen, int one);
CS home_replace_save(Book *buf, CS src);
CS homeReplaceA(Book *buf, CS src, Arena *a);
int fullpathcmp(CS s1, CS s2, int checkname, int expandenv);
CS gettail(CS fname);
CS gettail_sep(Byte *fname);
CS getnextcomp(CS fname);
CS get_past_head(CS path);
int dir_of_file_exists(CS fname);
CS concat_fnames(CS fname1, CS fname2, int sep);
void add_pathsep(Byte *p);
CS FullName_save(CS fname, int force);
int eeFexists(Byte *fname);
int expand_wildcards_eval(CS *pattern, Unt flags, ExpandMatch *files);
int expand_wildcards(int num_pat, CS *pat, Unt flags, ExpandMatch *files);
int match_suffix(Byte *fname);
int gen_expand_wildcards(int num_pat, CS *pat, Unt flags, ExpandMatch *matches);
void addFile(ExpandMatch *matches, CS fName, Unt flags);
int pathcmp(const char *p, const char *q, int maxlen);
int eeIsAbsName(Byte *name);
int eeFullFileName(CS fname, CS buf, int len, int force);
int mch_dirname(CS buf, int len);
int mch_FullName(CS fname, CS buf, int len, int force);
int mch_isFullName(Byte *fname);
FileSearchCtx *eeFindFile_init(CS path, Text filename, CS stopdirs, int level, Boole free_visited, Unt find_what, FileSearchCtx *search_ctx_arg, Boole tagfile, CS rel_fname);
int eeChdir(CS new_dir);
int eeChdirfile(CS fname, char *trigger_autocmd);
CS eeFindFile_stopdir(Byte *buf);
void eeFindFile_cleanup(FileSearchCtx *ctx);
CS eeFindFile(FileSearchCtx *search_ctx_arg);
CS findFileInPath(Text fname, Unt options, Boole first, CS rel_fname, Byte **file_to_find, FileSearchCtx **search_ctx);
void free_findfile(void);
CS grab_file_name(long count, LineNr *file_lnum);
CS file_name_at_cursor(int options, long count, LineNr *file_lnum);
CS file_name_in_line(Byte *line, int col, int options, long count, Byte *rel_fname, LineNr *file_lnum);
CS find_file_name_in_path(CS ptr, int len, Unt options, long count, CS rel_fname);
Unt simplify_filename(CS filename);
int mch_has_exp_wildcard(Byte *p);
int mch_has_wildcard(Byte *p);
void f_simplify(Var *argvars, Var *returnVar);
void fiGlobpath(CS path, CS file, ExpandMatch *matches, Unt expand_options, Boole onlyDirs);
int mch_chdir(char *path);
void filemess(Book *buf, Byte *name, Byte *s, int attr);
int readfile(Byte *fname, Byte *sfname, LineNr from, LineNr lines_to_skip, LineNr lines_to_read, Invocation *invo, int flags);
int read_blob(FILE *fd, Var *returnVar, FileSize offset, FileSize size_arg);
int write_blob(FILE *fd, Blob *blob);
int is_dev_fd_file(CS fname);
int prep_exarg(Invocation *invo, Book *book);
void set_file_options(Invocation *invo);
int check_file_readonly(CS fname, int perm);
int eeFsync(int fd);
int set_rw_fname(CS fname, CS sfname);
void msg_add_fname(Book *buf, CS fname);
void msg_add_lines(int insert_space, long lnum, FileSize nchars);
void msg_add_eol(void);
int time_differs(FileStat *st, long mtime, long mtime_ns);
CS shorten_fname1(CS full_path);
CS shorten_fname(Byte *full_path, Byte *dir_name);
void shorten_buf_fname(Book *buf, Byte *dirname, int force);
void shorten_fnames(int force);
CS modname(CS fname, CS ext, int prepend_dot);
CS buf_modname(Byte *fname, Byte *ext, int prepend_dot);
int eeFgets(CS buf, int size, FILE *fp);
int eeRename(Byte *from, Byte *to);
int check_timestamps(int focus);
int fiCheckBookTimestamp(Book *book);
void buf_reload(Book *book, int orig_mode, int reload_options);
void buf_store_time(Book *book, FileStat *st, Byte *fname);
void write_lnum_adjust(LineNr offset);
int mch_isrealdir(Byte *name);
Boole mch_isdir(Byte *name);
int mch_nodetype(Byte *name);
void eeDelTempDir(void);
CS eeTempName(int extra_char, int keep);
int match_file_pat(Byte *pattern, RegProg **prog, Byte *fname, Byte *sfname, Byte *tail, int allow_dirs);
int match_file_list(CS list, CS sfname, CS ffname);
CS file_pat_to_reg_pat(CS pat, CS pat_end, Byte *allow_dirs);
long read_eintr(int fd, void *buf, Unt bufsize);
long write_eintr(int fd, void *buf, Unt bufsize);
CS get_cmd_output(CS cmd, CS infile, int flags, int *ret_len);
void f_system(Var *argvars, Var *returnVar);
void f_systemlist(Var *argvars, Var *returnVar);
int filewritable(Byte *fname);
int get2c(FILE *fd);
int get3c(FILE *fd);
int get4c(FILE *fd);
CS read_string(FILE *fd, int cnt);
int call_shell(CS cmd, CS extraArg, int opt);
long mch_getperm(Byte *name);
int mch_setperm(CS name, long perm);
void mch_copy_xattr(CS from_file, CS to_file);
int mch_fsetperm(int fd, long perm);
int mch_can_exe(CS name, CS *path, int use_path);
/* eegl: set ft=c : */
