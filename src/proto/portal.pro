...
int portalLayout_locked(CommIndex cmd);
...
int splitPortalmove(Portal *po, int size, int flags);
void portMoveAfter(Portal *port0, Portal *port1);
void portEqualizeHeight(Portal *next_curPor, int current, int dir);
void leavingPortal(Portal *port);
void enteringPortal(Portal *port);
void curPor_init(void);
void closePortalsInto(Book *book, int keep_curPor);
int lastPortal(void);
int onePortal(void);
...
Portal *portRemoveFrame(Portal *port, int *dirp, Tab *t, Frame **unflat_altfr);
