...
int skip_expr(Byte **pp, EvalCtx *evalarg);
...
void *eval_for_line(CS arg, Boole *errp, Invocation *invo, EvalCtx *evalarg);
...
int eval0(CS arg, Var *returnVar, Invocation *invo, EvalCtx *evalarg);
int may_call_simple_func(CS arg, Var *returnVar);
int eval0_simple_funccal(CS arg, Var *returnVar, Invocation *invo, EvalCtx *evalarg);
...
CS do_string_sub(CS str, Unt len, Byte *pat, Byte *sub, Var *expr, Byte *flags, Unt *ret_len);
...
int get_spellword(List *list, Byte **pp);
...
int before_set_vvar(Byte *varname, DictItem *di, Var *tv, int copy, int *type_error);
...
FnError call_internal_func(Byte *name, int argcount, Var *argvars, Var *returnVar);
void call_internal_func_by_idx(int idx, Var *argvars, Var *returnVar);
FnError call_internal_method(Byte *name, int argcount, Var *argvars, Var *returnVar, Var *basetv);
...
int cause_errthrow(Byte *mesg, int severe, int *ignore);
void free_global_msglist(void);
void do_errthrow(CondStack *cstack, CS cmdname);
int do_intthrow(CondStack *cstack);
CS get_exception_string(void *value, ExceptionKind type, Byte *cmdname, int *should_free);
...
void c_if(Invocation *invo);
void c_endif(Invocation *invo);
void c_else(Invocation *invo);
void c_while(Invocation *invo);
void c_continue(Invocation *invo);
void c_break(Invocation *invo);
void c_endwhile(Invocation *invo);
void c_block(Invocation *invo);
void c_endblock(Invocation *invo);
int inside_block(Invocation *invo);
void c_throw(Invocation *invo);
void do_throw(CondStack *cstack);
void c_try(Invocation *invo);
void c_catch(Invocation *invo);
void c_finally(Invocation *invo);
void c_endtry(Invocation *invo);
void enter_cleanup(Cleanup *csp);
void leave_cleanup(Cleanup *csp);
int cleanup_conditionals(CondStack *cstack, int searched_cond, int inclusive);
