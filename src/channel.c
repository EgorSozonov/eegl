...
static volatile SigAtomic sig_alarm_called;
#endif
...
server_to_input_buf(CS str) {
   ...
   if (*ptr != ZERO) {   // trailing CTRL-V results in nothing
      ...
      if (typeBufG.mappedLen < typeBufG.validLen)
          del_typebuf(typeBufG.validLen - typeBufG.mappedLen, typeBufG.mappedLen);
      (void)insertIntoTypebuf(str, REMAP_NONE, typeBufG.validLen, TRUE, FALSE);
...
eval_client_expr_to_string(CS expr) {
   ...
   // Disable debugging, otherwise Eegl hangs, waiting for "cont" to be typed.
   debug_break_level = -1;
   redir_off = 0;
   // Do not display error message, otherwise Eegl hangs, waiting for "cont"
   ...
   // A client can tell us to redraw, but not to display the cursor, so do that here.
   setcursor();
   out_flush();
...
int
sendToLocalEm(CS cmd, int asExpr, Byte **result) {
   if (asExpr) {
      CS ret = eval_client_expr_to_string(cmd);
      if (result) {
         if (!ret)  {
            CS err = _(e_invalid_expression_received);
            Unt   len = STRLEN(cmd) + STRLEN(err) + 5;

            CS msg = alloc(len);
            eeSnprintf(msg, len, "%s: \"%s\"", err, cmd);
            *result = msg;
         } else
            *result = ret;
      } else
...
#if (!defined(NO_EEGL_MAIN)) || defined(PROTO)
...
exec_on_server(MainParams *params) {
   ...
   //Otherwise it's executed further on.
   if (params->serverArg) {
      cmdsrv_main(&params->argc, params->argv, params->serverName_arg, &params->serverStr);
   }

   //If we're still running, get the name to register ourselves.
   //X11 needs to setup the clipboard first, it's further down.
   params->servername = serverMakeName(params->serverName_arg, (CS)params->argv[0]);
}
...
# if defined(FEAT_X11)
   ...
   if (X_DISPLAY != NULL && params->servername != NULL && (
      (getuid() != ROOT_UID) ||
      params->serverName_arg != NULL)
   ){
      (void)serverRegisterName(X_DISPLAY, params->servername);
      eeglFree(params->servername);
      TIME_MSG("register server name");
   } else
      serverDelayedStartName = params->servername;
# endif
...
# endif
   ...
   for (i = 1; i < Argc; i++) {
      ...
      if (caseInsensitiveCompare(argv[i], "--remote-send") == 0)
      ...
      ei (STRNICMP(argv[i], "--remote", 8) == 0)    {
         ...
         while (*p != ZERO) {
            if (STRNICMP(p, "-wait", 5) == 0) {
               ...
               p += 5;
            } ei (STRNICMP(p, "-silent", 7) == 0) {
               silent = TRUE;
               p += 7;
            } ei (STRNICMP(p, "-tab", 4) == 0) {
               tabs = TRUE;
               p += 4;
            } else    {
...
# ifdef FEAT_X11
            if (xterm_dpy == NULL) {
               ...
               ret = serverSendToEegl(xterm_dpy, sname, *serverStr, NULL, &srv, 0, 0, 0, silent);
# else
            ret = serverSendToEegl(sname, *serverStr, NULL, &srv, 0, 0, silent);
# endif
         if (ret < 0) {
            if (argtype == ARGTYPE_SEND) {
      ...
      } ei (caseInsensitiveCompare(argv[i], "--remote-expr") == 0) {
         if (i == *argc - 1)
            mainerr_arg_missing((CS)argv[i]);
         if (xterm_dpy == NULL)
   ...
   int      sendReply)
{
   ...
   if (filec <= 0)
      mainerr_arg_missing((CS)filev[-1]);
   ...
   ga_concat(&ga, (CS)wig[0]);

   ga_concat(&ga, S"<CR><C-\\><C-N>:if exists('*inputsave')|call inputsave()|endif|");
   if (tabs)
      ga_concat(&ga, S"tab ");
   ga_concat(&ga, S"drop");
   for (i = 0; i < filec; i++) {
   ...
   ga_concat(&ga, (CS) "|if exists('*inputrestore')|call inputrestore()|endif<CR>");

   //The :drop commands goes to Insert mode when 'insertmode' is set, use
   //CTRL-\ CTRL-N again.
   ga_concat(&ga, (CS)"<C-\\><C-N>");
   ...
   //   endif
   // endif
   ga_concat(&ga, (CS)":if !exists('+acd')||!&acd|if haslocaldir()|");
   ga_concat(&ga, (CS)"cd -|lcd -|elseif getcwd() ==# '");
   ga_concat(&ga, cdp);
   ga_concat(&ga, (CS)"'|cd -|endif|endif<CR>");
   eeglFree(cdp);
   // reset wildignorecase
   ga_concat(&ga, (CS)wig[1]);

   if (sendReply)
      ga_concat(&ga, (CS)":call SetupRemoteReplies()<CR>");
   ga_concat(&ga, (CS)":");
   if (inicmd != NULL) {
   ...
   // Bring the portal to the foreground, goto Insert mode when 'im' set and clear command line.
   ga_concat(&ga, (CS)"cal foreground()|if &im|star|en|redr|f<CR>");
   ga_append(&ga, ZERO);
   return ga.c;
}
...
# endif
   ...
   CS server_name = convertVarToStringSingleUse(&argvars[0]);
   if (!server_name)
      return;      // type error; errmsg already given
   CS keys = tv_get_string_buf(&argvars[1], builder);
   if (serverSendToEegl(X_DISPLAY, server_name, keys, &r, &w, expr, timeout, 0, TRUE) < 0) {
      if (r) {
          emsg(r);   // sending worked but evaluation failed
          eeglFree(r);
      } else
         showErrFmtMsg(_(e_unable_to_send_to_str), server_name);
      return;
   }
   ...
   if (argvars[2].tag != VAR_UNKNOWN) {
      ...
      idvar = convertVarToStringSingleUse(&argvars[2]);
      if (idvar && *idvar != ZERO) {
         sprintf((char *)str, PRINTF_HEX_ULONG, (Ulong)w);
         v.c.tag = VAR_STRING;
         v.c.string = copyStr(str);
         set_var(text(idvar), &v.c, FALSE);
         eeglFree(v.c.string);
      }
   }
}
...
f_remote_peek(Var *argvars UNUSED, Var *returnVar) {
   ...
   returnVar->number = serverPeekReply(X_DISPLAY, serverStrToWin(serverid), &s);

   if (argvars[1].tag != VAR_UNKNOWN && returnVar->number > 0) {
      Byte      *retvar;

      v.c.tag = VAR_STRING;
      v.c.string = copyStr(s);
      retvar = convertVarToStringSingleUse(&argvars[1]);
      if (retvar)
          set_var(text(retvar), &v.c, FALSE);
      eeglFree(v.c.string);
   }
}
...
f_remote_startserver(Var *argvars UNUSED, Var *returnVar UNUSED) {
   ...
   CS server = convertVarToStringSingleUse(&argvars[0]);
# ifdef FEAT_X11
...
f_server2client(Var *argvars UNUSED, Var *returnVar) {
   ...
   CS server = convertVarToStringSingleUse(&argvars[0]);
   CS reply = convertVarToString(&argvars[1], builder);
   if (!server || !reply)
      return;
