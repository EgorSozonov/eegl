...
private int screenClearedS = FALSE;   // screen has been cleared, yes/no/maybe
...
//
//screenLinesG[off]  Contains a copy of the whole screen, as it is currently
//   displayed (excluding text written by external commands).
//   Dimensions of screenLinesG = (visibleRowsG + 1) * visibleColumnsG
//screenDecosG[off]  Contains the associated (drawn) decorations.
//screenColsG[off]   Contains the virtual columns in the line. -1 means not
//                   available or before buffer text.
//
//lineOffsetG[row]   Contains the offset into screenLinesG*[], screenDecosG[]
//                   and screenColsG[] for each line.
//lineWrapsG[row]    Flag for each line whether it wraps to the next line.
//
//Multi-byte characters are converted to Unicode and stored in screenLinesUCG[]. 
//screenLinesG[] contains the first byte only. For an ASCII character without composing chars 
//screenLinesUCG[] will be 0 and screenLinesCG[][] is not used.
//When the character occupies two display cells, the next byte in screenLinesG[] is 0.
...
getPortcolorDeco(Portal *po) {
   ...
   ei (PORTAL_IS_POPUP(po)) {
      if (isInfoPopup(po))
         ...
         portalDeco = getFullDecoration(HLF_PNI);    // Pmenu
   }
   return portalDeco;
}
...
toScreenDeco(Unt hiId) {
   ...
   return (Decoration) {.hiId = hiId, .flags = deco.flags, .under = deco.under };
}
...
char_needs_redraw(int off_from, int off_to, int cols) {
   if (cols > 0
          && ((screenLinesG[off_from] != screenLinesG[off_to]
             || screenDecosG[off_from].flags != screenDecosG[off_to].flags)
         || (screenLinesUCG[off_from] != screenLinesUCG[off_to]
               || (screenLinesUCG[off_from] != 0 && comp_char_differs(off_from, off_to))
               || (mb_off2cells(off_from, off_from + cols) > 1 
                     && screenLinesG[off_from + 1] != screenLinesG[off_to + 1])))
   ) {
      return TRUE;
   } 
   return FALSE;
...
//Return the index in screenLinesG[] for the current screen line.
int
screen_get_current_line_off(void) {
   return (int)(currScreenLineS - screenLinesG);
}
...
//SLF_INC_VCOL:
//   When FALSE, use "last_vcol" for screenColsG[] of the columns to clear.
//   When TRUE, use an increasing sequence starting from "last_vcol + 1" for
//   screenColsG[] of the columns to clear.
void
screen_line(
   int row,
   int coloff,
   ...
   int charCells;      // 1: normal char 2: occupies two display cells
   ...
   off_from = (unsigned)(currScreenLineS - screenLinesG);
   off_to = lineOffsetG[row] + coloff;
   max_off_from = off_from + screenLinesColsG;
   max_off_to = lineOffsetG[row] + screenLinesColsG;
   ...
   while (col < endcol) {
      if ((col + 1 < endcol))
         charCells = mb_off2cells(off_from, max_off_from);
      else
         charCells = 1;

      redraw_this = redraw_next;
      redraw_next = force 
         || char_needs_redraw(off_from + charCells, off_to + charCells, endcol - col - charCells);
     ...
     if (redraw_this && skip_for_popup(row, col + coloff))
      ...
      if (redraw_this) {
         ...
         screenLinesG[off_to] = screenLinesG[off_from];
         ...
         if (charCells == 2)
            screenLinesG[off_to + 1] = screenLinesG[off_from + 1];

         // The bold trick makes a single column of pixels appear in the next character. When a 
         // bold character is removed, the next character should be redrawn too. This happens 
         // for some xterms.
         if ( term_is_xterm) {
         ...
         // For simplicity set the decos of 2nd half of a double-wide character same as first half
         if (charCells == 2)
            screenDecosG[off_to + 1] = screenDecosG[off_from];
         screen_char(off_to, row, col + coloff);
      }

      screenColsG[off_to] = screenColsG[off_from];
      if (charCells == 2)
         screenColsG[off_to + 1] = screenColsG[off_from + 1];
   ...
   if (clear_width > 0) {
      ...
      while (col < clear_width 
            && screenLinesG[off_to] == ' '
            && screenDecosG[off_to].flags == 0
            && screenLinesUCG[off_to] == 0
      ){
          screenColsG[off_to] = (flags & SLF_INC_VCOL) ? ++last_vcol : last_vcol;
          ++off_to;
          ++col;
      }
      if (col < clear_width) {
   ...
   if (clear_width > 0 && !(flags & SLF_POPUP)) {  // no separator for popup portal
      ...
      if (coloff + col < (int)firstPor->portalCol + (int)topframeG->width) {
         if (!skip_for_popup(row, col + coloff)) {
            Decoration deco;
            int c = fillchar_vsep(OUT &deco);
            if (screenLinesG[off_to] != (Byte)c
               || ((int)screenLinesUCG[off_to] != (c >= 0x80 ? c : 0))
               || getDecoFlags(screenDecosG[off_to].hiId) != deco.flags
            ){
               screenLinesG[off_to] = c;
               screenDecosG[off_to].hiId = HLF_C;
               screenDecosG[off_to].flags = getDecoFlags(HLF_C);
...
get_keymap_str(
   ...
   int plen = eeSnprintf(buf, len, (char *)fmt, p);
   eeglFree(s);
...
// Output a single character directly to the screen and update screenLinesG.
void
...
//Get a single character directly from screenLinesG into "bytes", which must
...
screen_getbytes(int row, int col, Byte *bytes, OUT char* decoFlags) {
   ...
   unsigned off = lineOffsetG[row] + col;
   if (decoFlags)
      *decoFlags = screenDecosG[off].flags;
   bytes[0] = screenLinesG[off];
   bytes[1] = ZERO;
...
//decorations 'deco', and update screenLinesG[] and screenDecosG[].
...
drawTextLen(
   ...
   if (!screenLinesG
      ...
      return;
   off = lineOffsetG[row] + col;

   // When drawing over the right half of a double-wide char clear out the
   // left half.  Only needed in a terminal.
   if (col > 0 && (int)col < screenLinesColsG && (int)mb_fix_col(col, row) != col) {
      if (!skip_for_popup(row, col - 1)) {
         screenLinesG[off - 1] = ' ';
         screenDecosG[off - 1].flags = 0;
         screenLinesUCG[off - 1] = 0;
         screenLinesCG[0][off - 1] = 0;
   ...
   max_off = lineOffsetG[row] + screenLinesColsG;
   while ((int)col < screenLinesColsG
      ...
      if ((need_redraw || force_redraw_this) && !skip_for_popup(row, col)) {
         ...
         if (mbyte_cells == 2) {
             screenLinesG[off + 1] = 0;
             screenDecosG[off + 1].flags = flags;
             screenColsG[off + 1] = -1;
         }
         ...
         if (clear_next_cell) {
            ...
            ptr = (CS)" ";
            len = -1;
            flags = screenDecosG[off].flags;
         }
   }

   // If we detected the next character needs to be redrawn, but the text
   // doesn't extend up to there, update the character here.
   if (force_redraw_next && (int)col < screenLinesColsG && !skip_for_popup(row, col)) {
      screen_char(off, row, col);
   }
}
...
// Reset the colors for a cterm. Used when leaving Em. The machine-specific code may override this
// again.
void
...
//Put character screenLinesG["off"] on the screen at position "row" and "col",
//using the decorations from screenDecosG["off"].
void
screen_char(unsigned off, int row, int col) {
   ...
   //scrolled up).
   if (*termCodeS[KS_XN] == ZERO 
         && row == screenLinesRowsG - 1 && col == screenLinesColsG - 1
   ) {
      screenDecosG[off].flags = 0;
      screenColsG[off] = -1;
      return;
   }
   ...
   if (screenLinesUCG[off] != 0) {
      ...
      builder[utfc_char2bytes(off, builder)] = ZERO;
      out_str(builder);
   } else {
      out_char(screenLinesG[off]);
   }
   screen_cur_col++;
}

// Draw a rectangle of the screen, inverted when "invert" is TRUE.
// This uses the contents of screenLinesG[] and doesn't change it.
void
screen_draw_rectangle(
   ...
   for (int r = row; r < row + height; ++r) {
      ...
      for (int c = col; c < col + width; ++c) {
         ...
         if (mb_off2cells(off + c, max_off) > 1)
            ++c;
      }
   }
   screen_charDeco = 0;
...
fillRowsWithTwoChars(
   ...
   if (!screenLinesG || start_row >= end_row || start_col >= end_col)
      return;

   // it's a "normal" terminal when not in a GUI or cterm
   for (Unt row = start_row; row < end_row; ++row) {
      // When drawing over the right half of a double-wide char clear out the left half. When 
      // drawing over the left half of a double wide-char clear out the right half. Only needed in 
      // a terminal.
      if (start_col > 0 && mb_fix_col(start_col, row) != start_col)
         drawTextLen((CS)" ", 1, row, start_col - 1, 0);
      if ((int)end_col < screenLinesColsG && mb_fix_col(end_col, row) != end_col)
         drawTextLen((CS)" ", 1, row, end_col, 0);
      ...
      if (c2 == ' '
         ...
         off = lineOffsetG[row] + col;
         end_off = lineOffsetG[row] + end_col;

         // skip blanks (used often, keep it fast!)
         while (off < end_off && screenLinesG[off] == ' '
                && screenDecosG[off].flags == 0 && screenLinesUCG[off] == 0
         ) {
            ++off;
         } 
         if (off < end_off) {    // something to be cleared
            col = off - lineOffsetG[row];
            drawStopHilite();
            term_windgoto(row, col);// clear rest of this screen line
            out_str(termCodeS[KS_CE]);
            screen_start();      // don't know where cursor is now
            col = end_col - col;
            while (col--) {     // clear chars in screenLinesG
      ...
      for (col = start_col; col < end_col; ++col) {
         if ((screenLinesG[off] != c
             || ((int)screenLinesUCG[off] != (c >= 0x80 ? c : 0))
             || screenDecosG[off].flags != decoFlags
...
screen_valid(int doclear) {
   ...
   return (screenLinesG != NULL);
}

//Resize the shell to visibleRowsG and visibleColsG.
//Allocate screenLinesG[] and associated items.
//
//There may be some time between setting visibleRowsG and visibleColsG and (re)allocating
//screenLinesG[]. This happens when starting up and when (manually) changing
//the shell size. Always use screenLinesRowsG and screenLinesColsG to access items
//in screenLinesG[]. Use visibleRowsG and visibleColsG for positioning text etc. where the
...
retry:
   ...
   //- Allocate new arrays for screenLinesG and screenDecosG.
   //- Move lines from the old arrays into the new arrays, clear extra
   //   lines (unless the screen is going to be cleared).
   //- Free the old arrays.
   //
   //If anything fails, make screenLinesG NULL, so we don't do anything!
   //Continuing with the old screenLinesG may result in a crash, because the size is wrong.
   ...
   new_screenColsG = LALLOC_CLEAR_MULT(ColNr, (visibleRowsG + 1) * visibleColsG);
   new_lineOffsetG = LALLOC_MULT(unsigned, visibleRowsG);
   new_lineWrapsG = LALLOC_MULT(Byte, visibleRowsG);
   ...
   if (new_screenLinesG == NULL
       || (new_screenLinesUCG == NULL || found_null)
       || new_screenDecosG == NULL
       || new_screenColsG == NULL
       || new_lineOffsetG == NULL
       || new_lineWrapsG == NULL
       || new_tabInds == NULL
       || new_popupMaskG == NULL
      ...
      EE_CLEAR(new_screenDecosG);
      EE_CLEAR(new_screenColsG);
      EE_CLEAR(new_lineOffsetG);
      EE_CLEAR(new_lineWrapsG);
      ...
      for (new_row = 0; new_row < visibleRowsG; ++new_row) {
         new_lineOffsetG[new_row] = new_row * visibleColsG;
         ...
         (void)memset(
               new_screenDecosG + new_row * visibleColsG, 0, (Unt)visibleColsG * sizeof(char)
         );
         (void)memset(
               new_screenColsG + new_row * visibleColsG, 0, (Unt)visibleColsG * sizeof(ColNr)
         );
         ...
         if (!doclear) {
            ...
            if (old_row >= 0 && screenLinesG) {
               ...
               if (screenLinesUCG) {
                  mch_memmove(new_screenLinesG + new_lineOffsetG[new_row],
                     screenLinesG + lineOffsetG[old_row], (Unt)len * sizeof(Byte)
                  );
               } 
               if (screenLinesUCG) {
               ...
               mch_memmove(new_screenDecosG + new_lineOffsetG[new_row],
                  screenDecosG + lineOffsetG[old_row], (Unt)len
               );
               mch_memmove(new_screenColsG + new_lineOffsetG[new_row],
                  screenDecosG + lineOffsetG[old_row], (Unt)len * sizeof(ColNr)
               );
            }
         }
      }
   ...
   screenLinesG = new_screenLinesG;
   screenLinesUCG = new_screenLinesUCG;
   for (int i = 0; i < MAX_COMBINED_SYMBOLS; ++i)
      screenLinesCG[i] = new_screenLinesCG[i];
   screenDecosG = new_screenDecosG;
   screenColsG = new_screenColsG;
   lineOffsetG = new_lineOffsetG;
   lineWrapsG = new_lineWrapsG;
   ...
   // size of screenLinesG[].  Set them before calling anything.
...
free_screenlines(void) {
   ...
   EE_CLEAR(screenLinesG);
   EE_CLEAR(screenDecosG);
   EE_CLEAR(screenColsG);
   EE_CLEAR(lineOffsetG);
   EE_CLEAR(lineWrapsG);
   EE_CLEAR(tabIndsG);
   EE_CLEAR(popupMaskG);
...
screenclear2(int doclear) {
   if (starting == NO_SCREEN || !screenLinesG)
      return FALSE;
      
   int did_clear = FALSE;

   activeDecoS = EMPTY_DECO;   // force setting the Normal colors
   drawStopHilite();   // don't want hiliting here

   // disable selection without redrawing it
   clip_scroll_selection(9999);

   // blank out screenLinesG
   for (int i = 0; i < visibleRowsG; ++i) {
      lineclear(lineOffsetG[i], (int)visibleColsG, 0);
      lineWrapsG[i] = FALSE;
   }
   ...
   if (doclear && can_clear(termCodeS[KS_CL])) {
      ...
      for (int i = 0; i < visibleRowsG; ++i)
         lineinvalid(lineOffsetG[i], (int)visibleColsG);
...
// Clear one line in screenLinesG.
private void
...
// Mark one line in screenLinesG invalid by setting the decorations to an invalid value.
private void
lineinvalid(unsigned off, int width) {
   (void)memset(screenDecosG + off, -1, (Unt)width);
   (void)memset(screenColsG + off, -1, (Unt)width * sizeof(ColNr));
}
...
line_was_clobbered(int screen_lnum) {
   lineinvalid(lineOffsetG[screen_lnum], (int)visibleColsG);
}
...
#define PLAN_WRITE  4
   ...
   if (activeDecoS.flags != 0 && *termCodeS[KS_MS] == ZERO)
   ...
   // 2. Use termCodeS[KS_LE] to move it a few columns to the left.
   // 3. Use NL to move a few lines down, column 0.
   // 4. Move a few columns to the right with termCodeS[KS_ND] or by writing chars.
   //
   ...
   if (row >= screen_cur_row && screen_cur_col < visibleColsG) {
      ...
      if (row == screen_cur_row && col < screen_cur_col) {
         ...
         if (*termCodeS[KS_LE])
            ...
            bs = termCodeS[KS_BC];          // "backspace character (old)
         if (*bs)
      ...
      if (cost < goto_cost && i > 0) {
         ...
         dec = screenDecosG + lineOffsetG[row] + wouldbe_col;
         ...
         for (i = wouldbe_col; i < col; ++i) {
            if (screenLinesUCG[lineOffsetG[row] + i] != 0) {
               cost = 999;
               break;
            }
         } 
      ...
      if (cost < goto_cost) {
         ...
         if (i > 0) {
            ...
            if (termCodeS[KS_ND][0] != ZERO && termCodeS[KS_ND][1] == ZERO) {
                while (i-- > 0)
               out_char(*termCodeS[KS_ND]);
            } else {
               int off = lineOffsetG[row] + screen_cur_col;
               while (i-- > 0) {
                  if (getDecoFlags(screenDecosG[off].hiId) != activeDecoS.flags)
                     drawStopHilite();
                  out_char(screenLinesG[off]);
                  ++off;
               }
...
doPortalLines(
   ...
   //Always do this in a vertically split portal. This will redraw from screenLinesG[] when 
   ...
   if (scroll_region || po->width != topframeG->width) {
      if (scroll_region && (po->width == topframeG->width || *termCodeS[KS_CSV] != ZERO))
      ...
      if (del)
         ...
         retval = 
            screen_ins_lines(po->portalRow + row, 0, line_count, po->height - row, clearHiId, po);
      if (scroll_region && (po->width == topframeG->width || *termCodeS[KS_CSV] != ZERO))
         scroll_region_reset();
      return retval;
   }
...
//insert lines on the screen and update screenLinesG[]
...
screen_ins_lines(
   ...
   //   characters from screenLinesG[].
   //1. Use termCodeS[KS_CD] (clear to end of display) if it exists and the result of
   //    the insert is just empty lines
   //2. Use termCodeS[KS_CAL] (insert multiple lines) if it exists and termCodeS[KS_AL] is not
   //    present or line_count > 1. It looks better if we do all the inserts at once.
   //3. Use termCodeS[KS_CDL] (delete multiple lines) if it exists and the result of the
   //    insert is just empty lines and termCodeS[KS_CE] is not present or line_count > 1.
   //4. Use termCodeS[KS_AL] (insert line) if it exists.
   //5. Use termCodeS[KS_CE] (erase line) if it exists and the result of the insert is
   //    just empty lines.
   //6. Use termCodeS[KS_DL] (delete line) if it exists and the result of the insert is
   //    just empty lines.
   //7. Use termCodeS[KS_SR] (scroll reverse) if it exists and inserting at row 0 and
   //    the 'da' flag is not set or we have clear line capability.
   //8. redraw the characters from screenLinesG[].
   //
   ...
   if (po && po->width != topframeG->width && *termCodeS[KS_CSV] == ZERO) {
      ...
      if (line_count > 3)
         return FAIL;
      type = USE_REDRAW;
   } ei (can_clear(termCodeS[KS_CD]) && result_empty)
      type = USE_T_CD;
   ei (*termCodeS[KS_CAL] != ZERO 
         && (line_count > 1 || *termCodeS[KS_AL] == ZERO)
   )
      type = USE_T_CAL;
   ei (*termCodeS[KS_CDL] != ZERO && result_empty && (line_count > 1 || !can_ce))
      type = USE_T_CDL;
   ei (*termCodeS[KS_AL] != ZERO)
      type = USE_T_AL;
   ei (can_ce && result_empty)
      type = USE_T_CE;
   ei (*termCodeS[KS_DL] != ZERO && result_empty)
      type = USE_T_DL;
   ei (*termCodeS[KS_SR] != ZERO && row == 0 && (*termCodeS[KS_DA] == ZERO || can_ce))
      type = USE_T_SR;
   else
      return FAIL;
   ...
   if (po && po->portalCol != 0 && *termCodeS[KS_CSV] != ZERO && *termCodeS[KS_CCS] == ZERO)
      cursor_col = po->portalCol;

   if (*termCodeS[KS_CCS] != ZERO)      // cursor relative to region
      cursor_row = row;
   else
      cursor_row = row + off;

   // Shift lineOffsetG[] line_count down to reflect the inserted lines.
   //Clear the inserted lines in screenLinesG[].
   row += off;
   end += off;
   for (i = 0; i < line_count; ++i) {
      if (po && po->width != topframeG->width) {
         ...
         temp = lineOffsetG[j];
         while ((j -= line_count) >= row) {
            lineOffsetG[j + line_count] = lineOffsetG[j];
            lineWrapsG[j + line_count] = lineWrapsG[j];
         }
   ...
   ei (type == USE_T_CAL) {
      ...
      for (i = 0; i < line_count; i++) {
         if (type == USE_T_AL) {
            if (i && cursor_row != 0)
               windgoto(cursor_row, cursor_col);
            out_str(termCodeS[KS_AL]);
         } else  // type == USE_T_SR
            out_str(termCodeS[KS_SR]);
         screen_start();       // don't know where cursor is now
      }
   }
   ...
   if (type == USE_T_SR && *termCodeS[KS_DA]) {
      for (i = 0; i < line_count; ++i) {
...
//Delete lines on the screen and update screenLinesG[].
//"end" is the line after the scrolled part. Normally it is visibleRowsG.
//When scrolling region used "off" is the offset from the top for the region.
//"row" and "end" are relative to the start of the region.
//
...
screen_del_lines(
   ...
   if (!screen_valid(TRUE)
    ...
    can_delete = (*termCodeS[KS_DB] == ZERO || can_clear(termCodeS[KS_CE]));

   //There are six ways to delete lines:
   //0. When in a vertically split portal and t_CV isn't set, redraw the
   //   characters from screenLinesG[].
   //1. Use termCodeS[KS_CD] if it exists and the result is empty.
   //2. Use newlines if row == 0 and count == 1 or termCodeS[KS_CDL] does not exist.
   //3. Use termCodeS[KS_CDL] (delete multiple lines) if it exists and line_count > 1 or
   //    none of the other ways work.
   //4. Use termCodeS[KS_CE] (erase line) if the result is empty.
   //5. Use termCodeS[KS_DL] (delete line) if it exists.
   //6. redraw the characters from screenLinesG[].
   if (po && po->width != topframeG->width && *termCodeS[KS_CSV] == ZERO) {
      ...
      type = USE_T_CD;
   ei (row == 0 && (line_count == 1 || *termCodeS[KS_CDL] == ZERO))
      type = USE_NL;
   ei (*termCodeS[KS_CDL] != ZERO && line_count > 1 && can_delete)
      type = USE_T_CDL;
   ei (can_clear(termCodeS[KS_CE]) && result_empty && (po == NULL || po->width == topframeG->width))
      type = USE_T_CE;
   ei (*termCodeS[KS_DL] != ZERO && can_delete)
      type = USE_T_DL;
   ei (*termCodeS[KS_CDL] != ZERO && can_delete)
      type = USE_T_CDL;
   else
      return FAIL;
   ...
   //Now shift lineOffsetG[] line_count up to reflect the deleted lines.
   //Clear the inserted lines in screenLinesG[].
   row += off;
   end += off;
   for (i = 0; i < line_count; ++i) {
   ...
   ei (type == USE_NL) {
      ...
      for (i = line_count; --i >= 0; ) {
         if (type == USE_T_DL) {
            windgoto(cursor_row, cursor_col);
            out_str(termCodeS[KS_DL]);      // delete a line
         } else { // type == USE_T_CE
            windgoto(cursor_row + i, cursor_col);
            out_str(termCodeS[KS_CE]);      // erase a line
         }
         screen_start();      // don't know where cursor is now
      }
   }
   ...
   if (*termCodeS[KS_DB] && (type == USE_T_DL || type == USE_T_CDL)) {
          ...
          if (msg_scrolled > visibleRowsG - 5) {       // redrawing is faster
            ...
            FOR_ALL_PORTALS(po) {
               if (msg_scrolled > -1 && po->portalRow < msg_scrolled) {
                  if (po->portalRow + po->height > (Unt)msg_scrolled
                        && po->redrawType < UPD_REDRAW_TOP
                        && po->validLines > 0
                        && po->topLine == po->lines[0].bookLnum
                  ){
                     po->rowsToUpdate = msg_scrolled - po->portalRow;
                     po->redrawType = UPD_REDRAW_TOP;
                  } else {
...
text_to_screenline(Portal *po, Byte *text, int col) {
   ...
   // Store multibyte characters in screenLinesG[] et al. correctly.
   for (Byte* p = text; *p != ZERO; ) {
...
get_cursor_rel_lnum(
   Portal   *po,
   LineNr   lnum)  { // line number to get the result for
...
fold_line(
   ...
   if (po == commPortPortG) {
      screenLinesG[off] = commPortTypeG;
      screenDecosG[off].flags = getDecoFlags(HLF_AT);
      screenLinesUCG[off] = 0;
      ++col;
   }
...
# define RL_MEMSET(p, v, l) \
   ...
   while (col < (int)po->width) {
      ...
      if (c >= 0x80) {
         ...
         screenLinesG[off + col] = 0x80; // avoid storing zero
      } else {
         screenLinesUCG[off + col] = 0;
         screenLinesG[off + col] = c;
      }
      col++;
   }
   ...
   if (po->o.cursorColumn) {
      ...
      if (txtcol >= 0 && txtcol < (int)po->width) {
         screenDecosG[off + txtcol] = combineDecorations(
            screenDecosG[off + txtcol], getFullDecoration(HLF_CUC)
         );
      } 
   }
...
# define DID_FOLD 3   // updated a folded line
   ...
   // set "screenClearedS" to TRUE.  The special value MAYBE (which is still
   // non-zero and thus not FALSE) will indicate that screenclear() was not called.
   if (screenClearedS)
      screenClearedS = MAYBE;
            ...
            ))
      {
         ...
         if (j < po->height - 2) {     // not too far off
            ...
            if (po->lines[0].bookLnum != po->topLine)
                i += diff_check_fill(po, po->lines[0].bookLnum)
                             - po->topFillOld;
      ...
      if (VIsual_active) {
         ...
         if (VIsual_mode == Ctrl_V) {
            ...
            if (curPor->cursWant == MAXCOL) {
               ...
               toc = 0;
               pos.coladd = 0;
               for (pos.lnum = curPor->cursor.lnum; cursor_above
   ...
   for (;;) {
      ...
      // Safety check: if any of the height values is wrong we might go over
      // the end of lines[].
      if (idx >= visibleRowsG)
         break;
   }
   ...
   if (!eof && !didline) {
      if (lnum == po->topLine) {
         ...
         drawEndPortal(po, fillCharsG.lastline, ' ', TRUE, srow, po->height, HLF_AT);
         po->bottomLine = lnum;
      }
   } else {
      ...
      if (eof) {     // we hit the end of the file
         ...
         if (j > 0 && !po->bottFill) {
            ...
            if (row + j > po->height)
               j = po->height - row;
            drawEndPortal(po, i, i, TRUE, row, row + (int)j, HLF_DED);
            row += j;
          }
      }
      po->bottomLine = lnum;
      ...
      if (PORTAL_IS_POPUP(po))
          drawEndPortal(po, ' ', ' ', FALSE, row, po->height, HLF_AT);
      else
          drawEndPortal(po, fillCharsG.eob, ' ', FALSE, row, po->height, HLF_EOB);
  }
...
redraw_asap(int type) {
   ...
   if (ret != 2) {
      ...
      for (r = 0; r < rows; ++r) {
         mch_memmove(
            screenline + r * cols, 
            screenLinesG + lineOffsetG[commlineRowG + r], 
            (Unt)cols * sizeof(Byte)
         );
         mch_memmove(
            screenDecos + r * cols, 
            screenDecosG + lineOffsetG[commlineRowG + r], 
            (Unt)cols * sizeof(Unt)
         );
         mch_memmove(
            screenlineUC + r * cols, 
...
typedef struct {
   ...
   int row;      // row in the portal, excl portalRow
   int screen_row;   // row on the screen, incl portalRow
...
wlv_screen_line(Portal *po, DrawCtx* m, int clear_end) {
   if (m->row == 0 && po->skipCol > 0
      ...
      for (int i = 0; i < 3 && i + skip < (int)po->width; ++i) {
         ...
         screenLinesG[off] = '<';
         screenLinesUCG[off] = 0;
         screenDecosG[off].flags = getFullDecoration(HLF_AT).flags;
         ++off;
      }
   }

   screen_line(
      m->screen_row, po->portalCol, m->col, clear_end ? po->width : -po->width,
...
#  define VCOL_HLC (m->vcol - m->virtualOffset)
   ...
   // Set increasing virtual columns in screenColsG[] to set correct curswant
...
drawLineSub( DrawCtx* m, Portal* port, Subcontext* c, SubSubcontext* sc, int currSymb) {
   ...
   // Hilite the cursor column if 'cursorcolumn' is set. But don't hilite the cursor position 
   // itself.
   ...
   if (m->draw_state < WL_LINE || m->cellsToSkip <= 0) {
      // Store the character.
      screenLinesG[m->off] = currSymb;
      if (sc->mb_utf8) {
         screenLinesUCG[m->off] = sc->mb_c;
         if ((currSymb & 0xff) == 0)
            screenLinesG[m->off] = 0x80;   // avoid storing zero
         for (int i = 0; i < MAX_COMBINED_SYMBOLS; ++i) {
      ...
      if (sc->multiDeco.hiId < SHORT) {
         screenDecosG[m->off].flags = sc->multiDeco.flags;
         sc->multiDeco = EMPTY_DECO;
      } else
         screenDecosG[m->off].flags = m->charDeco.flags;
   ...
   if ((m->col >= (int)port->width)
      ...
      if (m->draw_state != WL_LINE && m->filler_todo <= 0) {
         drawEndPortal(port, '@', ' ', TRUE, m->row, port->height, HLF_AT);
      ...
      if (screen_cur_row == m->screen_row - 1
         ...
         lineWrapsG[m->screen_row - 1] = TRUE;
         ...
         // (regardless of the xn,am settings). Only do this on a fast tty.
         // Only do this if the cursor is on the current line (something has been written in it).
         // Don't do this for double-width characters. Don't do this for a portal not at the right
         // screen border.
         if (!((mb_off2cells(lineOffsetG[m->screen_row],
                  lineOffsetG[m->screen_row] + screenLinesColsG) == 2
                || mb_off2cells(
                    lineOffsetG[m->screen_row - 1] + (int)topframeG->width - 2,
                    lineOffsetG[m->screen_row] + screenLinesColsG) == 2)))
         {
            ...
            if (screen_cur_col != (int)port->width)
               screen_char(
                  lineOffsetG[m->screen_row - 1] + (unsigned)topframeG->width - 1,
                         m->screen_row - 1, (int)(topframeG->width - 1)
               );

            // When there is a multi-byte character, just output a space to keep it simple.
            if (utf8CharLens[screenLinesG[lineOffsetG[m->screen_row - 1]
                     + (topframeG->width - 1)]] > 1)
               out_char(' ');
            else
               out_char(screenLinesG[lineOffsetG[m->screen_row - 1] + (topframeG->width - 1)]);
            // force a redraw of the first char on the next line
            screenDecosG[lineOffsetG[m->screen_row]].hiId = SHORT;
            screen_start();   // don't know where cursor is now
         }
      }
      ...
      if (m->draw_state == WL_LINE && (c->areaHiliting || c->hasExtraHiliting)) {
         ...
         if (c->hasExtraHiliting && m->countExtraBytes == 0) {
            if (c->needDecoFromTerm)
               syntaxDeco = termGetDeco(port, c->lnum, m->vcol);
            else {
               syntaxDeco = EMPTY_DECO;
            }
...
drawLineOnScreen(
   ...
   m.endRow = endrow;
   m.row = startrow;
   m.screen_row = m.row + port->portalRow;
   m.fromcol = -10;
   m.tocol = MAXCOL;
   m.vcol_sbr = -1;
...
# endif
      ...
      if (term_shobuffer(port->book)) {
         ...
         m.portalDeco = termGetDeco(port, lnum, -1);
      }
