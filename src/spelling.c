...
struct SpellLang {
   SpellLang   *sl_next;   // next language
   Byte   *sl_name;   // language name "en", "en.rare", "nl", etc.
   Byte   *sl_fname;   // name of .spl file
   int      sl_add;      // TRUE if it's a .add file.

   Byte   *sl_fbyts;   // case-folded word bytes
   long   sl_fbyts_len;   // length of sl_fbyts
   idx_T   *sl_fidxs;   // case-folded word indexes
   Byte   *sl_kbyts;   // keep-case word bytes
   idx_T   *sl_kidxs;   // keep-case word indexes
   Byte   *sl_pbyts;   // prefix tree word bytes
   idx_T   *sl_pidxs;   // prefix tree word indexes

   Byte   *sl_info;   // infotext string or NULL

   Byte   sl_regions[MAXREGIONS * 2 + 1]; // table with up to 8 region names plus NUL

   Byte   *sl_midword;   // MIDWORD string or NULL

   EeSet   sl_wordcount;   // hashtable with word count, wordcount_T

   int      sl_compmax;   // COMPOUNDWORDMAX (default: MAXWLEN)
   int      sl_compminlen;   // COMPOUNDMIN (default: 0)
   int      sl_compsylmax;   // COMPOUNDSYLMAX (default: MAXWLEN)
   int      sl_compoptions;   // COMP_* flags
   ArrayList   sl_comppat;   // CHECKCOMPOUNDPATTERN items
   RegProg* sl_compprog; // COMPOUNDRULE turned into a regexp progrm (NULL when no compounding)
   Arr(Byte) sl_comprules;   // all COMPOUNDRULE concatenated (or NULL)
   Byte   *sl_compstartflags; // flags for first compound word
   Byte   *sl_compallflags; // all flags for compound words
   Byte   nobreak;   // When TRUE: no spaces between words
   Byte   *sl_syllable;   // SYLLABLE repeatable chars or NULL
   ArrayList   sl_syl_items;   // syllable items

   int      sl_prefixcnt;   // number of items in "sl_prefprog"
   RegProg   **sl_prefprog;   // table with regprogs for prefixes

   ArrayList   sl_rep;      // list of FromTo entries from REP lines
   short   sl_rep_first[256];  // indexes where byte first appears, -1 if there is none
   ArrayList   sl_sal;      // list of SalItem entries from SAL lines
   ...
   ArrayList   sl_repsal;   // list of FromTo entries from REPSAL lines
   ...
   Byte   *sl_sbyts;   // soundfolded word bytes
   idx_T   *sl_sidxs;   // soundfolded word indexes
   Book* sl_sugbuf;   // buffer with word number table
   int      sl_sugloaded;   // TRUE when .sug file was loaded or failed to load

    int      sl_has_map;   // TRUE if there is a MAP line
    EeSet sl_map_hash;   // MAP for multi-byte chars
    int      sl_map_array[256]; // MAP for first 256 chars
   EeSet sl_sounddone; // table of soundfolded words that have handled, see add_sound_suggest()
};
...
struct SpellTab {
   Byte  st_isw[256];   // flags: is word char
   Byte  st_isu[256];   // flags: is uppercase char
   Byte  st_fold[256];   // chars: folded case
   Byte  st_upper[256];   // chars: upper case
};
...
typedef struct {
   Byte   *sm_lead;   // leading letters
   int      sm_leadlen;   // length of "sm_lead"
   Byte   *sm_oneof;   // letters from () or NULL
   Byte   *sm_rules;   // rules like ^, $, priority
   Byte   *sm_to;      // replacement.
...
#define WF_PFX_COMPPERMIT (WFP_COMPPERMIT << 24) // postponed prefix with COMPOUNDPERMITFLAG
#define WF_PFX_COMPFORBID (WFP_COMPFORBID << 24) // postponed prefix with COMPOUNDFORBIDFLAG

#define SPL_FNAME_TMPL  "%s.ut8.spl"
...
typedef struct Langp {
   SpellLang   *lp_slang;   // info for this language
   SpellLang   *lp_sallang;   // language used for sound folding or NULL
   SpellLang   *lp_replang;   // language used for REP items or NULL
   int      lp_region;   // bitmask for region or REGION_ALL
} Langp;


#define LANGP_ENTRY(ga, i)   (((Langp *)(ga).c) + (i))

#define EEGLSUGMAGIC "EEGLsug"   // string at start of Eegl .sug file
#define EEGLSUGMAGICL 6
#define EEGLSUGVERSION 1
...
#define SPELL_TOUPPER(c) ((c) >= 128 ? utf_toupper(c) \
       : ((c) < 256 ? spelltab.st_upper[c] : towupper(c)))

#define SPELL_ISUPPER(c) ((c) >= 128 ? utf_isupper(c) \
...
SPELL_EXTERN SpellLang* first_lang SPELL_INIT(= NULL);

// file used for "zG" and "zW"
SPELL_EXTERN Byte   *int_wordlist SPELL_INIT(= NULL);

SPELL_EXTERN SpellTab   spelltab;
SPELL_EXTERN int     did_set_spelltab;

typedef struct wordcount_S {
    Short   wc_count;       // nr of times word was seen
    Byte   wc_word[1];       // word, actually longer
} wordcount_T;
...
SPELL_EXTERN Byte   *repl_from SPELL_INIT(= NULL);
SPELL_EXTERN Byte   *repl_to SPELL_INIT(= NULL);


//}}}

private int spell_iswordp_nmw(Byte *p);
...
//
// A ZERO byte is used where the word may end.  The bytes are sorted, so that
// binary searching can be used and the ZERO bytes are at the start.  The
...
//
// Why doesn't Eegl use aspell/ispell/myspell/etc.?
...
typedef struct matchinf_S {
   Langp   *mi_lp;         // info for language and region

   // pointers to original text to be checked
   Byte   *mi_word;      // start of word being checked
   Byte   *mi_end;      // end of matching word so far
   Byte   *mi_fend;      // next char to be added to mi_fword
   Byte   *mi_cend;      // char after what was used for mi_capflags

   // case-folded text
   Byte   mi_fword[MAXWLEN + 1];   // mi_word case-folded
   ...
   Byte   mi_compflags[MAXWLEN];   // flags for compound words used
   ...
   Portal   *mi_win;      // buffer being checked

   // for NOBREAK
   int      mi_result2;      // "mi_result" without following word
   Byte   *mi_end2;      // "mi_end" without following word
} MatchInf;


private int spell_mb_isword_class(int cl);
...
private void find_word(MatchInf *mip, int mode);
private void find_prefix(MatchInf *mip, int mode);
private int fold_more(MatchInf *mip);
private void spell_load_cb(Byte *fname, void *cookie);
private int count_syllables(SpellLang *slang, Byte *word);
private void clear_midword(Portal *buf);
private void use_midword(SpellLang *lp, Portal *buf);
private int find_region(Byte *rp, Byte *region);
private void spell_soundfold_sofo(SpellLang *slang, Byte *inword, Byte *res);
private void spell_soundfold_wsal(SpellLang *slang, Byte *inword, Byte *res);
private void dump_word(
      SpellLang *slang, Byte *word, Byte *pat, Unt *dir, int round, int flags, LineNr lnum
);
private LineNr dump_prefixes(
      SpellLang *slang, Byte *word, Byte *pat, Unt *dir, int round, int flags, LineNr startlnum
);
private Byte *advance_camelcase_word(Byte *p, Portal *wp, int *is_camel_case);

//Main spell-checking function.
//"ptr" points to a character that could be the start of a word.
//"*retDeco" is set to the highlight index for a badly spelled word.  For a
   ...
   int      docount)   // count good words
{
   ...
   if (wp->ownSyntax->b_langp.len == 0)
      return 1;

   CLEAR_FIELD(mi);

   // A number is always OK.  Also skip hexadecimal numbers 0xFF99 and
   // 0X99FF.  But always do check spelling to find "3GPP" and "11 julifeest".
   if (*ptr >= '0' && *ptr <= '9') {
      if (*ptr == '0' && (ptr[1] == 'b' || ptr[1] == 'B'))
          mi.mi_end = skipbin(ptr + 2);
      ei (*ptr == '0' && (ptr[1] == 'x' || ptr[1] == 'X'))
   ...
   if (spell_iswordp(mi.mi_fend, wp)) {
      if (use_camel_case)
         ...
         } while (*mi.mi_fend != ZERO && spell_iswordp(mi.mi_fend, wp));
      }
   }
   if (capcol)
      *capcol = -1;
   ...
   for (lpi = 0; lpi < wp->ownSyntax->b_langp.len; ++lpi) {
      mi.mi_lp = LANGP_ENTRY(wp->ownSyntax->b_langp, lpi);
      ...
      if (mi.mi_lp->lp_slang->nobreak && mi.mi_result == SP_BAD && mi.mi_result2 != SP_BAD) {
   ...
   if (mi.mi_result != SP_OK) {
      ...
      if (nrlen > 0
         && (mi.mi_result == SP_BAD || mi.mi_result == SP_BANNED)
      ) 
            return nrlen;

      //When we are at a non-word character there is no error, just
      //skip over the character (try looking for a word after it).
      ei (!spell_iswordp_nmw(ptr)) {
         return utfCharLen(ptr);
      } ei (mi.mi_end == ptr) {
         // Always include at least one character.  Required for when there
         // is a mixup in "midword".
         MB_PTR_ADV(mi.mi_end); 
      } ei (mi.mi_result == SP_BAD && LANGP_ENTRY(wp->ownSyntax->b_langp, 0)->lp_slang->nobreak) {
         Byte   *p, *fp;
         int      save_result = mi.mi_result;

         // First language in 'spelllang' is NOBREAK.  Find first position
         // at which any word would be valid.
         mi.mi_lp = LANGP_ENTRY(wp->ownSyntax->b_langp, 0);
         if (mi.mi_lp->lp_slang->sl_fidxs != NULL) {
            p = mi.mi_word;
            fp = mi.mi_fword;
            for (;;) {
...
// Returns a pointer to the end of the word starting at "str". Supports camelCase words.
private Byte *
advance_camelcase_word(Byte *str, Portal *wp, int *is_camel_case) {
   ...
   return str;
    ...
    while (*end != ZERO && spell_iswordp(end, wp)) {
   c = mb_ptr2char(end);
   ...
   ei ((this_type == CHAR_UPPER && last_type == CHAR_OTHER)
      || (this_type != last_type
...
private void
find_word(MatchInf *mip, int mode) {
   idx_T   arridx = 0;
   ...
   Byte   *p;
   int      res = SP_BAD;
   SpellLang   *slang = mip->mi_lp->lp_slang;
   unsigned   flags;
   Byte   *byts;
   idx_T   *idxs;
   int      word_ends;
  ...
  idxs = slang->sl_kidxs;
   ...
   if (spell_iswordp(ptr + wlen, mip->mi_win)) {
       if (slang->sl_compprog == NULL && !slang->nobreak)
   ...
   for (len = byts[arridx - 1]; len > 0 && byts[arridx] == 0; --len, ++arridx) {
      ...
      if (mip->mi_capflags == WF_KEEPCAP
       ...
       ei (mode == FIND_PREFIX && !prefix_found) {
      ...
      if ((mode == FIND_COMPOUND || mode == FIND_KEEPCOMPOUND) && (flags & WF_BANNED) == 0) {
       ...
       } ei ((mode == FIND_COMPOUND || mode == FIND_KEEPCOMPOUND || !word_ends)) {
      ...
      if (slang->sl_compminlen > 0
            && mb_charlen_len(mip->mi_word + mip->mi_compoff, wlen - mip->mi_compoff) < slang->sl_compminlen)
         continue;
      ...
      mip->mi_compflags[mip->mi_complen + 1] = ZERO;
      if (word_ends) {
         Byte   fword[MAXWLEN];
         ...
         if (ptr == mip->mi_word)
             ...
             copySubstrToAllocation(fword, (Text){ptr, endlen[endidxcnt]});
         }
          if (!can_compound(slang, fword, mip->mi_compflags))
            continue;
      } ei (slang->sl_comprules && !match_compoundrule(slang, mip->mi_compflags))
      ...
      if (!word_ends) {
         ...
         Byte   *save_end = mip->mi_end;
         Langp   *save_lp = mip->mi_lp;
         int   lpi;
         ...
         for (lpi = 0; lpi < mip->mi_win->ownSyntax->b_langp.len; ++lpi) {
             if (slang->nobreak) {
            mip->mi_lp = LANGP_ENTRY(mip->mi_win->ownSyntax->b_langp, lpi);
            if (mip->mi_lp->lp_slang->sl_fidxs == NULL
                   || !mip->mi_lp->lp_slang->nobreak)
                continue;
             }
             ...
             // need to find the longest match, thus try with keep-case and prefix too.
             if (!slang->nobreak || mip->mi_result == SP_BAD) {
            // Find following word in keep-case tree.
            mip->mi_compoff = wlen;
            find_word(mip, FIND_KEEPCOMPOUND);

#if 0       // Disabled, a prefix must not appear halfway a compound word,
          // unless the COMPOUNDPERMITFLAG is used and then it can't be a postponed prefix.
            if (!slang->nobreak || mip->mi_result == SP_BAD) {
      ...
      res = SP_OK;
       ...
       // we separately keep the longest match without a following good word as a fall-back.
       if (nobreak_result == SP_BAD) {
      if (mip->mi_result2 > res) {
          mip->mi_result2 = res;
          mip->mi_end2 = mip->mi_word + wlen;
      } ei (mip->mi_result2 == res && mip->mi_end2 < mip->mi_word + wlen)
          mip->mi_end2 = mip->mi_word + wlen;
       } ei (mip->mi_result > res) {
      mip->mi_result = res;
      mip->mi_end = mip->mi_word + wlen;
       } ei (mip->mi_result == res && mip->mi_end < mip->mi_word + wlen)
      mip->mi_end = mip->mi_word + wlen;
...
match_checkcompoundpattern(
    Byte   *ptr,
    int      wlen,
    ArrayList   *gap)  // &sl_comppat
{
    int      i;
    Byte   *p;
    int      len;

    for (i = 0; i + 1 < gap->len; i += 2)
    {
   p = ((Byte **)gap->c)[i + 1];
...
int
can_compound(SpellLang *slang, Byte *word, Byte *flags) {
    Byte   uflags[MAXWLEN * 2];
    int      i;
    Byte   *p;
   ...
   p = uflags;
   for (i = 0; flags[i] != ZERO; ++i)
       p += mb_char2bytes(flags[i], p);
   *p = ZERO;
   p = uflags;
   
   if (!eeRegexec_prog(&slang->sl_compprog, FALSE, p, 0))
     return FALSE;
...
int
match_compoundrule(SpellLang *slang, Byte *compflags) { Byte   *p;
    int      i;
    int      c;

    // loop over all the COMPOUNDRULE entries
    for (p = slang->sl_comprules; *p != ZERO; ++p) {
    ...
    int      cond_req)   // only use prefixes with a condition
{
   ...
   if (*rp != NULL) {
       if (!eeRegexec_prog(rp, FALSE, word, 0))
      continue;
   } ei (cond_req)
       continue;
...
//For a match mip->mi_result is updated.
private void
find_prefix(MatchInf *mip, int mode) {
    idx_T   arridx = 0;
    int      len;
    int      wlen = 0;
    int      flen;
...
fold_more(MatchInf *mip) {
   ...
   while (*mip->mi_fend != ZERO && spell_iswordp(mip->mi_fend, mip->mi_win));
...
// Check case flags for a word.  Return TRUE if the word has the requested case.
int
spell_valid_case(
...
int
isSpellcheckingEnabledInPortal(Portal *wp) {
   return wp->bookOpts.spell
      && *wp->ownSyntax->spellLang != ZERO
      && wp->ownSyntax->b_langp.len > 0
      && *(char **)(wp->ownSyntax->b_langp.c) != NULL;
}

//Return TRUE and give an error if spell checking is not enabled.
private int
no_spell_checking(Portal *wp) {
   if (isSpellcheckingEnabledInPortal(wp))
      return FALSE;
...
spell_move_to(
   ...
   SpellMoveTo   behaviour,   // Behaviour of the function
   int      curline,
   OUT Unt* retDeco // return: attributes of bad word or NULL (only when "dir" is FORWARD)
){
   LineNr   lnum;
   Pos   found_pos;
   int      found_len = 0;
   ...
   while (!gotInterruptG) {
      ...
      len = memGetBookLen(wp->book, lnum);
      if (buflen < len + MAXWLEN + 2) {
         eeglFree(builder);
         buflen = len + MAXWLEN + 2;
         builder = alloc(buflen);
      }
      ...
      // Copy the line into "builder" and append the start of the next line if
      // possible.  Note: this memGetLine() may make "line" invalid, check for empty line first.
      empty_line = *skipwhite(line) == ZERO;
      STRCPY(builder, line);
      if (lnum < wp->book->mem.lineCount)
         spell_cat_line(builder + STRLEN(builder), memGetLine(wp->book, lnum + 1, FALSE), MAXWLEN);
      ...
      while (p < endp) {
         ...
         if (dir == BACKWARD
                && lnum == wp->cursor.lnum
                && !wrapped
                && (ColNr)(p - builder) >= wp->cursor.col)
            break;

         // start of word
         deco = 0;
         len = spell_check(wp, p, &deco, &capcol, FALSE);

         if (deco != 0) {
            // We found a bad word.  Check the attribute.
            if (behaviour == SMT_ALL
                  || (behaviour == SMT_BAD && deco == HLF_SPB)
                  || (behaviour == SMT_RARE && deco == HLF_SPR)
            ){
               // When searching forward only accept a bad word after the cursor.
               if (dir == BACKWARD
                   || lnum != wp->cursor.lnum
                   || (wrapped
                   || (ColNr)(curline ? p - builder + len : p - builder) > wp->cursor.col)
               ) {
                  if (has_syntax) {
                     col = (int)(p - builder);
                     (void)syn_get_id(wp, lnum, (ColNr)col, FALSE, &can_spell, FALSE);
                     if (!can_spell)
                        deco = 0;
                  } else
                     can_spell = TRUE;
      ...
      // is no match, give up.
      if (lnum == wp->cursor.lnum && wrapped)
          break;
      ...
      if (dir == BACKWARD) {
         ...
         if (lnum == wp->cursor.lnum && !found_one)
            break;
...
//"builder", blanking-out special characters.  Copy less than "maxlen" bytes.
//Keep the blanks at the start of the next line, this is used in drawLineOnScreen()
//to skip those bytes if the word was OK.
void
spell_cat_line(Byte *buf, Byte *line, int maxlen) {
   CS p = skipwhite(line);
   while (firstOccurrence((CS)"*#/\"\t", *p) != NULL)
      p = skipwhite(p + 1);
   ...
   if (n < maxlen - 1) {
      memset(buf, ' ', n);
      copySubstrToAllocation(buf +  n, (Text){p, maxlen - 1 - n});
   }
}

// Structure used for the cookie argument of doInPath().
typedef struct spelload_S {
   Byte  langName[MAXWLEN + 1];   // language name
   SpellLang *spellLang;         // resulting SpellLang struct
   int       nobreak;         // NOBREAK language found
} Cookie;

private CS spellPath = S"/usr/share/doc/eegl/spell/";

//Load word list(s) for "lang" from Eegl spell file(s).
//"lang" must be a language without the region: e.g., "en".
private void
spell_load_lang(Byte *lang) {
   Byte   fname_enc[85];
   int      r;
   Cookie cookie;
   int      round;
   ...
   STRCPY(cookie.langName, lang);
   cookie.spellLang = NULL;
   cookie.nobreak = FALSE;
   ...
   for (round = 1; round <= 2; ++round) {
      ...
      eeSnprintf(fname_enc, sizeof(fname_enc) - 5, "spell/%s.spl", lang);
      r = doInPath(spellPath, E, fname_enc, 0, spell_load_cb, &cookie);

      if (r == FAIL && *cookie.langName != ZERO) {
          // Try loading the ASCII version.
          eeSnprintf(fname_enc, sizeof(fname_enc) - 5, "spell/%s.ascii.spl", lang);
          r = doInPath(spellPath, E, fname_enc, 0, spell_load_cb, &cookie);

          if (r == FAIL && *cookie.langName != ZERO && round == 1
             && apply_autocmds(EVENT_SPELLFILEMISSING, lang,
                        curBook->currFileName, FALSE, curBook))
         continue;
          break;
      }
      break;
   ...
   if (r == FAIL) {
      smsg(
      _("Warning: Cannot find word list \"%s.spl\" or \"%s.ascii.spl\""), lang, lang);
   } ei (cookie.spellLang) {
      // At least one file was loaded, now load ALL the additions.
      STRCPY(fname_enc + STRLEN(fname_enc) - 3, "add.spl");
      doInPath(spellPath, E, fname_enc, DIP_ALL, spell_load_cb, &cookie);
    }
...
private void
int_wordlist_spl(Byte *fname) {
   eeSnprintf(fname, MAXPATHL, SPL_FNAME_TMPL, int_wordlist);
}

//Allocate a new SpellLang for language "lang".  "lang" can be NULL.
//Caller must fill "sl_next".
SpellLang *
slang_alloc(Byte *lang) {
   SpellLang* lp = ALLOC_CLEAR_ONE(SpellLang);
   if (lang)
      lp->sl_name = copyStr(lang);
   ga_init2(&lp->sl_rep, sizeof(FromTo), 10);
   ga_init2(&lp->sl_repsal, sizeof(FromTo), 10);
...
//Free the contents of a SpellLang and the structure itself.
void
slang_free(SpellLang *lp) {
    eeglFree(lp->sl_name);
    eeglFree(lp->sl_fname);
    slang_clear(lp);
    eeglFree(lp);
}

// Clear a SpellLang so that the file can be reloaded.
void
slang_clear(SpellLang *lp) {
    ArrayList   *gap;
    FromTo   *ftp;
    SalItem   *smp;
    int      i;
    ...
    EE_CLEAR(lp->sl_fbyts);
    EE_CLEAR(lp->sl_kbyts);
    EE_CLEAR(lp->sl_pbyts);

    EE_CLEAR(lp->sl_fidxs);
    EE_CLEAR(lp->sl_kidxs);
    EE_CLEAR(lp->sl_pidxs);

    for (round = 1; round <= 2; ++round) {
   gap = round == 1 ? &lp->sl_rep : &lp->sl_repsal;
   while (gap->len > 0) {
       ftp = &((FromTo *)gap->c)[--gap->len];
       eeglFree(ftp->ft_from);
       eeglFree(ftp->ft_to);
   }
   ga_clear(gap);
    }
    ...
    gap = &lp->sl_sal;
   if (lp->sl_sofo) {
   // "len" is set to 1 without adding an item for latin1
   if (gap->c != NULL)
       // SOFOFROM and SOFOTO items: free lists of wide characters.
       for (i = 0; i < gap->len; ++i)
      eeglFree(((int **)gap->c)[i]);
    } else
   // SAL items: free SalItem items
   while (gap->len > 0) {
       smp = &((SalItem *)gap->c)[--gap->len];
       eeglFree(smp->sm_lead);
       // Don't free sm_oneof and sm_rules, they point into sm_lead.
       eeglFree(smp->sm_to);
       eeglFree(smp->sm_lead_w);
       eeglFree(smp->sm_oneof_w);
       eeglFree(smp->sm_to_w);
   }
    ga_clear(gap);

    for (i = 0; i < lp->sl_prefixcnt; ++i)
   eeRegFree(lp->sl_prefprog[i]);
    lp->sl_prefixcnt = 0;
    EE_CLEAR(lp->sl_prefprog);

    EE_CLEAR(lp->sl_info);

    EE_CLEAR(lp->sl_midword);

    eeRegFree(lp->sl_compprog);
    lp->sl_compprog = NULL;
    EE_CLEAR(lp->sl_comprules);
    EE_CLEAR(lp->sl_compstartflags);
    EE_CLEAR(lp->sl_compallflags);

    EE_CLEAR(lp->sl_syllable);
    ...
    lp->sl_compminlen = 0;
    lp->sl_compsylmax = MAXWLEN;
    lp->sl_regions[0] = ZERO;
}

// Clear the info from the .sug file in "lp".
void
slang_clear_sug(SpellLang *lp) {
   EE_CLEAR(lp->sl_sbyts);
   EE_CLEAR(lp->sl_sidxs);
...
//Load one spell file and store the info into a SpellLang. Invoked through doInPath().
private void
spell_load_cb(Byte *fname, void *cookie) {
   Cookie   *slp = (Cookie *)cookie;
   SpellLang   *slang;

   slang = spell_load_file(fname, slp->langName, NULL, FALSE);
   if (slang == NULL)
   return;

    // When a previously loaded file has NOBREAK also use it for the
    // ".add" files.
   if (slp->nobreak && slang->sl_add)
   slang->nobreak = TRUE;
    ei (slang->nobreak)
   slp->nobreak = TRUE;
...
count_common_word(
   SpellLang   *lp,
   Byte   *word,
   int      len,       // word length, -1 for up to ZERO
   int      count)       // 1 to count once, 10 to init
{
   Hash   hash;
   wordcount_T   *wc;
   Byte   builder[MAXWLEN];
   Byte   *p;
   ...
   ei (len >= MAXWLEN)
      ...
      copySubstrToAllocation(builder, (Text){word, len});
      p = builder;
   }

   Text t = mbText(p);
   hash = calcHash(t);
   EeSetItem* hi = hash_lookup(&lp->sl_wordcount, t, hash);
   if (HASHITEM_EMPTY(hi)) {
      wc = alloc(offsetof(wordcount_T, wc_word) + t.len + 1);
      STRCPY(wc->wc_word, t.c);
      wc->wc_count = count;
      hash_add_item(&lp->sl_wordcount, hi, text(wc->wc_word), hash);
   } else {
      wc = HI2WC(hi);
...
//Return TRUE if byte "n" appears in "str". Like strchr() but independent of locale.
int
byte_in_str(Byte *str, int n) {
    Byte   *p;
...
typedef struct syl_item_S {
    Byte   sy_chars[SY_MAXLEN];       // the sequence of chars
    int      sy_len;
} syl_item_T;

//Truncate "slang->sl_syllable" at the first slash and put the following items in "slang->sl_syl_items".
int
init_syl_tab(SpellLang *slang) {
    Byte   *p;
    Byte   *s;
    int      l;
    syl_item_T   *syl;
    ...
    p = firstOccurrence(slang->sl_syllable, '/');
    while (p != NULL) {
   *p++ = ZERO;
   if (*p == ZERO)       // trailing slash
       break;
   s = p;
   p = firstOccurrence(p, '/');
   if (p == NULL)
   ...
   syl = ((syl_item_T *)slang->sl_syl_items.c)
                      + slang->sl_syl_items.len++;
   copySubstrToAllocation(syl->sy_chars, (Text){s, l});
   syl->sy_len = l;
    }
   return OK;
}

//Count the number of syllables in "word". When "word" contains spaces the syllables after 
//the last space are counted. Return zero if syllables are not defines.
private int
count_syllables(SpellLang *slang, Byte *word) {
   int      cnt = 0;
   int      skip = FALSE;
   Byte   *p;
   int      len;
   ...
   len = 0;
   for (i = 0; i < slang->sl_syl_items.len; ++i) {
       syl = ((syl_item_T *)slang->sl_syl_items.c) + i;
       if (syl->sy_len > len
   ...
   if (len != 0) {  // found a match, count syllable
       ...
       c = mb_ptr2char(p);
       len = utfCharLen(p);
       if (firstOccurrence(slang->sl_syllable, c) == NULL)
...
parse_spelllang(Portal *wp) {
   ...
   int      c;
   Byte   lang[MAXWLEN + 1];
   Byte   spf_name[MAXPATHL];
   int      len;
   Byte   *p;
   int      round;
   Byte   *spf;
   Byte   *use_region = NULL;
   ...
   bookStoreInRef(OUT &bufref, wp->book);
   ...
   ga_init2(&ga, sizeof(Langp), 2);
   clear_midword(wp);

   // Make a copy of 'spelllang', the SpellFileMissing autocommands may change
   // it under our fingers.
   spl_copy = copyStr(wp->ownSyntax->spellLang);

   // Loop over comma separated language names.
   for (splp = spl_copy; *splp != ZERO; ) {
      ...
      // If there is a region name let "region" point to it and remove it from the name.
      if (len > 4 && fnamecmp(lang + len - 4, ".spl") == 0) {
          filename = TRUE;

          // Locate a region and remove it from the file name.
          p = firstOccurrence(gettail(lang), '_');
          if (p != NULL && ASCII_ISALPHA(p[1]) && ASCII_ISALPHA(p[2]) && !ASCII_ISALPHA(p[3])) {
         copySubstrToAllocation(region_cp, (Text){p + 1, 2});
         ...
         dont_use_region = TRUE;
          ...
          // Check if we loaded this language before.
          FOR_ALL_SPELL_LANGS(slang)
         if (caseInsensitiveCompare(lang, slang->sl_name) == 0)
             break;
      }
      ...
      if (slang == NULL) {
         ...
         // destroying the buffer we are using or closing the portal.
         if (!bookRefValid(&bufref) || !doesPortalExistInAnyTab(wp)) {
      ...
      FOR_ALL_SPELL_LANGS(slang)
          if (filename ? fullpathcmp(lang, slang->sl_fname, FALSE, TRUE) == FPC_SAME
             : caseInsensitiveCompare(lang, slang->sl_name) == 0)
          {
         ...
         if (region_mask != 0) {
            ...
            goto theend;
             ...
             LANGP_ENTRY(ga, ga.len)->lp_slang = slang;
             LANGP_ENTRY(ga, ga.len)->lp_region = region_mask;
             ++ga.len;
   ...
   // round 2: load second name in 'spellfile. etc.
   spf = curPor->ownSyntax->spellFile;
   for (round = 0; round == 0 || *spf != ZERO; ++round) {
      if (round == 0) {
         ...
         for (c = 0; c < ga.len; ++c) {
            ...
            if (p != NULL && fullpathcmp(spf_name, p, FALSE, TRUE) == FPC_SAME)
               break;
         }
         if (c < ga.len)
            continue;
      ...
      FOR_ALL_SPELL_LANGS(slang) {
         if (fullpathcmp(spf_name, slang->sl_fname, FALSE, TRUE) == FPC_SAME)
         break;
      } 
      if (slang == NULL) {
         // Not loaded, try loading it now. The language name includes the
         // region name, the region is ignored otherwise. For int_wordlist use an arbitrary name.
         if (round == 0)
            STRCPY(lang, "internal wordlist");
         else {
            copySubstrToAllocation(lang, (Text){gettail(spf_name), MAXWLEN});
            p = firstOccurrence(lang, '.');
            if (p != NULL)
               *p = ZERO;   // truncate at ".encoding.add"
         }
      ...
      if (slang != NULL && ga_grow(&ga, 1) == OK) {
         ...
         ei (*slang->sl_regions != ZERO)
          ...
          if (region_mask != 0) {
         LANGP_ENTRY(ga, ga.len)->lp_slang = slang;
         LANGP_ENTRY(ga, ga.len)->lp_sallang = NULL;
         LANGP_ENTRY(ga, ga.len)->lp_replang = NULL;
         LANGP_ENTRY(ga, ga.len)->lp_region = region_mask;
         ++ga.len;
         use_midword(slang, wp);
          }
   ...
   ga_clear(&wp->ownSyntax->b_langp);
   wp->ownSyntax->b_langp = ga;
...
private void
clear_midword(Portal *wp) {
   CLEAR_FIELD(wp->ownSyntax->midwordFlags);
   EE_CLEAR(wp->ownSyntax->multibyteMidwordChars);
}

// Use the "sl_midword" field of language "lp" for buffer "buf".
// They add up to any currently used midword characters.
private void
use_midword(SpellLang *lp, Portal *wp) {
   Byte   *p;

   if (lp->sl_midword == NULL)       // there aren't any
      return;

   for (p = lp->sl_midword; *p != ZERO; ) {
       int       c, l, n;
       Byte  *bp;
       ...
       l = utfCharLen(p);
       if (c < 256 && l <= 2)
      wp->ownSyntax->midwordFlags[c] = TRUE;
       ei (wp->ownSyntax->multibyteMidwordChars == NULL)
      // First multi-byte char in "multibyteMidwordChars".
      wp->ownSyntax->multibyteMidwordChars = copySubstr(p, l);
       else {
      // Append multi-byte chars to "multibyteMidwordChars".
      n = (int)STRLEN(wp->ownSyntax->multibyteMidwordChars);
      bp = copySubstr(wp->ownSyntax->multibyteMidwordChars, n + l);
      if (bp != NULL) {
          eeglFree(wp->ownSyntax->multibyteMidwordChars);
          wp->ownSyntax->multibyteMidwordChars = bp;
          copySubstrToAllocation(bp + n, (Text){p, l});
      }
       }
       p += l;
    } 
...
private int
find_region(Byte *rp, Byte *region) {
   int      i;
...
captype(
    Byte   *word,
    Byte   *end)       // When NULL use up to ZERO byte.
{
    Byte   *p;
    int      c;
    int      firstcap;
   ...
   for ( ; end == NULL ? *p != ZERO : p < end; MB_PTR_ADV(p))
   if (spell_iswordp_nmw(p)) {
      c = mb_ptr2char(p);
      if (!SPELL_ISUPPER(c)) {
         // UUl -> KEEPCAP
...
spell_reload(void) {
   ...
   FOR_ALL_PORTALS(wp) {
      // Only load the wordlists when 'spelllang' is set and there is a portal into this buffer in 
      // which 'spell' is set.
      if (*wp->ownSyntax->spellLang != ZERO) {
         if (wp->bookOpts.spell) {
...
//Open a spell buffer. This is a nameless buffer that is not in the buffer list and only contains
//text lines. Can use a swapfile to reduce memory use. Most other fields are invalid! Esp. watch 
//out for string options being NULL and there is no undo info.
Book*
open_spellbuf(void) {
   Book* buf = ALLOC_CLEAR_ONE(Book);

   buf->kind = BOOK_SPELL;
   buf->o.swapFile = TRUE;   // may create a swap file
   ml_open(buf);
...
void
clear_spell_chartab(SpellTab *sp) {
   int      i;
...
// Init the chartab used for spelling.
// Called once while starting up. We don't use isalpha() but our own functions.
void
   ...
   Portal   *wp)       // buffer used
{
   ...
   if (l == 1) {
       // be quick for ASCII
       if (wp->ownSyntax->midwordFlags[*p])
      s = p + 1;      // skip a mid-word character
   } else {
      c = mb_ptr2char(p);
      if (c < 256 ? wp->ownSyntax->midwordFlags[c]
             : (wp->ownSyntax->multibyteMidwordChars != NULL
               && firstOccurrence(wp->ownSyntax->multibyteMidwordChars, c) != NULL)) {
         s = p + l;
      } 
   }
...
private int
spell_mb_isword_class(int cl) {
   return cl >= 2 && cl != 0x2070 && cl != 0x2080 && cl != 3;
}

// Return TRUE if "p" points to a word character. Wide version of spell_iswordp().
private int
spell_iswordp_w(int *p, Portal *wp) {
   int      *s;

   if (*p < 256 ? wp->ownSyntax->midwordFlags[*p]
       : (wp->ownSyntax->multibyteMidwordChars != NULL
              && firstOccurrence(wp->ownSyntax->multibyteMidwordChars, *p) != NULL))
      s = p + 1;
   else
      s = p;
...
// Case-fold "str[len]" into "buf[buflen]".  The result is ZERO terminated. Uses the character
// definitions from the .spl file. When using a multi-byte 'encoding' the length may change!
...
// ":spellrepall"
void
c_spellrepall(Invocation *invo UNUSED) {
   ...
   Unt   repl_from_len = STRLEN(repl_from);
   Unt   repl_to_len = STRLEN(repl_to);
   int      addlen = (int)(repl_to_len - repl_from_len);

   CS frompat = alloc(repl_from_len + 7);
   frompatlen = eeSnprintf(frompat, repl_from_len + 7, "\\V\\<%s\\>", repl_from);
   wrapSearchG = false;
   ...
   while (!gotInterruptG) {
      ...
      line = ml_get_curline();
      if (addlen <= 0 || STRNCMP(line + curPor->cursor.col, repl_to, repl_to_len) != 0) {
         p = alloc(ml_get_curline_len() + addlen + 1);
         mch_memmove(p, line, curPor->cursor.col);
         STRCPY(p + curPor->cursor.col, repl_to);
         STRCAT(p, line + curPor->cursor.col + repl_from_len);
         ml_replace(curPor->cursor.lnum, p, FALSE);
         changed_bytes(curPor->cursor.lnum, curPor->cursor.col);
         if (curBook->hasTextprop && addlen != 0)
            adjustPropColumns(curPor->cursor.lnum, curPor->cursor.col, addlen, APC_SUBSTITUTE);
      ...
      curPor->cursor.col += (ColNr)repl_to_len;
   }
   ...
   if (sub_nsubs == 0)
      showErrFmtMsg(_(e_not_found_str), repl_from);
   else
      do_sub_msg(FALSE);
}

// Make a copy of "word", with the first letter upper or lower cased, to
// "wcopy[MAXWLEN]".  "word" must not be empty. The result is ZERO terminated.
void
onecap_copy(
   Byte   *word,
   Byte   *wcopy,
   int      upper)       // TRUE: first letter made upper case
{
   ...
   copySubstrToAllocation(wcopy + l, (Text){p, MAXWLEN - l - 1});
}

// Make a copy of "word" with all the letters upper cased into
// "wcopy[MAXWLEN]".  The result is ZERO terminated.
void
allcap_copy(Byte *word, Byte *wcopy) {
   Byte   *s;
   Byte   *d;
   int      c;
...
int
nofold_len(Byte *fword, int flen, Byte *word) {
   Byte   *p;
   int      i = 0;
...
void
make_case_word(Byte *fword, Byte *cword, int flags) {
   if (flags & WF_ALLCAP)
...
// Soundfold a string, for soundfold(). Result is in allocated memory, NULL for an error.
CS
eval_soundfold(Byte *word) {
   Langp   *lp;
   Byte   sound[MAXWLEN];
   int      lpi;

   if (curPor->bookOpts.spell && *curPor->ownSyntax->spellLang != ZERO) {
      // Use the sound-folding of the first language that supports it.
      for (lpi = 0; lpi < curPor->ownSyntax->b_langp.len; ++lpi) {
         lp = LANGP_ENTRY(curPor->ownSyntax->b_langp, lpi);
         if (lp->lp_slang->sl_sal.len > 0) {
            // soundfold the word
            spell_soundfold(lp->lp_slang, word, FALSE, sound);
            return copyStr(sound);
         }
      }
   } 
...
//swedish name matching - survey and test of different algorithms" by Klas Erikson.
//
...
spell_soundfold(
   ...
   if (slang->sl_sofo)
      ...
      if (folded)
          ...
          (void)spell_casefold(curPor, inword, (int)STRLEN(inword), fword, MAXWLEN);
          word = fword;
      }
...
// Perform sound folding of "inword" into "res" according to SOFOFROM and SOFOTO lines.
private void
spell_soundfold_sofo(SpellLang *slang, Byte *inword, Byte *res) {
   Byte   *s;
   int      ri = 0;
   int      c;
   ...
   // The sl_sal_first[] table contains the translation for chars up to 255, sl_sal the rest.
   for (s = inword; *s != ZERO; ) {
      c = mb_cptr2char_adv(&s);
      if (utf_class(c) == 0)
         c = ' ';
...
private void
spell_soundfold_wsal(SpellLang *slang, Byte *inword, Byte *res) {
   SalItem   *smp = (SalItem *)slang->sl_sal.c;
   int      word[MAXWLEN];
   int      wres[MAXWLEN];
   int      l;
   Byte   *s;
   ...
   // Converted from C++ to C.  Added support for multi-byte chars. Changed to keep spaces.
   i = reslen = z = 0;
   while ((c = word[i]) != ZERO) {
      ...
      if (n >= 0) {
          ...
          // (c & 0xff) is ZERO.
          for (; ((ws = smp[n].sm_lead_w)[0] & 0xff) == (c & 0xff) && ws[0] != ZERO; ++n) {
         ...
         if (*s == ZERO
            || (*s == '^'
                && (i == 0 || !(word[i - 1] == ' '
                   || spell_iswordp_w(word + i - 1, curPor)))
                && (*(s + 1) != '$'
               || (!spell_iswordp_w(word + i + k0, curPor))))
            || (*s == '$' && i > 0
                && spell_iswordp_w(word + i - 1, curPor)
                && (!spell_iswordp_w(word + i + k0, curPor))))
         {
             ...
             if (slang->sl_followup && k > 1 && n0 >= 0 && p0 != '-' && word[i + k] != ZERO) {
            ...
            for ( ; ((ws = smp[n0].sm_lead_w)[0] & 0xff) == (c0 & 0xff); ++n0) {
               ...
               s++;
                ...
                if (*s == ZERO
                   // *s == '^' cuts
                   || (*s == '$' && !spell_iswordp_w(word + i + k0, curPor))) {
               if (k0 == k)
            ...
            ws = smp[n].sm_to_w;
            s = smp[n].sm_rules;
            p0 = (firstOccurrence(s, '<') != NULL) ? 1 : 0;
            if (p0 == 1 && z == 0) {
               // rule with '<' is used
               if (reslen > 0 && ws != NULL && *ws != ZERO
            ...
            z = 0;
            if (ws != NULL)
                while (*ws != ZERO && ws[1] != ZERO && reslen < MAXWLEN) {
...
// ":spellinfo"
void
c_spellinfo(Invocation *invo UNUSED) {
   int      lpi;
   Langp   *lp;
   Byte   *p;

   if (no_spell_checking(curPor))
      return;

   msg_start();
   for (lpi = 0; lpi < curPor->ownSyntax->b_langp.len && !gotInterruptG; ++lpi) {
      lp = LANGP_ENTRY(curPor->ownSyntax->b_langp, lpi);
   msg_puts(S"file: ");
...
// ":spelldump"
void
c_spelldump(Invocation *invo) {
   if (no_spell_checking(curPor))
      return;
   OptionValue optValue = optGetValue(null, S"spellang", OPT_LOCAL);

   // Create a new empty buffer in a new portal.
   executeCommLine(S"new");

   // enable spelling locally in the new portal
   optChangeAndReportError(S"spell", optBoole(true), SET_LOCAL);
   optChangeAndReportError(S"spellang",  optValue, SET_LOCAL);
   eeglFree(optValue.string);

   if (!BUFEMPTY())
      return;

   spell_dump_compl(NULL, 0, NULL, invo->forceit ? DUMPFLAG_COUNT : 0);
...
void
spell_dump_compl(
   CS pat,       // leading part of the word
   int ic,       // ignore case
   OUT Unt* dir,       // direction for adding matches
   ...
   int      n;
   int      flags;
   Byte   *region_names = NULL;       // region names being used
   ...
   //When ignoring case or when the pattern starts with capital pass this on to dump_word().
   if (pat) {
      if (ic)
         ...
         ei (n == WF_ALLCAP && STRLEN(pat) > utfCharLen(pat))
   ...
   if (do_region && region_names != NULL && pat == NULL) {
      eeSnprintf(IObuff, IOSIZE, "/regions=%s", region_names);
      ml_append(lnum++, IObuff, (ColNr)0, FALSE);
   } else
      do_region = FALSE;
   ...
   for (lpi = 0; lpi < curPor->ownSyntax->b_langp.len; ++lpi) {
      ...
      if (pat == NULL) {
          eeSnprintf(IObuff, IOSIZE, "# file: %s", slang->sl_fname);
          ml_append(lnum++, IObuff, (ColNr)0, FALSE);
      }
      ...
      for (round = 1; round <= 2; ++round) {
         ...
         arridx[0] = 0;
         curi[0] = 1;
         while (depth >= 0 && !gotInterruptG && (pat == NULL || !ins_compl_interrupted())) {
            if (curi[depth] > byts[arridx[depth]]) {
                               ...
                               & lp->lp_region) != 0))
               {
                  ...
                  // Dump the basic word if there is no prefix or when it's the first one.
                  c = (unsigned)flags >> 24;
                  if (c == 0 || curi[depth] == 2) {
               ...
               // Check if this character matches with the pattern. If not skip the whole tree 
               // below it. Always ignore case here, dump_word() will check proper case later. 
               // This isn't exactly right when length changes for multi-byte characters with
               // ignore case...
    ...
    LineNr   lnum)
{
   ...
   if (dumpflags & DUMPFLAG_COUNT) {
      ...
      EeSetItem* hi = hash_find(&slang->sl_wordcount, mbText(tw));
      if (!HASHITEM_EMPTY(hi)) {
         eeSnprintf(IObuff, IOSIZE, "%s\t%d", tw, HI2WC(hi)->wc_count);
         p = IObuff;
      }
   }

   ml_append(lnum, p, (ColNr)0, FALSE);
    } ei (((dumpflags & DUMPFLAG_ICASE)
          ? MB_STRNICMP(p, pat, STRLEN(pat)) == 0
          : STRNCMP(p, pat, STRLEN(pat)) == 0)
      && ins_compl_add_infercase(p, (int)STRLEN(p), p_ic, NULL, *dir, FALSE, 0) == OK)
...
// When "lnum" is zero add insert mode completion. Return the updated line number.
private LineNr
dump_prefixes(
   SpellLang   *slang,
   Byte   *word,       // case-folded word
   Byte   *pat,
   Unt* dir,
   int      dumpflags,
   ...
   LineNr   startlnum)
{
   ...
   int      curi[MAXWLEN];
   Byte   prefix[MAXWLEN];
   Byte   word_up[MAXWLEN];
   int      has_word_up = FALSE;
   Byte   *byts;
   idx_T   *idxs;
   LineNr   lnum = startlnum;
   int      depth;
   int      n;
   int      len;
   int      i;
      ...
      while (depth >= 0 && !gotInterruptG) {
         ...
         line_breakcheck();
            ...
            if (c == 0) {
               ...
               if (c != 0) {
                  copySubstrToAllocation(prefix + depth, (Text){word, MAXWLEN - depth - 1});
               ...
               // first letter is upper-case, but only if the prefix has a condition.
               if (has_word_up) {
                  c = valid_word_prefix(i, n, flags, word_up, slang, TRUE);
                  if (c != 0) {
                      copySubstrToAllocation(prefix + depth, (Text){word_up, MAXWLEN - depth - 1});
                      dump_word(slang, prefix, pat, dir, dumpflags,
...
// Move "p" to the end of word "start". Uses the spell-checking word characters.
Byte *
spell_to_word_end(Byte *start, Portal *win) {
    Byte  *p = start;

    while (*p != ZERO && spell_iswordp(p, win))
   MB_PTR_ADV(p);
   return p;
}
...
// Returns the number of matches.  The matches are in "matchp[]", array of allocated strings.
int
expand_spelling(
   LineNr   lnum UNUSED,
   CS   pat,
   ...
   ArrayList   ga;
   spell_suggest_list(&ga, pat, 100, TRUE);
   matches->c = ga.c;
   return ga.len;
}
...
int
valid_spelllang(CS val) {
   return valid_name(val, S".-_,@");
}

// Return TRUE if "val" is a valid 'spellfile' value.
int
valid_spellfile(CS val) {
   Byte   spf_name[MAXPATHL];
   Byte   *spf;
   Byte   *s;
   int      l;
   ...
   while (*spf != ZERO) {
      ...
      if (l >= MAXPATHL - 4 || l < 4 || STRCMP(spf_name + l - 4, ".add") != 0)
         return FALSE;
      for (s = spf_name; *s != ZERO; ++s)
          if (!eeIsFnameCharForGf(*s))
        return FALSE;
   }
   return TRUE;
}
...
did_set_spell_option(void) {
   ...
   Portal   *wp;
   FOR_ALL_PORTALS(wp) {
      if (wp->book == curBook && wp->bookOpts.spell) {
...
//
//<fileID>     8 bytes    "EEGLspell"
//<versionnr>  1 byte       EEGLSPELLVERSION
//
//
...
//<word>    N bytes    ZERO terminated common word
//
...
//            BY_FLAGS2:  End of word, <flags> and <flags2> follow. Not used in PREFIXTREE.
//            BY_INDEX:   Child of sibling is shared, <nodeidx> and <xbyte> follow.
//
...
//            omitted it's valid in all regions. Lowest bit is for region 1.
//
//<affixID>   1 byte       ID of affix that can be used with this word.  In
//            PREFIXTREE used for the required prefix ID.
//
//<prefcondnr>   2 bytes       Prefix condition number, index in <prefcond> list from HEADER.
//
//All text characters are in 'encoding', but stored as single bytes.

//Eegl .sug file format:  <SUGHEADER>
//          <SUGWORDTREE>
//          <SUGTABLE>
//
//<SUGHEADER>: <fileID> <versionnr> <timestamp>
//
//<fileID>     6 bytes     "EEGLsug"
//<versionnr>  1 byte      EEGLSUGVERSION
...
#define BY_NOFLAGS  0   // end of word without flags or region; for postponed prefix: no <pflags>
#define BY_INDEX    1   // child is shared, index follows
#define BY_FLAGS    2   // end of word, <flags> byte follows; for postponed prefix: <pflags> follows
#define BY_FLAGS2   3   // end of word, <flags> and <flags2> bytes follow; never used in prefix tree
...
#define EEGLSPELLMAGIC "EEGLspell"  // string at start of Eegl spell file
#define EEGLSPELLMAGICL   8
#define EEGLSPELLVERSION 50

// Section IDs.  Only renumber them when EEGLSPELLVERSION changes!
...
private int set_spell_finish(SpellTab   *new_st);
private int write_spell_prefcond(FILE *fd, ArrayList *gap, Unt *fwv);
private int read_region_section(FILE *fd, SpellLang *slang, int len);
private int read_charflags_section(FILE *fd);
private int read_prefcond_section(FILE *fd, SpellLang *lp);
private int read_rep_section(FILE *fd, ArrayList *gap, short *first);
private int read_sal_section(FILE *fd, SpellLang *slang);
private int read_words_section(FILE *fd, SpellLang *lp, int len);
private int read_sofo_section(FILE *fd, SpellLang *slang);
private int read_compound(FILE *fd, SpellLang *slang, int len);
private int set_sofo(SpellLang *lp, Byte *from, Byte *to);
private void set_sal_first(SpellLang *lp);
private int *mb_str2wide(Byte *s);
private int spell_read_tree(
      FILE *fd, Byte **bytsp, long *bytsp_len, idx_T **idxsp, int prefixtree, int prefixcnt
);
private idx_T read_tree_node(
      FILE *fd, Byte *byts, idx_T *idxs, int maxidx, idx_T startidx, int prefixtree, 
      int maxprefcondnr
);
private void set_spell_charflags(Byte *flags, int cnt, Byte *upp);
private void set_map_str(SpellLang *lp, Byte *map);


private CS e_afftrailing = S"Trailing text in %s line %d: %s";
private CS e_affname = S"Affix name too long in %s line %d: %s";
private CS msg_compressing = S"Compressing word tree...";

//Load one spell file and store the info into a SpellLang.
//
//This is invoked in three ways:
//- From spell_load_cb() to load a spell file for the first time.  "lang" is
//  the language name, "old_lp" is NULL.  Will allocate an SpellLang.
//- To reload a spell file that was changed.  "lang" is NULL and "old_lp"
//  points to the existing SpellLang.
//- Just after writing a .spl file; it's read back to produce the .sug file.
//  "old_lp" is NULL and "lang" is NULL.  Will allocate an SpellLang.
//
//Return the SpellLang the spell file was loaded into.  NULL for error.
SpellLang *
spell_load_file(
    Byte   *fname,
    Byte   *lang,
    ...
    int      silent)      // no error if file doesn't exist
{
   ...
   //<HEADER>: <fileID>
   for (i = 0; i < EEGLSPELLMAGICL; ++i)
      buf[i] = (c = getc(fd)) == EOF ? 0 : c;      // <fileID>
   if (STRNCMP(buf, EEGLSPELLMAGIC, EEGLSPELLMAGICL) != 0) {
      emsg(_(e_this_does_not_look_like_spell_file));
      goto endFAIL;
   }
   c = getc(fd);               // <versionnr>
   if (c < EEGLSPELLVERSION) {
      emsg(_(e_old_spell_file_needs_to_be_updated));
      goto endFAIL;
   } ei (c > EEGLSPELLVERSION) {
      emsg(_(e_spell_file_is_for_newer_version_of_eegl));
      goto endFAIL;
   }
...
//Fill in the wordcount fields for a trie. Returns the total number of words.
private void
tree_count_words(Byte *byts, idx_T *idxs) {
   int      depth;
   idx_T   arridx[MAXWLEN];
   int      curi[MAXWLEN];
   int      c;
   ...
   while (depth >= 0 && !gotInterruptG) {
      if (curi[depth] > byts[arridx[depth]]) {
         ...
         if (c == 0) {
            ...
            // Skip over any other ZERO bytes (same word with different
            // flags).
            while (byts[n + 1] == 0) {
                ++n;
                ++curi[depth];
...
suggest_load_files(void) {
   ...
   for (lpi = 0; lpi < curPor->ownSyntax->b_langp.len; ++lpi) {
      ...
      if (slang->sl_sugtime != 0 && !slang->sl_sugloaded) {
          ...
          dotp = lastOccurrence(slang->sl_fname, '.');
         ...
         goto nextone;
           ...
           * <SUGHEADER>: <fileID> <versionnr> <timestamp>
           */
          for (i = 0; i < EEGLSUGMAGICL; ++i)
         buf[i] = (c = getc(fd)) == EOF ? 0 : c;   // <fileID>
          if (STRNCMP(buf, EEGLSUGMAGIC, EEGLSUGMAGICL) != 0) {
         showErrFmtMsg(_(e_this_does_not_look_like_sug_file_str),
                             slang->sl_fname);
         goto nextone;
          }
          c = getc(fd);            // <versionnr>
          if (c < EEGLSUGVERSION) {
         showErrFmtMsg(_(e_old_sug_file_needs_to_be_updated_str), slang->sl_fname);
         goto nextone;
          } ei (c > EEGLSUGVERSION) {
         showErrFmtMsg(_(e_sug_file_is_for_newer_version_of_eegl_str), slang->sl_fname);
         goto nextone;
          }
         ...
         showErrFmtMsg(_(e_sug_file_doesnt_match_spl_file_str),
                             slang->sl_fname);
         goto nextone;
          }
   ...
   someerror:
         showErrFmtMsg(_(e_error_while_reading_sug_file_str), slang->sl_fname);
         ...
         goto someerror;
          ...
          // Read all the wordnr lists into the buffer, one ZERO terminated
          // list per line.
          ga_init2(&ga, 1, 100);
         ...
         if (ml_append_buf(slang->sl_sugbuf, (LineNr)wordnr,
                   ga.c, ga.len, TRUE) == FAIL)
             goto someerror;
          }
          ga_clear(&ga);
...
//Allocate memory, read the string into it and add a ZERO at the end.
//Return NULL when the count is zero.
//Sets "*cntp" to SP_*ERROR when there is an error, length of the result otherwise.
private CS
read_cnt_string(FILE* fd, int cnt_bytes, OUT int* cntp) {
   int      cnt = 0;
   ...
   *cntp = cnt;
   ...
   CS str = read_string(fd, cnt);
   if (!str)
      *cntp = SP_OTHERERROR;
   return str;
}

//Read SN_REGION: <regionname> ... Return SP_*ERROR flags.
private int
read_region_section(FILE *fd, SpellLang *lp, int len) {
   int      i;
   int      c = 0;
...
read_charflags_section(FILE *fd) {
   ...
   return ((flags == NULL) != (fol == NULL)) ? SP_FORMERROR : 0;
}

//Read SN_PREFCOND section. Return SP_*ERROR flags.
private int
read_prefcond_section(FILE *fd, SpellLang *lp) {
   int      cnt;
   int      i;
   int      n;
   int      c;
   ...
   lp->sl_prefprog = ALLOC_CLEAR_MULT(RegProg *, cnt);
   ...
   for (i = 0; i < cnt; ++i) {
      ...
      if (n > 0) {
         ...
         *p++ = (c = getc(fd)) == EOF ? 0 : c;       // <condstr>
         ...
         break;
          *p = ZERO;
          lp->sl_prefprog[i] = compileRegexp(buf, RE_MAGIC + RE_STRING);
      }
   }
   return 0;
}

// Read REP or REPSAL items section from "fd": <repcount> <rep> ... Return SP_*ERROR flags.
private int
read_rep_section(FILE *fd, ArrayList *gap, short *first) {
   int      cnt;
   FromTo   *ftp;
   int      i;
...
//Read SN_SAL section: <salflags> <salcount> <sal> ... Return SP_*ERROR flags.
private int
read_sal_section(FILE *fd, SpellLang *slang) {
   int      i;
   int      cnt;
   ArrayList   *gap;
   SalItem   *smp;
   ...
   gap = &slang->sl_sal;
   ga_init2(gap, sizeof(SalItem), 10);
   ...
   for (; gap->len < cnt; ++gap->len) {
      ...
      for (i = 0; i < ccnt; ++i) {
         c = getc(fd);         // <salfrom>
         if (firstOccurrence((CS)"0123456789(-<^$", c) != NULL)
            break;
         *p++ = c;
      }
      ...
      *p++ = ZERO;
      ...
      if (i < ccnt)
         ...
         *p++ = c;
      for (++i; i < ccnt; ++i)
         *p++ = (c = getc(fd)) == (Unt)EOF ? 0 : c;   // <salfrom>
      *p++ = ZERO;
...
//Read SN_WORDS: <word> ... Return SP_*ERROR flags.
private int
read_words_section(FILE *fd, SpellLang *lp, int len) {
   int      done = 0;
   int      i;
   int      c;
   Byte   word[MAXWLEN];
...
//SN_SOFO: <sofofromlen> <sofofrom> <sofotolen> <sofoto> Return SP_*ERROR flags.
private int
read_sofo_section(FILE *fd, SpellLang *slang) {
   int      cnt;
   Byte   *from, *to;
   int      res;
...
//Return SP_*ERROR flags.
private int
read_compound(FILE *fd, SpellLang *slang, int len) {
   int      todo = len;
   int      c;
   int      atstart;
   Byte   *pat;
   ...
   if (c < 0)
      return SP_TRUNCERROR;
   todo -= 2;
   ga_init2(gap, sizeof(CS), c);
   if (ga_grow(gap, c) == OK)
      while (--c >= 0) {
      ((Byte **)(gap->c))[gap->len++] = read_cnt_string(fd, 1, &cnt);
   ...
   // "a[bc]/a*b+" -> "^\(a[bc]\|a*b\+\)$".
   // Inserting backslashes may double the length, "^\(\)$<ZERO>" is 7 bytes.
   ...
   *pp = ZERO;
   ...
   slang->sl_compprog = compileRegexp(pat, RE_MAGIC + RE_STRING + RE_STRICT);
   eeglFree(pat);
   if (slang->sl_compprog == NULL)
     return SP_FORMERROR;
...
//Return SP_*ERROR flags when there is something wrong.
private int
set_sofo(SpellLang *lp, Byte *from, Byte *to) {
   int      i;
   ...
   // The list contains from-to pairs with a terminating ZERO.
   // sl_sal_first[] is used for latin1 "from" characters.
   gap = &lp->sl_sal;
   ga_init2(gap, sizeof(int *), 1);
   if (ga_grow(gap, 256) == FAIL)
      return SP_OTHERERROR;
   memset(gap->c, 0, sizeof(int *) * 256);
   gap->len = 256;

   // First count the number of items for each list.  Temporarily use
   // sl_sal_first[] for this.
   for (p = from, s = to; *p != ZERO && *s != ZERO; ) {
       c = mb_cptr2char_adv(&p);
       MB_CPTR_ADV(s);
       if (c >= 256)
      ++lp->sl_sal_first[c & 0xff];
   }
   if (*p != ZERO || *s != ZERO)       // lengths differ
      return SP_FORMERROR;
   ...
   for (p = from, s = to; *p != ZERO && *s != ZERO; ) {
      ...
      if (c >= 256) {
         // Append the from-to chars at the end of the list with the low byte.
         inp = ((int **)gap->c)[c & 0xff];
         while (*inp != 0)
             ++inp;
         *inp++ = c;      // from char
         *inp++ = i;      // to char
         *inp++ = ZERO;      // ZERO at the end
       }
       else
...
private void
set_sal_first(SpellLang *lp) {
    salfirst_T   *sfirst;
    int      i;
    SalItem   *smp;
    int      c;
    ArrayList   *gap = &lp->sl_sal;
   ...
   for (i = 0; i < gap->len; ++i) {
      ...
      if (sfirst[c] == -1) {
         ...
         while (i + 1 < gap->len && (*smp[i + 1].sm_lead_w & 0xff) == c)
            // Skip over entry with same index byte.
            ++i;

         for (n = 1; i + n < gap->len; ++n) {
            if ((*smp[i + n].sm_lead_w & 0xff) == c) {
               ...
               --n;
               tsal = smp[i + n];
               mch_memmove(smp + i + 1, smp + i,  sizeof(SalItem) * n);
               smp[i] = tsal;
            }
...
mb_str2wide(CS s) {
   ...
   Arr(int) res = ALLOC_MULT(int, mb_charlen(s) + 1);
...
//Allocate the memory and stores pointers in "bytsp" and "idxsp".
   ...
   int      prefixcnt)   // when "prefixtree" is TRUE: prefix count
{
   ...
   *bytsp = bp;
   ...
   ip = lallocZeroed(len * sizeof(int), TRUE);
   *idxsp = ip;
...
//
//Return the index (>= 0) following the siblings.
//Return SP_TRUNCERROR if the file is shorter than expected.
//Return SP_FORMERROR if there is a format error.
private idx_T
read_tree_node(
    FILE   *fd,
    Byte   *byts,
...
#define SHARED_MASK   0x8000000
      ...
      if (c <= BY_SPECIAL) {
         ...
         c = 0;
            ...
            if (prefixtree) {
                ...
                // byte, the condition index shifted up 8 bits, the flags shifted up 24 bits.
                if (c == BY_FLAGS)
               c = getc(fd) << 24;      // <pflags>
                else
               c = 0;
   ...
   // Skip the end-of-word ones (zero byte value) and the shared ones (and remove SHARED_MASK)
   for (i = 1; i <= len; ++i)
...
private void
spell_reload_one(CS fname, int      added_word) {  // invoked through "zg"
   SpellLang   *slang;
   int      didit = FALSE;
   ...
   // When "zg" was used and the file wasn't loaded yet, should redo @spelllang to load it now.
   if (added_word && !didit)
...
typedef struct afffile_S {
    Byte   *af_enc;   // "SET", normalized, alloc'ed string or NULL
    int      flagtype;   // AFT_CHAR, AFT_LONG, AFT_NUM or AFT_CAPLONG
    unsigned   rare;   // RARE ID for rare word
    unsigned   keepcase;   // KEEPCASE ID for keep-case word
    unsigned   af_bad;      // BAD ID for banned word
    unsigned   af_needaffix;   // NEEDAFFIX ID
    unsigned   af_circumfix;   // CIRCUMFIX ID
    unsigned   needcomp;   // NEEDCOMPOUND ID
    unsigned   comproot;   // COMPOUNDROOT ID
    unsigned   af_compforbid;   // COMPOUNDFORBIDFLAG ID
    unsigned   af_comppermit;   // COMPOUNDPERMITFLAG ID
    unsigned   nosuggest;   // NOSUGGEST ID
    int      af_pfxpostpone;   // postpone prefixes without chop string and
            // without flags
    int      af_ignoreextra;   // IGNOREEXTRA present
    EeSet   af_pref;   // hashtable for prefixes, affheader_T
    EeSet   af_suff;   // hashtable for suffixes, affheader_T
    EeSet   af_comp;   // hashtable for compound flags, compitem_T
} AffFile;
...
typedef struct affentry_S AffEntry;
// Affix entry from ".aff" file.  Used for prefixes and suffixes.
struct affentry_S {
    AffEntry   *ae_next;   // next affix with same name/number
    Byte   *ae_chop;   // text to chop off basic word (can be NULL)
    Byte   *ae_add;   // text to add to basic word (can be NULL)
    Byte   *ae_flags;   // flags on the affix (can be NULL)
    Byte   *ae_cond;   // condition (NULL for ".")
    RegProg   *ae_prog;   // regexp program for ae_cond or NULL
    char   ae_compforbid;   // COMPOUNDFORBIDFLAG found
    char   ae_comppermit;   // COMPOUNDPERMITFLAG found
};

#define AH_KEY_LEN 17      // 2 x 8 bytes + ZERO

// Affix header from ".aff" file.  Used for af_pref and af_suff.
typedef struct affheader_S {
   Byte   ah_key[AH_KEY_LEN]; // key for hashtab == name of affix
   unsigned   ah_flag;   // affix name as number, uses "flagtype"
   int      ah_newID;   // prefix ID after renumbering; 0 if not used
   int      ah_combine;   // suffix may combine with prefix
   int      ah_follows;   // another affix block should be following
   AffEntry   *ah_first;   // first affix entry
} affheader_T;
...
typedef struct compitem_S {
   Byte   ci_key[AH_KEY_LEN]; // key for hashtab == name of compound
   unsigned   ci_flag;   // affix name as number, uses "flagtype"
...
struct sblock_S {
    ...
    Byte   sb_data[1];   // data, actually longer
};

// A node in the tree.
typedef struct wordnode_S WordNode;
struct wordnode_S {
   union {  // shared to save space
      Byte   hashkey[6];   // the hash key, only used while compressing
      int   index;      // index in written nodes (valid after first round)
   } wn_u1;
   union {  // shared to save space
      WordNode *next;   // next node with same hash key
      WordNode *wnode;   // parent node that will write this node
   } wn_u2;
   WordNode   *wn_child;   // child (next byte in word)
   WordNode  *wn_sibling;   // next sibling (alternate byte in word, always sorted)
   int      wn_refs;   // Nr. of references to this node.  Only
            // relevant for first node in a list of
            // siblings, in following siblings it is always one.
   Byte   wn_byte;   // Byte for this node. ZERO for word end

    // Info for when "wn_byte" is ZERO.
    ...
    Byte   wn_affixID;   // supported/required prefix ID or 0
    Short   wn_flags;   // WF_ flags
...
#define HI2WN(hi)    (WordNode *)((hi)->hi_key)

// Info used while reading the spell files.
typedef struct spellinfo_S {
   WordNode   *si_foldroot;   // tree with case-folded words
   long   si_foldwcount;   // nr of words in si_foldroot

   WordNode   *si_keeproot;   // tree with keep-case words
   long   si_keepwcount;   // nr of words in si_keeproot

   WordNode   *si_prefroot;   // tree with postponed prefixes

   long   si_sugtree;   // creating the soundfolding trie

   sblock_T   *si_blocks;   // memory blocks used
   long   si_blocks_cnt;   // memory blocks allocated
   int      si_anyEmsgG;   // TRUE when ran out of memory

   long   si_compress_cnt;    // words to add before lowering compression limit
   WordNode   *si_first_free; // List of nodes that have been freed during
                             // compression, linked by "wn_child" field.
   long   si_free_count;   // number of nodes in si_first_free
#ifdef SPELL_PRINTTREE
   int      si_wordnode_nr;   // sequence nr for nodes
#endif
   Book   *si_spellbuf;   // buffer used to store soundfold word table
   ...
   Byte   *si_info;   // info text chars or NULL
   int      si_region_count; // number of regions supported (1 when there
            // are no regions)
   Byte   si_region_name[MAXREGIONS * 2 + 1];
           // region names; used only if
           // si_region_count > 1)

   ArrayList   si_rep;      // list of FromTo entries from REP lines
   ArrayList   si_repsal;   // list of FromTo entries from REPSAL lines
   ArrayList   si_sal;      // list of FromTo entries from SAL lines
   Byte   *si_sofofr;   // SOFOFROM text
   Byte   *si_sofoto;   // SOFOTO text
   int      si_nosugfile;   // NOSUGFILE item found
   int      si_nosplitsugs;   // NOSPLITSUGS item found
   int      si_nocompoundsugs; // NOCOMPOUNDSUGS item found
   int      si_followup;   // soundsalike: ?
   int      si_collapse;   // soundsalike: ?
   EeSet   si_commonwords;   // hashtable for common words
   time_t   si_sugtime;   // timestamp for .sug file
   int      si_rem_accents;   // soundsalike: remove accents
   ArrayList   si_map;      // MAP info concatenated
   Byte   *si_midword;   // MIDWORD chars or NULL
   int      si_compmax;   // max nr of words for compounding
   int      si_compminlen;   // minimal length for compounding
   int      si_compsylmax;   // max nr of syllables for compounding
   int      si_compoptions;   // COMP_ flags
   ArrayList   si_comppat;   // CHECKCOMPOUNDPATTERN items, each stored as
           // a string
   Byte   *si_compflags;   // flags used for compounding
   Byte   si_nobreak;   // NOBREAK
   Byte   *si_syllable;   // syllable string
   ArrayList   si_prefcond;   // table with conditions for postponed
           // prefixes, each stored as a string
   int      si_newprefID;   // current value for ah_newID
   int      si_newcompID;   // current value for compound ID
} SpellInfo;

private int is_aff_rule(Byte **items, int itemcnt, char *rulename, int    mincount);
private void aff_process_flags(AffFile *affile, AffEntry *entry);
private int spell_info_item(Byte *s);
private unsigned affitem2flag(int flagtype, Byte *item, Byte   *fname, int lnum);
private unsigned get_affitem(int flagtype, Byte **pp);
private void process_compflags(SpellInfo *spin, AffFile *aff, Byte *compflags);
private void check_renumber(SpellInfo *spin);
private void aff_check_number(int spinval, int affval, char *name);
private void aff_check_string(Byte *spinval, Byte *affval, char *name);
private int str_equal(Byte *s1, Byte   *s2);
private void add_fromto(SpellInfo *spin, ArrayList *gap, Byte   *from, Byte *to);
private int sal_to_bool(Byte *s);
private int get_affix_flags(AffFile *affile, Byte *afflist);
private int get_pfxlist(AffFile *affile, Byte *afflist, Byte *store_afflist);
private void get_compflags(AffFile *affile, Byte *afflist, Byte *store_afflist);
private int store_aff_word(
   SpellInfo *spin, Byte *word, Byte *afflist, AffFile *affile, EeSet *ht, 
   EeSet *xht, int condit, int flags, Byte *pfxlist, int pfxlen
);
private void *getroom(SpellInfo *spin, Unt len, int align);
private Byte *getroom_save(SpellInfo *spin, Byte *s);
private int store_word(
   SpellInfo *spin, Byte *word, int flags, int region, Byte *pfxlist, int need_affix
);
private int tree_add_word(
      SpellInfo *spin, Byte *word, WordNode *tree, int flags, int region, int affixID
);
private WordNode *get_wordnode(SpellInfo *spin);
private void free_wordnode(SpellInfo *spin, WordNode *n);
private void wordtree_compress(SpellInfo *spin, WordNode *root, char *name);
private long node_compress(SpellInfo *spin, WordNode *node, EeSet *ht, long *tot);
private int node_equal(WordNode *n1, WordNode *n2);
private void clear_node(WordNode *node);
private int put_node(FILE *fd, WordNode *node, int idx, int regionmask, int prefixtree);
private int sug_filltree(SpellInfo *spin, SpellLang *slang);
private int sug_maketable(SpellInfo *spin);
private int sug_filltable(SpellInfo *spin, WordNode *node, int startwordnr, ArrayList *gap);
private int offset2bytes(int nr, Byte *buf);
private void sug_write(SpellInfo *spin, Byte *fname);
private void spell_message(SpellInfo *spin, Byte *str);
private void init_spellfile(void);
...
//Tunable parameters for when the tree is compressed.  Filled from the 'mkspellmem' option.
private long compress_start = 30000;  // memory / SBLOCKSIZE
private long compress_inc = 100;      // memory / SBLOCKSIZE
private long compress_added = 500000; // word count

//Check the @mkspellmem option. Return FAIL if it's wrong. Set "sps_flags".
int
...
#define PRINTSOME(l, depth, fmt, a1, a2) eeSnprintf(l + depth * PRINTWIDTH, \
       PRINTLINESIZE - PRINTWIDTH * depth, fmt, a1, a2)

private char line1[PRINTLINESIZE];
private char line2[PRINTLINESIZE];
private char line3[PRINTLINESIZE];

private void
spell_clear_flags(WordNode *node) {
    WordNode   *np;
...
private void
spell_print_node(WordNode *node, int depth) {
   if (node->wn_u1.index) {
      ...
      // do the children
      if (node->wn_byte != ZERO && node->wn_child != NULL)
...
private void
spell_print_tree(WordNode *root) {
   if (root == NULL)
      return;
...
//Read the affix file "fname". Return an AffFile, NULL for complete failure.
private AffFile *
spell_read_aff(SpellInfo *spin, Byte *fname) {
   FILE   *fd;
   AffFile   *aff;
   Byte   rline[MAXLINELEN];
   Byte   *line;
   Byte   *pc = NULL;
#define MAXITEMCNT  30
   Byte   *(items[MAXITEMCNT]);
   int      itemcnt;
   Byte   *p;
   int      lnum = 0;
   ...
   Byte   *compflags = NULL;   // COMPOUNDFLAG and COMPOUNDRULE concatenated
   Byte   *midword = NULL;   // MIDWORD value
   Byte   *syllable = NULL;   // SYLLABLE value
   Byte   *sofofrom = NULL;   // SOFOFROM value
   Byte   *sofoto = NULL;      // SOFOTO value
   ...
   eeSnprintf(IObuff, IOSIZE, _("Reading affix file %s..."), fname);
   spell_message(spin, IObuff);

   // Only do REP lines when not done in another .aff file already.
   do_rep = spin->si_rep.len == 0;

   // Only do REPSAL lines when not done in another .aff file already.
   do_repsal = spin->si_repsal.len == 0;

   // Only do SAL lines when not done in another .aff file already.
   do_sal = spin->si_sal.len == 0;

   // Only do MAP lines when not done in another .aff file already.
   do_mapline = spin->si_map.len == 0;

   //Allocate and init the AffFile structure.
   aff = (AffFile *)getroom(spin, sizeof(AffFile), TRUE);
   if (aff == NULL) { 
      fclose(fd);
      return NULL;
   }
   ...
   while (!eeFgets(rline, MAXLINELEN, fd) && !gotInterruptG) {
      line_breakcheck();
      ++lnum;
      ...
      // Split the line up in white separated items. Put a ZERO after each item.
      itemcnt = 0;
      for (p = line; ; ) {
          while (*p != ZERO && *p <= ' ')  // skip white space and CR/NL
         ++p;
          if (*p == ZERO)
         break;
      ...
      if (itemcnt > 0) {
         if (is_aff_rule(items, itemcnt, "FLAG", 2) && aff->flagtype == AFT_CHAR) {
            if (STRCMP(items[1], "long") == 0)
                aff->flagtype = AFT_LONG;
            ei (STRCMP(items[1], "num") == 0)
                aff->flagtype = AFT_NUM;
            ei (STRCMP(items[1], "caplong") == 0)
                aff->flagtype = AFT_CAPLONG;
            else
         ...
         } ei (is_aff_rule(items, itemcnt, "MIDWORD", 2) && midword == NULL) {
            midword = getroom_save(spin, items[1]);
         } ei (is_aff_rule(items, itemcnt, "TRY", 2)) {
         // ignored, we look in the tree for what chars may appear
         }
          // TODO: remove "RAR" later
         ei ((is_aff_rule(items, itemcnt, "RAR", 2)
            || is_aff_rule(items, itemcnt, "RARE", 2)) && aff->rare == 0
         ){
            aff->rare = affitem2flag(aff->flagtype, items[1], fname, lnum);
         }
         // TODO: remove "KEP" later
         ei ((is_aff_rule(items, itemcnt, "KEP", 2)
            || is_aff_rule(items, itemcnt, "KEEPCASE", 2))
                          && aff->keepcase == 0
         ) {
            aff->keepcase = affitem2flag(aff->flagtype, items[1], fname, lnum);
         } ei ((is_aff_rule(items, itemcnt, "BAD", 2)
            || is_aff_rule(items, itemcnt, "FORBIDDENWORD", 2)) && aff->af_bad == 0
         ) {
            aff->af_bad = affitem2flag(aff->flagtype, items[1], fname, lnum);
         } ei (is_aff_rule(items, itemcnt, "NEEDAFFIX", 2) && aff->af_needaffix == 0) {
            aff->af_needaffix = affitem2flag(aff->flagtype, items[1], fname, lnum);
         } ei (is_aff_rule(items, itemcnt, "CIRCUMFIX", 2) && aff->af_circumfix == 0) {
            aff->af_circumfix = affitem2flag(aff->flagtype, items[1], fname, lnum);
         } ei (is_aff_rule(items, itemcnt, "NOSUGGEST", 2) && aff->nosuggest == 0) {
            aff->nosuggest = affitem2flag(aff->flagtype, items[1], fname, lnum);
         } ei ((is_aff_rule(items, itemcnt, "NEEDCOMPOUND", 2)
            || is_aff_rule(items, itemcnt, "ONLYINCOMPOUND", 2))
                          && aff->needcomp == 0
         ) {
            aff->needcomp = affitem2flag(aff->flagtype, items[1], fname, lnum);
         } ei (is_aff_rule(items, itemcnt, "COMPOUNDROOT", 2) && aff->comproot == 0) {
            aff->comproot = affitem2flag(aff->flagtype, items[1], fname, lnum);
         } ei (is_aff_rule(items, itemcnt, "COMPOUNDFORBIDFLAG", 2) && aff->af_compforbid == 0
         ){
            aff->af_compforbid = affitem2flag(aff->flagtype, items[1], fname, lnum);
            if (aff->af_pref.count > 0)
               smsg(_("Defining COMPOUNDFORBIDFLAG after PFX item may give wrong results"
                       " in %s line %d"), fname, lnum
               );
         } ei (is_aff_rule(items, itemcnt, "COMPOUNDPERMITFLAG", 2) && aff->af_comppermit == 0) {
            aff->af_comppermit = affitem2flag(aff->flagtype, items[1], fname, lnum);
            if (aff->af_pref.count > 0)
               smsg(
                   _("Defining COMPOUNDPERMITFLAG after PFX item may give wrong results"
                     " in %s line %d"), fname, lnum
               );
         } ei (is_aff_rule(items, itemcnt, "COMPOUNDFLAG", 2) && compflags == NULL) {
         ...
         } ei (is_aff_rule(items, itemcnt, "COMPOUNDRULES", 2)) {
            // We don't use the count, but do check that it's a number and
            // not COMPOUNDRULE mistyped.
            if (atoi((char *)items[1]) == 0)
                smsg(_("Wrong COMPOUNDRULES value in %s line %d: %s"), fname, lnum, items[1]);
         } ei (is_aff_rule(items, itemcnt, "COMPOUNDRULE", 2)) {
            // Don't use the first rule if it is a number.
            if (compflags != NULL || *skipdigits(items[1]) != ZERO) {
         ...
         } ei (is_aff_rule(items, itemcnt, "COMPOUNDWORDMAX", 2) && compmax == 0) {
            compmax = atoi((char *)items[1]);
            if (compmax == 0)
                smsg(_("Wrong COMPOUNDWORDMAX value in %s line %d: %s"),
                               fname, lnum, items[1]);
         } ei (is_aff_rule(items, itemcnt, "COMPOUNDMIN", 2) && compminlen == 0) {
            compminlen = atoi((char *)items[1]);
            if (compminlen == 0)
               smsg(_("Wrong COMPOUNDMIN value in %s line %d: %s"), fname, lnum, items[1]);
         } ei (is_aff_rule(items, itemcnt, "COMPOUNDSYLMAX", 2) && compsylmax == 0) {
            compsylmax = atoi((char *)items[1]);
            if (compsylmax == 0)
               smsg(_("Wrong COMPOUNDSYLMAX value in %s line %d: %s"), fname, lnum, items[1]);
         } ei (is_aff_rule(items, itemcnt, "CHECKCOMPOUNDDUP", 1)) {
            compoptions |= COMP_CHECKDUP;
         } ei (is_aff_rule(items, itemcnt, "CHECKCOMPOUNDREP", 1)) {
            compoptions |= COMP_CHECKREP;
         } ei (is_aff_rule(items, itemcnt, "CHECKCOMPOUNDCASE", 1)) {
            compoptions |= COMP_CHECKCASE;
         } ei (is_aff_rule(items, itemcnt, "CHECKCOMPOUNDTRIPLE", 1)) {
            compoptions |= COMP_CHECKTRIPLE;
         } ei (is_aff_rule(items, itemcnt, "CHECKCOMPOUNDPATTERN", 2)) {
            if (atoi((char *)items[1]) == 0)
                smsg(_("Wrong CHECKCOMPOUNDPATTERN value in %s line %d: %s"),
                               fname, lnum, items[1]);
         } ei (is_aff_rule(items, itemcnt, "CHECKCOMPOUNDPATTERN", 3)) {
            ArrayList    *gap = &spin->si_comppat;
            int       i;

            // Only add the couple if it isn't already there.
            for (i = 0; i < gap->len - 1; i += 2)
                if (STRCMP(((Byte **)(gap->c))[i], items[1]) == 0
                   && STRCMP(((Byte **)(gap->c))[i + 1],
                                  items[2]) == 0)
               break;
            if (i >= gap->len && ga_grow(gap, 2) == OK) {
                ((Byte **)(gap->c))[gap->len++]
                            = getroom_save(spin, items[1]);
                ((Byte **)(gap->c))[gap->len++]
                            = getroom_save(spin, items[2]);
            }
         } ei (is_aff_rule(items, itemcnt, "SYLLABLE", 2) && syllable == NULL) {
            syllable = getroom_save(spin, items[1]);
         } ei (is_aff_rule(items, itemcnt, "NOBREAK", 1)) {
            spin->si_nobreak = TRUE;
         } ei (is_aff_rule(items, itemcnt, "NOSPLITSUGS", 1)) {
            spin->si_nosplitsugs = TRUE;
         } ei (is_aff_rule(items, itemcnt, "NOCOMPOUNDSUGS", 1)) {
            spin->si_nocompoundsugs = TRUE;
         } ei (is_aff_rule(items, itemcnt, "NOSUGFILE", 1)) {
            spin->si_nosugfile = TRUE;
         } ei (is_aff_rule(items, itemcnt, "PFXPOSTPONE", 1)) {
            aff->af_pfxpostpone = TRUE;
         } ei (is_aff_rule(items, itemcnt, "IGNOREEXTRA", 1)) {
            aff->af_ignoreextra = TRUE;
         } ei ((STRCMP(items[0], "PFX") == 0 || STRCMP(items[0], "SFX") == 0)
             && aff_todo == 0
             && itemcnt >= 4
         ){
            int   lasti = 4;
            ...
            //"S" flag on all but the last block, thus we check for that and store it in ah_follows.
            copySubstrToAllocation(key, (Text){items[1], AH_KEY_LEN - 1});
            hi = hash_find(tp, text(key));
            if (!HASHITEM_EMPTY(hi)) {
               cur_aff = HI2AH(hi);
               ...
               if (cur_aff == NULL)
                  break;
               cur_aff->ah_flag = affitem2flag(aff->flagtype, items[1], fname, lnum);
               if (cur_aff->ah_flag == 0 || STRLEN(items[1]) >= AH_KEY_LEN)
                  break;
               if (cur_aff->ah_flag == aff->af_bad
                      || cur_aff->ah_flag == aff->rare
                      || cur_aff->ah_flag == aff->keepcase
                      || cur_aff->ah_flag == aff->af_needaffix
                      || cur_aff->ah_flag == aff->af_circumfix
                      || cur_aff->ah_flag == aff->nosuggest
                      || cur_aff->ah_flag == aff->needcomp
                      || cur_aff->ah_flag == aff->comproot)
                  smsg(_("Affix also used for BAD/RARE/KEEPCASE/NEEDAFFIX/NEEDCOMPOUND/"
                           "NOSUGGEST in %s line %d: %s"), fname, lnum, items[1]);
               STRCPY(cur_aff->ah_key, items[1]);
               hash_add(tp, mbText(cur_aff->ah_key), S"spelling");
            ...
            --aff_todo;
            aff_entry = (AffEntry *)getroom(spin,
                            sizeof(AffEntry), TRUE);
            if (aff_entry == NULL)
                break;
            ...
            if (STRCMP(items[3], "0") != 0) {
               ...
               aff_entry->ae_flags = firstOccurrence(aff_entry->ae_add, '/');
               if (aff_entry->ae_flags != NULL) {
                  *aff_entry->ae_flags++ = ZERO;
                       ...
                       || has_non_ascii(aff_entry->ae_add)))
            {
               ...
               if (STRCMP(items[4], ".") != 0) {
                  ...
                  if (*items[0] == 'P')
                      ...
                      sprintf((char *)buf, "%s$", items[4]);
                  aff_entry->ae_prog = compileRegexp(buf,
               ...
               if (*items[0] == 'P' && aff->af_pfxpostpone && aff_entry->ae_flags == NULL) {
                  ...
                  if (aff_entry->ae_chop != NULL
                     ...
                     && aff_entry->ae_chop[utfCharLen(aff_entry->ae_chop)] == ZERO
                  ){
                     Unt c = mb_ptr2char(aff_entry->ae_chop);
                     Unt c_up = SPELL_TOUPPER(c);
                     if (c_up != c
                         && (aff_entry->ae_cond == NULL || mb_ptr2char(aff_entry->ae_cond) == c)
                     ) {
                        ...
                        if (mb_ptr2char(p) == c_up) {
                            ...
                            *p = ZERO;
                            ...
                            // actual word, thus must check for the upper-case letter.
                           ...
                           if (aff_entry->ae_cond != NULL) {
                               sprintf((char *)buf, "^%s",
                                   aff_entry->ae_cond);
                               eeRegFree(aff_entry->ae_prog);
                               aff_entry->ae_prog = compileRegexp(
                  ...
                  if (aff_entry->ae_chop == NULL && aff_entry->ae_flags == NULL) {
                     ...
                     for (idx = spin->si_prefcond.len - 1; idx >= 0; --idx) {
                     p = ((Byte **)spin->si_prefcond.c)[idx];
                     ...
                     idx = spin->si_prefcond.len++;
                     pp = ((Byte **)spin->si_prefcond.c)
                                    + idx;
                     if (aff_entry->ae_cond == NULL)
                         *pp = NULL;
                     else
         ...
         } ei (is_aff_rule(items, itemcnt, "FOL", 2) && fol == NULL) {
            fol = copyStr(items[1]);
         } ei (is_aff_rule(items, itemcnt, "LOW", 2) && low == NULL) {
            low = copyStr(items[1]);
         } ei (is_aff_rule(items, itemcnt, "UPP", 2) && upp == NULL) {
            upp = copyStr(items[1]);
         } ei (is_aff_rule(items, itemcnt, "REP", 2)
              || is_aff_rule(items, itemcnt, "REPSAL", 2)
         ) {
            // Ignore REP/REPSAL count
            if (!SAFE_isdigit(*items[1]))
                smsg(_("Expected REP(SAL) count in %s line %d"), fname, lnum);
         } ei ((STRCMP(items[0], "REP") == 0 || STRCMP(items[0], "REPSAL") == 0) 
               && itemcnt >= 3
         ) {
            // REP/REPSAL item
            ...
            if (items[0][3] == 'S' ? do_repsal : do_rep) {
                ...
                // directly).
                for (p = items[1]; *p != ZERO; MB_PTR_ADV(p))
               if (*p == '_')
                   *p = ' ';
                for (p = items[2]; *p != ZERO; MB_PTR_ADV(p))
               if (*p == '_')
                   *p = ' ';
         ...
         } ei (is_aff_rule(items, itemcnt, "MAP", 2)) {
            // MAP item or count
            if (!found_map) {
               ...
               for (p = items[1]; *p != ZERO; ) {
               c = mb_ptr2char_adv(&p);
               if ((spin->si_map.len > 0
                      && firstOccurrence(spin->si_map.c, c) != NULL)
                  || firstOccurrence(p, c) != NULL)
         ...
         ei (is_aff_rule(items, itemcnt, "SAL", 3)) {
            if (do_sal) {
               // SAL item (sounds-a-like)
               // Either one of the known keys or a from-to pair.
               if (STRCMP(items[1], "followup") == 0)
                  spin->si_followup = sal_to_bool(items[2]);
               ei (STRCMP(items[1], "collapse_result") == 0)
                  spin->si_collapse = sal_to_bool(items[2]);
               ei (STRCMP(items[1], "remove_accents") == 0)
                  spin->si_rem_accents = sal_to_bool(items[2]);
               else
                  // when "to" is "_" it means empty
                  add_fromto(spin, &spin->si_sal, items[1],
                          STRCMP(items[2], "_") == 0 ? (CS)"" : items[2]);
            }
         } ei (is_aff_rule(items, itemcnt, "SOFOFROM", 2) && sofofrom == NULL) {
            sofofrom = getroom_save(spin, items[1]);
         } ei (is_aff_rule(items, itemcnt, "SOFOTO", 2) && sofoto == NULL) {
            sofoto = getroom_save(spin, items[1]);
         } ei (STRCMP(items[0], "COMMON") == 0) {
            int   i;

            for (i = 1; i < itemcnt; ++i) {
               if (HASHITEM_EMPTY(hash_find(&spin->si_commonwords, mbText(items[i])))) {
                  p = copyStr(items[i]);
                  hash_add(&spin->si_commonwords, text(p), S"spelling");
               }
            }
         } else
   ...
   if (spin->si_newcompID < spin->si_newprefID) {
      ...
      ei (spin->si_newprefID == 0 || spin->si_newprefID == 127)
 ...
 */
private void
aff_process_flags(AffFile *affile, AffEntry *entry) {
    Byte   *p;
    Byte   *prevp;
    unsigned   flag;

   if (entry->ae_flags != NULL && (affile->af_compforbid != 0 || affile->af_comppermit != 0)) {
   for (p = entry->ae_flags; *p != ZERO; ) {
       prevp = p;
       flag = get_affitem(affile->flagtype, &p);
      ...
      if (flag == affile->af_comppermit)
       ...
       if (affile->flagtype == AFT_NUM && *p == ',')
      ++p;
   }
   if (*entry->ae_flags == ZERO)
...
//Turn an affix flag name into a number, according to the FLAG type. return zero for failure.
private unsigned
affitem2flag(
    int      flagtype,
    Byte   *item,
...
//Return ZERO_FLAG for "0".
//Return zero for an error, still advances the pointer then.
private unsigned
get_affitem(int flagtype, Byte **pp) {
    int      res;
...
//spin->si_compflags.
//The processing involves changing the affix names to ID numbers, so that they fit in one byte.
private void
process_compflags( SpellInfo   *spin, AffFile   *aff, Byte   *compflags) {
    Byte   *p;
    Byte   *prevp;
    unsigned   flag;
    compitem_T   *ci;
   ...
   if (firstOccurrence((CS)"/?*+[]", *p) != NULL)
       ...
       *tp++ = *p++;
       ...
       prevp = p;
       flag = get_affitem(aff->flagtype, &p);
       if (flag != 0) {
      // Find the flag in the hashtable.  If it was used before, use
      // the existing ID.  Otherwise add a new entry.
      copySubstrToAllocation(key, (Text){prevp, p - prevp});
      hi = hash_find(&aff->af_comp, mbText(key));
      if (!HASHITEM_EMPTY(hi))
         ...
         id = spin->si_newcompID--;
          } while (firstOccurrence(S"/?*+[]\\-^", id) != NULL);
          ci->ci_newID = id;
          hash_add(&aff->af_comp, mbText(ci->ci_key), S"spelling");
      }
      *tp++ = id;
       }
       if (aff->flagtype == AFT_NUM && *p == ',')
      ++p;
   }
    }
...
private int
flag_in_afflist(int flagtype, Byte *afflist, unsigned flag) {
   Byte   *p;
   Unt   n;

   switch (flagtype) {
   case AFT_CHAR:
      return firstOccurrence(afflist, flag) != NULL;

   case AFT_CAPLONG:
   case AFT_LONG:
       for (p = afflist; *p != ZERO; ) {
      n = mb_ptr2char_adv(&p);
      if ((flagtype == AFT_LONG || (n >= 'A' && n <= 'Z')) && *p != ZERO)
...
private void
aff_check_string(Byte *spinval, Byte *affval, char *name) {
   if (spinval != NULL && STRCMP(spinval, affval) != 0)
   smsg(_("%s value differs from what is used in another .aff file"), name);
}

//Return TRUE if strings "s1" and "s2" are equal.  Also consider both being NULL as equal.
private int
str_equal(Byte *s1, Byte *s2) {
   if (s1 == NULL || s2 == NULL)
      return s1 == s2;
   return STRCMP(s1, s2) == 0;
}

//Add a from-to item to "gap".  Used for REP and SAL items. They are stored case-folded.
private void
add_fromto(
   SpellInfo   *spin,
   ArrayList   *gap,
   ...
   if (ga_grow(gap, 1) == FAIL)
    ...
    ftp = ((FromTo *)gap->c) + gap->len;
    (void)spell_casefold(curPor, from, (int)STRLEN(from), word, MAXWLEN);
    ftp->ft_from = getroom_save(spin, word);
    (void)spell_casefold(curPor, to, (int)STRLEN(to), word, MAXWLEN);
...
//Read dictionary file "fname". Return OK or FAIL;
private int
spell_read_dic(SpellInfo *spin, Byte *fname, AffFile *affile) {
   EeSet   ht;
   Byte   line[MAXLINELEN];
   Byte   *p;
   Byte   *afflist;
   Byte   store_afflist[MAXWLEN];
   int      pfxlen;
   int      need_affix;
   Byte   *dw;
   Byte   *pc;
   ...
   int      non_ascii = 0;
   int      retval = OK;
   Byte   message[MAXLINELEN + MAXWLEN];
   int      flags;
   int      duplicate = 0;
   Tyme   last_msg_time = 0;
   ...
   eeSnprintf(IObuff, IOSIZE, _("Reading dictionary file %s..."), fname);
   spell_message(spin, IObuff);
   ...
   if (eeFgets(line, MAXLINELEN, fd) || !eeIsDigit(*skipwhite(line)))
   showErrFmtMsg(_(e_no_word_count_in_str), fname);

   //Read all the lines in the file one by one.
   //The words are converted to 'encoding' here, before being added to the hashtable.
   while (!eeFgets(line, MAXLINELEN, fd) && !gotInterruptG) {
      line_breakcheck();
      ++lnum;
      if (line[0] == '#' || line[0] == '/')
      ...
      if (spin->si_verbose && spin->si_msg_count > 10000) {
         spin->si_msg_count = 0;
         if (eeTime() > last_msg_time) {
            last_msg_time = eeTime();
            eeSnprintf(
               message, sizeof(message), _("line %6d, word %6ld - %s"), lnum, 
               spin->si_foldwcount + spin->si_keepwcount, w
            );
            msg_start();
            outputShortenedToALine(text(message), 0);
            msg_clr_eos();
            msg_didout = FALSE;
            msgColG = 0;
            out_flush();
      ...
      if (!HASHITEM_EMPTY(hi)) {
         ...
         hash_add_item(&ht, hi, tw, hash);

      flags = 0;
      store_afflist[0] = ZERO;
      pfxlen = 0;
      need_affix = FALSE;
      if (afflist != NULL) {
         ...
         if (affile->af_needaffix != 0 && flag_in_afflist(
             affile->flagtype, afflist, affile->af_needaffix))
         need_affix = TRUE;
...
//Check for affix flags in "afflist" that are turned into word flags. Return WF_ flags.
private int
get_affix_flags(AffFile *affile, Byte *afflist) {
   Unt      flags = 0;

   if (affile->keepcase != 0 && flag_in_afflist( affile->flagtype, afflist, affile->keepcase))
      flags |= WF_KEEPCAP | WF_FIXCAP;
   if (affile->rare != 0 && flag_in_afflist(affile->flagtype, afflist, affile->rare))
      flags |= WF_RARE;
   if (affile->af_bad != 0 && flag_in_afflist(affile->flagtype, afflist, affile->af_bad))
      flags |= WF_BANNED;
   if (affile->needcomp != 0 && flag_in_afflist(affile->flagtype, afflist, affile->needcomp))
      flags |= WF_NEEDCOMP;
   if (affile->comproot != 0 && flag_in_afflist(affile->flagtype, afflist, affile->comproot))
      flags |= WF_COMPROOT;
   if (affile->nosuggest != 0 && flag_in_afflist(affile->flagtype, afflist, affile->nosuggest))
      flags |= WF_NOSUGGEST;
   return flags;
}

//Get the list of prefix IDs from the affix list "afflist". Used for PFXPOSTPONE.
//Put the resulting flags in "store_afflist[MAXWLEN]" with a terminating ZERO
...
get_pfxlist(
   ...
   for (p = afflist; *p != ZERO; ) {
      prevp = p;
      if (get_affitem(affile->flagtype, &p) != 0) {
          //A flag is a postponed prefix flag if it appears in "af_pref" and its ID is not 0.
          copySubstrToAllocation(key, (Text){prevp, p - prevp});
          hi = hash_find(&affile->af_pref, text(key));
          if (!HASHITEM_EMPTY(hi)) {
    ...
    store_afflist[cnt] = ZERO;
   return cnt;
}

//Get the list of compound IDs from the affix list "afflist" that are used
//for compound words. Put the flags in "store_afflist[]".
private void
get_compflags(
    AffFile   *affile,
    Byte   *afflist,
    Byte   *store_afflist)
{
   ...
   if (get_affitem(affile->flagtype, &p) != 0) {
       ...
       copySubstrToAllocation(key, (Text){prevp, p - prevp});
       hi = hash_find(&affile->af_comp, mbText(key));
       if (!HASHITEM_EMPTY(hi))
...
//"ht" is the hashtable with AffEntry that need to be applied, either prefixes or suffixes.
...
private int
store_aff_word(
   SpellInfo   *spin,      // spell info
   Byte   *word,      // basic word start
   Byte   *afflist,   // list of names of supported affixes
   AffFile   *affile,
   EeSet   *ht,
   EeSet   *xht,
   int      condit,      // CONDIT_SUF et al.
   int      flags,      // flags for the word
   Byte   *pfxlist,   // list of prefix IDs
   int      pfxlen)      // nr of flags in "pfxlist" for prefixes, rest is compound flags
{
   int      todo;
   EeSetItem   *hi;
   affheader_T   *ah;
   AffEntry   *ae;
   Byte   newword[MAXWLEN];
   int      retval = OK;
   int      i, j;
   Byte   *p;
   int      use_flags;
   ...
   int      need_affix;
   Byte   store_afflist[MAXWLEN];
   Byte   pfx_pfxlist[MAXWLEN];
   Unt   wordlen = STRLEN(word);
   int      use_condit;

   todo = (int)ht->count;
   for (hi = ht->array; todo > 0 && retval == OK; ++hi) {
      if (!HASHITEM_EMPTY(hi)) {
          --todo;
          ah = HI2AH(hi);
          ...
          if (((condit & CONDIT_COMB) == 0 || ah->ah_combine)
             && flag_in_afflist(affile->flagtype, afflist,
                            ah->ah_flag))
          {
         // Loop over all affix entries with this name.
         for (ae = ah->ah_first; ae != NULL; ae = ae->ae_next) {
             // Check the condition.  It's not logical to match case
             // here, but it is required for compatibility with Myspell.
             ...
             // must have it too, if it had not then this one must not have one either.
             if ((xht != NULL || !affile->af_pfxpostpone
               || ae->ae_chop != NULL
               || ae->ae_flags != NULL)
                && (ae->ae_chop == NULL || STRLEN(ae->ae_chop) < wordlen)
                && (ae->ae_prog == NULL
               || eeRegexec_prog(&ae->ae_prog, FALSE, word, (ColNr)0))
                && (((condit & CONDIT_CFIX) == 0)
               == ((condit & CONDIT_AFF) == 0
                   || ae->ae_flags == NULL
                   || !flag_in_afflist(affile->flagtype,
            ...
            if (xht == NULL) {
               ...
               *newword = ZERO;
               ...
               copySubstrToAllocation(newword, (Text){ae->ae_add, MAXWLEN - 1});
                p = word;
                if (ae->ae_chop != NULL) {
               // Skip chop string.
                ...
                copySubstrToAllocation(newword, (Text){word, MAXWLEN - 1});
                if (ae->ae_chop != NULL) {
               // Remove chop string.
            ...
            if (ae->ae_flags != NULL) {
                ...
                if (affile->af_needaffix != 0 && flag_in_afflist(
                  affile->flagtype, ae->ae_flags, affile->af_needaffix))
               need_affix = TRUE;

                // When there is a CIRCUMFIX flag the other affix
                // must also have it and we don't add the word with one affix.
                if (affile->af_circumfix != 0 && flag_in_afflist(
                  affile->flagtype, ae->ae_flags,
                        affile->af_circumfix))
                {
               ...
               for (i = pfxlen; pfxlist[i] != ZERO; ++i) {
                   for (j = use_pfxlen;
                        use_pfxlist[j] != ZERO; ++j)
                  if (pfxlist[i] == use_pfxlist[j])
                      break;
                   if (use_pfxlist[j] == ZERO) {
            ...
            // Obey a "COMPOUNDFORBIDFLAG" of the affix: don't use the compound flags.
            if (use_pfxlist != NULL && ae->ae_compforbid) {
                copySubstrToAllocation(pfx_pfxlist, (Text){use_pfxlist, use_pfxlen});
                use_pfxlist = pfx_pfxlist;
            }

            // When there are postponed prefixes...
            if (spin->si_prefroot && spin->si_prefroot->wn_sibling != NULL) {
                // ... add a flag to indicate an affix was used.
                use_flags |= WF_HAS_AFF;

                // ... don't use a prefix list if combining
                // affixes is not allowed.  But do use the compound flags after them.
            ...
            // When added a suffix and combining is allowed also try adding a prefix additionally.
            // Both for the word flags and for the affix flags. RECURSIVE!
...
private int
spell_read_wordfile(SpellInfo *spin, Byte *fname) {
   long   lnum = 0;
   Byte   rline[MAXLINELEN];
   Byte   *line;
   Byte   *pc = NULL;
   ...
   eeSnprintf(IObuff, IOSIZE, _("Reading word file %s..."), fname);
   spell_message(spin, IObuff);
   ...
   while (!eeFgets(rline, MAXLINELEN, fd) && !gotInterruptG) {
      ...
      p = firstOccurrence(line, '/');
      if (p != NULL) {
          *p++ = ZERO;
          while (*p != ZERO) {
         if (*p == '=')      // keep-case word
             flags |= WF_KEEPCAP | WF_FIXCAP;
         ei (*p == '!')   // Bad, bad, wicked word.
             flags |= WF_BANNED;
         ei (*p == '?')   // Rare word.
             flags |= WF_RARE;
         ei (EE_ISDIGIT(*p)) {// region number(s)
   ...
   if (spin->si_ascii && non_ascii > 0) {
      eeSnprintf(IObuff, IOSIZE, _("Ignored %d words with non-ASCII characters"), non_ascii);
      spell_message(spin, IObuff);
   }
...
//This avoids calling free() for every little struct we use (and keeping track of them).
//The memory is cleared to all zeros. Returns NULL when out of memory.
private void *
getroom(
    SpellInfo *spin,
    Unt   len,      // length needed
    int      align)      // align for pointer
{
   ...
   if (bl == NULL || bl->sb_used + len > SBLOCKSIZE) {
      if (len >= SBLOCKSIZE)
         ...
         bl = allocZeroed(offsetof(sblock_T, sb_data) + SBLOCKSIZE + 1);
      if (bl == NULL) {
         if (!spin->si_anyEmsgG) {
            emsg(_(e_insufficient_memory_word_list_will_be_incomplete));
            spin->si_anyEmsgG = TRUE;
         }
         return NULL;
      }
...
//Make a copy of a string into memory allocated with getroom(). Return NULL when out of memory.
private Byte *
getroom_save(SpellInfo *spin, Byte *s) {
   CS sc = (CS)getroom(spin, STRLEN(s) + 1, FALSE);
   if (sc != NULL)
      STRCPY(sc, s);
   return sc;
}
...
//Allocate the root of a word tree. Return NULL when out of memory.
private WordNode *
wordtree_alloc(SpellInfo *spin) {
   return (WordNode *)getroom(spin, sizeof(WordNode), TRUE);
}

//Return TRUE if "word" contains valid word characters.
//Control characters and trailing '/' are invalid.  Space is OK.
private int
valid_spell_word(Byte *word, Byte *end) {
   Byte *p;

   if (!utf_valid_string(word, end))
     return FALSE;
   for (p = word; *p != ZERO && p < end; p += utfCharLen(p)) {
      if (*p < ' ' || (p[0] == '/' && p[1] == ZERO))
        return FALSE;
   } 
   return TRUE;
...
//When "pfxlist" is not NULL store the word for each postponed prefix ID and compound flag.
private int
store_word(
    SpellInfo   *spin,
    Byte   *word,
    ...
    Byte   *pfxlist,   // list of prefix IDs or NULL
    int      need_affix)   // only store word with affix ID
{
    int      len = (int)STRLEN(word);
    int      ct = captype(word, word + len);
    Byte   foldword[MAXWLEN];
    int      res = OK;
    Byte   *p;
   ...
   return FAIL;
    ...
    (void)spell_casefold(curPor, word, len, foldword, MAXWLEN);
    for (p = pfxlist; res == OK; ++p) {
   if (!need_affix || (p != NULL && *p != ZERO))
...
//"rare" and "region" is the condition nr. Return FAIL when out of memory.
private int
tree_add_word(
   SpellInfo   *spin,
   Byte   *word,
   ...
   WordNode   *node = root;
   WordNode   *np;
   WordNode   *copyp, **copyprev;
   WordNode   **prev = NULL;
   int      i;

   // Add each byte of the word to the tree, including the ZERO at the end.
   for (i = 0; ; ++i) {
   ...
   if (spin->si_compress_cnt > 1 && --spin->si_compress_cnt == 1)
      // Did enough words to lower the block count limit.
      spin->si_blocks_cnt += compress_inc;

   //When we have allocated lots of memory, we need to compress the word tree to free up some room. 
   //But compression is slow, and we might actually need that room, thus only compress in the 
   //following situations:
   //1. When not compressed before (si_compress_cnt == 0): when using "compress_start" blocks.
   ...
   //   (si_compress_cnt == 1) and the number of free nodes drops below the maximum word length
...
#endif
    {
   // Decrement the block counter. The effect is that we compress again when the freed up room has
   // been used and another "compress_inc" blocks have been allocated.  Unless "compress_added" 
   // words have been added, then the limit is put back again.
   spin->si_blocks_cnt -= compress_inc;
...
//Get a WordNode, either from the list of previously freed nodes or
//allocate a new one. Return NULL when out of memory.
private WordNode *
get_wordnode(SpellInfo *spin) {
   WordNode *n;

   if (spin->si_first_free == NULL)
      n = (WordNode *)getroom(spin, sizeof(WordNode), TRUE);
   else {
...
private int
deref_wordnode(SpellInfo *spin, WordNode *node) {
    WordNode   *np;
    int      cnt = 0;
...
//Free a WordNode for re-use later. Only the "wn_child" field becomes invalid.
private void
free_wordnode(SpellInfo *spin, WordNode *n) {
...
private void
wordtree_compress(SpellInfo *spin, WordNode *root, char *name) {
    EeSet       ht;
    long       n;
    long       tot = 0;
    long       perc;

   // Skip the root itself, it's not actually used.  The first sibling is the start of the tree.
...
//Compress a node, its siblings and its children, depth first. Return the number of compressed nodes.
private long
node_compress(
    SpellInfo   *spin,
    WordNode   *node,
             ...
             // incremented while going through the tree
{
     ...
     */
   for (np = node; np != NULL && !gotInterruptG; np = np->wn_sibling) {
      ++len;
      if ((child = np->wn_child) != NULL) {
         ...
         Text t = mbText(child->wn_u1.hashkey);
         hash = calcHash(t);
         hi = hash_lookup(ht, t, hash);
         if (!HASHITEM_EMPTY(hi)) {
            ...
            for (tp = HI2WN(hi); tp; tp = tp->wn_u2.next)
         ...
         hash_add_item(ht, hi, mbText(child->wn_u1.hashkey), hash);
      }
    }
   ...
   if (np->wn_byte == ZERO)
       ...
       n = (unsigned)(np->wn_byte + ((Ulong)np->wn_child << 8));
   nr = nr * 101 + n;
    }

    // Avoid ZERO bytes, it terminates the hash key.
    n = nr & 0xff;
 ...
 */
private int
node_equal(WordNode *n1, WordNode *n2) {
    WordNode   *p1;
    WordNode   *p2;
...
private int rep_compare(const void *s1, const void *s2);
...
private int
put_bytes(FILE *fd, Ulong nr, int len) {
   int       i;
...
// Write the Eegl .spl file "fname". Return FAIL or OK;
private int
writeEeglSpell(SpellInfo *spin, CS fname) {
    int      regionmask;
    int      round;
    WordNode   *tree;
    int      nodecount;
    ...
    int      rr;
    int      retval = OK;
    Unt   fwv = 1;  // collect return value of fwrite() to avoid
   ...
   // <HEADER>: <fileID> <versionnr>
                         // <fileID>
   fwv &= fwrite(EEGLSPELLMAGIC, EEGLSPELLMAGICL, (Unt)1, fd);
   if (fwv != (Unt)1)
      // Catch first write error, don't try writing more.
      goto theend;

   putc(EEGLSPELLVERSION, fd);                // <versionnr>
   ...
   if (spin->si_info != NULL) {
      ...
      put_bytes(fd, (Ulong)i, 4);         // <sectionlen>
      fwv &= fwrite(spin->si_info, (Unt)i, (Unt)1, fd); // <infotext>
   }
   ...
   if (spin->si_region_count > 1) {
      ...
      put_bytes(fd, (Ulong)l, 4);         // <sectionlen>
      fwv &= fwrite(spin->si_region_name, (Unt)l, (Unt)1, fd);
                        // <regionname> ...
   ...
   // Skip this for ASCII, the table may conflict with the one used for 'encoding'.
   ...
   if (!spin->si_ascii && !spin->si_add) {
      ...
      put_bytes(fd, (Ulong)(1 + 128 + 2 + l), 4);   // <sectionlen>
      ...
      put_bytes(fd, (Ulong)l, 2);         // <folcharslen>
      fwv &= fwrite(folchars, (Unt)l, (Unt)1, fd); // <folchars>
   }
   ...
   put_bytes(fd, (Ulong)i, 4);         // <sectionlen>
   fwv &= fwrite(spin->si_midword, (Unt)i, (Unt)1, fd);
                     // <midword>
    }

    // SN_PREFCOND: <prefcondcnt> <prefcond> ...
   if (spin->si_prefcond.len > 0) {
   ...
   if (round != 2)
       qsort(gap->c, (Unt)gap->len, sizeof(FromTo), rep_compare);
   ...
   put_bytes(fd, (Ulong)gap->len, 2);   // <repcount> or <salcount>
   for (i = 0; i < gap->len; ++i) {
       // <rep> : <repfromlen> <repfrom> <reptolen> <repto>
       // <sal> : <salfromlen> <salfrom> <saltolen> <salto>
       ftp = &((FromTo *)gap->c)[i];
   ...
   put_bytes(fd, (Ulong)(l + STRLEN(spin->si_sofoto) + 4), 4);
                     // <sectionlen>

   put_bytes(fd, (Ulong)l, 2);         // <sofofromlen>
   fwv &= fwrite(spin->si_sofofr, l, (Unt)1, fd); // <sofofrom>

   l = (int)STRLEN(spin->si_sofoto);
   put_bytes(fd, (Ulong)l, 2);         // <sofotolen>
   fwv &= fwrite(spin->si_sofoto, l, (Unt)1, fd); // <sofoto>
    }

    // SN_WORDS: <word> ...
    // This is for making suggestions, section is not required.
   if (spin->si_commonwords.count > 0) {
   ...
   for (round = 1; round <= 2; ++round) {
       ...
       todo = (int)spin->si_commonwords.count;
       FOR_ALL_HASHTAB_ITEMS(&spin->si_commonwords, hi, todo)
      if (!HASHITEM_EMPTY(hi)) {
          l = (int)STRLEN(hi->hi_key) + 1;
          len += l;
          if (round == 2)         // <word>
         fwv &= fwrite(hi->hi_key, (Unt)l, (Unt)1, fd);
          --todo;
      }
       if (round == 1)
      put_bytes(fd, (Ulong)len, 4);      // <sectionlen>
   }
    }
   ...
   l = spin->si_map.len;
   put_bytes(fd, (Ulong)l, 4);         // <sectionlen>
   fwv &= fwrite(spin->si_map.c, (Unt)l, (Unt)1, fd);
                     // <mapstr>
    }
   ...
   putc(0, fd);               // <sectionflags>
   put_bytes(fd, (Ulong)8, 4);         // <sectionlen>
   ...
   putc(0, fd);               // <sectionflags>
   put_bytes(fd, (Ulong)0, 4);         // <sectionlen>
    }
   ...
   for (i = 0; i < spin->si_comppat.len; ++i)
       l += (int)STRLEN(((Byte **)(spin->si_comppat.c))[i]) + 1;
   put_bytes(fd, (Ulong)(l + 7), 4);      // <sectionlen>
   ...
   put_bytes(fd, (Ulong)spin->si_comppat.len, 2);
                     // <comppatcount>
   for (i = 0; i < spin->si_comppat.len; ++i) {
       p = ((Byte **)(spin->si_comppat.c))[i];
       putc((int)STRLEN(p), fd);         // <comppatlen>
       fwv &= fwrite(p, (Unt)STRLEN(p), (Unt)1, fd);
                     // <comppattext>
   }
                     // <compflags>
   fwv &= fwrite(spin->si_compflags, (Unt)STRLEN(spin->si_compflags), (Unt)1, fd);
    }
   ...
   put_bytes(fd, (Ulong)l, 4);         // <sectionlen>
   fwv &= fwrite(spin->si_syllable, (Unt)l, (Unt)1, fd);
                     // <syllable>
    }
   ...
   put_bytes(fd, (Ulong)nodecount, 4);   // <nodecount>
    ...
    int      prefixtree)   // TRUE for PREFIXTREE
{
   ...
   if (np->wn_byte == 0) {
       if (fd != NULL) {
      // For a ZERO byte (end of word) write the flags etc.
      if (prefixtree) {
          ...
          // combining" flags
          if (np->wn_flags == (Short)PFX_FLAGS)
         ...
         putc(np->wn_flags, fd);      // <pflags>
          ...
          put_bytes(fd, (Ulong)np->wn_region, 2); // <prefcondnr>
      } else {
      ...
      if (fd != NULL) {
          putc(BY_INDEX, fd);         // <byte>
                     // <nodeidx>
          put_bytes(fd, (Ulong)np->wn_child->wn_u1.index, 3);
      }
       }
       ei (np->wn_child->wn_u2.wnode == NULL)
...
c_mkspell(Invocation *invo) {
   ...
   // Expand all the remaining arguments (e.g., $EEGLRUNTIME).
   if (bookParseAndExpandFnames(arg, false, OUT &matches) != OK)
      return;

   mkspell(matches, ascii, invo->forceit, false);
   deleteArena(matches.a);
}

//Create the .sug file. Uses the soundfold info in "spin".
//Writes the file with the name "wfname", with ".spl" changed to ".sug".
private void
spell_make_sugfile(SpellInfo *spin, Byte *wfname) {
   int      len;
   SpellLang   *slang;
   ...
   if (slang == NULL) {
      spell_message(spin, (CS)_("Reading back spell file..."));
   ...
   //Go through the trie of good words, soundfold each word and add it to the soundfold trie.
   spell_message(spin, (CS)_("Performing soundfolding..."));
   ...
   //This also removes the wordnr from the ZERO byte entries to make compression possible.
   if (sug_maketable(spin) == FAIL)
      goto theend;

   smsg(_("Number of words after soundfolding: %ld"), (long)spin->si_spellbuf->mem.lineCount);

   //Compress the soundfold trie.
   spell_message(spin, (CS)_(msg_compressing));
   wordtree_compress(spin, spin->si_foldroot, "case-folded");

   //Write the .sug file. Make the file name by changing ".spl" to ".sug".
   Byte fname[MAXPATHL];
   copySubstrToAllocation(fname, (Text){wfname, MAXPATHL - 1});
...
private int
sug_filltree(SpellInfo *spin, SpellLang *slang) {
   Byte   *byts;
   idx_T   *idxs;
   int      depth;
   idx_T   arridx[MAXWLEN];
...
//Make the table that links each word in the soundfold trie to the words it can be produced from.
//This is not unlike lines in a file, thus use a memfile to be able to access the table efficiently.
//Return FAIL when out of memory.
private int
sug_maketable(SpellInfo *spin) {
   ArrayList   ga;
   int      res = OK;
   ...
   // Use a buffer to store the line info, avoids allocating many small pieces of memory.
   ga_init2(&ga, 1, 100);
...
//Return the wordnr at the start of the node. Return -1 when out of memory.
private int
sug_filltable(
   SpellInfo   *spin,
   WordNode   *node,
   int      startwordnr,
   ArrayList   *gap)       // place to store line of numbers
{
   WordNode   *p, *np;
   ...
   FOR_ALL_NODE_SIBLINGS(node, p) {
      if (p->wn_byte == ZERO) {
         gap->len = 0;
         prev_nr = 0;
         for (np = p; np != NULL && np->wn_byte == ZERO; np = np->wn_sibling) {
            ...
            // It's a bit like utf-8, but without the need to mark following bytes.
            nr -= prev_nr;
            prev_nr += nr;
            gap->len += offset2bytes(nr, (CS)gap->c + gap->len);
          }

          // add the ZERO byte
          ((CS)gap->c)[gap->len++] = ZERO;

          if (ml_append_buf(spin->si_spellbuf, (LineNr)wordnr,
                    gap->c, gap->len, TRUE) == FAIL)
        return -1;
          ++wordnr;

          // Remove extra ZERO entries, we no longer need them. We don't
          // bother freeing the nodes, they won't be reused anyway.
          while (p->wn_sibling != NULL && p->wn_sibling->wn_byte == ZERO)
         p->wn_sibling = p->wn_sibling->wn_sibling;

          // Clear the flags on the remaining ZERO node, so that compression
          // works a lot better.
          p->wn_flags = 0;
          p->wn_region = 0;
      } else {
 ...
 * Similar to utf_char2byters, but use 8 bits in followup bytes and avoid ZERO bytes.
 */
private int
offset2bytes(int nr, Byte *buf) {
    int       rem;
    int       b1, b2, b3, b4;

    // Split the number in parts of base 255.  We need to avoid ZERO bytes.
    b1 = nr % 255 + 1;
    rem = nr / 255;
    b2 = rem % 255 + 1;
    rem = rem / 255;
...
private void
sug_write(SpellInfo *spin, Byte *fname) {
   FILE   *fd;
   WordNode   *tree;
   int      nodecount;
   int      wcount;
   ...
   return;
    ...
    eeSnprintf(IObuff, IOSIZE, _("Writing suggestion file %s..."), fname);
    spell_message(spin, IObuff);

    /*
     * <SUGHEADER>: <fileID> <versionnr> <timestamp>
     */
   if (fwrite(EEGLSUGMAGIC, EEGLSUGMAGICL, (Unt)1, fd) != 1) {// <fileID>
   emsg(_(e_error_while_writing));
   goto theend;
    }
    putc(EEGLSUGVERSION, fd);            // <versionnr>
     ...
     * <SUGTABLE>: <sugwcount> <sugline> ...
     */
    wcount = spin->si_spellbuf->mem.lineCount;
    put_bytes(fd, (Ulong)wcount, 4);   // <sugwcount>

    for (lnum = 1; lnum <= (LineNr)wcount; ++lnum) {
   // <sugline>: <sugnr> ... ZERO
   line = memGetLine(spin->si_spellbuf, lnum, FALSE);
   len = memGetBookLen(spin->si_spellbuf, lnum) + 1;
   if (fwrite(line, (Unt)len, (Unt)1, fd) == 0) {
       emsg(_(e_error_while_writing));
       goto theend;
   }
   spin->si_memtot += len;
...
//Create an Eegl spell file from one or more word lists. "fnames[0]" is the output file name.
//"files.c[files.len - 1]" is the last input file name.
//Exception: when "files.c[0]" ends in ".add", it's used as the input file name
   ...
   int      added_word)       // invoked through "zg"
{
   ...
   ga_init2(&spin.si_rep, sizeof(FromTo), 20);
   ga_init2(&spin.si_repsal, sizeof(FromTo), 20);
   ga_init2(&spin.si_sal, sizeof(FromTo), 20);
   ga_init2(&spin.si_map, sizeof(Byte), 100);
   ga_init2(&spin.si_comppat, sizeof(CS), 20);
   ga_init2(&spin.si_prefcond, sizeof(CS), 50);
   hash_init(&spin.si_commonwords);
   spin.si_newcompID = 127;   // start compound ID at first maximum

   //default: fnames[0] is output file, following are input files
   //When "fcount" is 1 there is only one file.
   innames = files.c + (files.len == 1 ? 0 : 1);
   incount = files.len - 1;

   Byte wfname[MAXPATHL];

   if (files.len >= 1) {
      len = (int)STRLEN(files.c[0]);
      if (files.len == 1 && len > 4 && STRCMP(files.c[0] + len - 4, ".add") == 0) {
          //For ":mkspell path/en.latin1.add" output file is "path/en.latin1.add.spl".
          incount = 1;
          eeSnprintf(wfname, MAXPATHL, "%s.spl", files.c[0]);
      } ei (files.len == 1) {
         //For ":mkspell path/eegl" output file is "path/eegl.spl".
         incount = 1;
         eeSnprintf(wfname, MAXPATHL, SPL_FNAME_TMPL, files.c[0]);
      } ei (len > 4 && STRCMP(files.c[0] + len - 4, ".spl") == 0) {
          // Name ends in ".spl", use as the file name.
          copySubstrToAllocation(wfname, (Text){files.c[0], MAXPATHL - 1});
      } else
   ...
   ei (firstOccurrence(gettail(wfname), '_') != NULL)
      emsg(_(e_output_file_name_must_not_have_region_name));
   ei (incount > MAXREGIONS)
      showErrFmtMsg(_(e_only_up_to_nr_regions_supported), MAXREGIONS);
   else {
      // Check for overwriting before doing things that may take a lot of time.
      if (!over_write && stat((char *)wfname, &st) >= 0) {
         emsg(_(e_file_exists));
         return;
      }
      if (mch_isdir(wfname)) {
         showErrFmtMsg(_(e_str_is_directory), wfname);
         return;
      }

      Byte fname[MAXPATHL];

      //Init the aff and dic pointers. Get the region names if there are more than 2 arguments.
      for (i = 0; i < incount; ++i) {
         afile[i] = NULL;
         ...
         if (incount > 1) {
            ...
            if (STRLEN(gettail(innames[i])) < 5 || innames[i][len - 3] != '_') {
                showErrFmtMsg(_(e_invalid_region_in_str), innames[i]);
                return;
            }
      ...
      for (i = 0; i < incount && !error; ++i) {
         ...
         eeSnprintf(fname, MAXPATHL, "%s.aff", innames[i]);
         if (stat((char *)fname, &st) >= 0) {
            // Read the .aff file.  Will init "spin->si_conv" based on the "SET" line.
            afile[i] = spell_read_aff(&spin, fname);
            if (afile[i] == NULL)
                error = TRUE;
            else {
               // Read the .dic file and store the words in the trees.
               eeSnprintf(fname, MAXPATHL, "%s.dic", innames[i]);
      ...
      if (!error && !gotInterruptG) {
          ...
          eeSnprintf(IObuff, IOSIZE, _("Writing spell file %s..."), wfname);
          spell_message(&spin, IObuff);

          error = writeEeglSpell(&spin, wfname) == FAIL;

          spell_message(&spin, (CS)_("Done!"));
          eeSnprintf(IObuff, IOSIZE,
          _("Estimated runtime memory use: %d bytes"), spin.si_memtot);
          spell_message(&spin, IObuff);
      ...
      if (spin.si_sugtime != 0 && !error && !gotInterruptG)
...
private void
spell_message(SpellInfo *spin, Byte *str) {
   if (spin->si_verbose || p_verbose > 2) {
      if (!spin->si_verbose)
         verbose_enter();
      msg(str);
...
void
c_spell(Invocation *invo) {
   spell_add_word(
      invo->arg, (int)STRLEN(invo->arg),
      invo->id == C_spellwrong ? SPELL_ADD_BAD :
      invo->id == C_spellrare ? SPELL_ADD_RARE : SPELL_ADD_GOOD,
      invo->forceit ? 0 : (int)invo->line2,
      invo->id == C_spellundo
   );
}
...
spell_add_word(
   ...
   int      idx,       // "zG" and "zW": zero, otherwise index in 'spellfile'
   ...
   if (idx == 0) {       // use internal wordlist
      if (int_wordlist == NULL) {
         int_wordlist = eeTempName('s', FALSE);
         if (int_wordlist == NULL)
           return;
      }
      fname = int_wordlist;
      ...
      if (*curPor->ownSyntax->spellFile == ZERO) {
         init_spellfile();
         new_spf = TRUE;
      }

      if (*curPor->ownSyntax->spellFile == ZERO) {
          showErrFmtMsg(_(e_option_str_is_not_set), "spellfile");
         return;
      }
      Byte fnamebuf[MAXPATHL];

      for (spf = curPor->ownSyntax->spellFile, i = 1; *spf != ZERO; ++i) {
         copy_option_part(&spf, fnamebuf, MAXPATHL, ",");
         if (i == idx)
            break;
         if (*spf == ZERO) {
            showErrFmtMsg(_(e_spellfile_does_not_have_nr_entries), idx);
            return;
         }
      }

      // Check that the user isn't editing the .add file somewhere.
      buf = buflistFindByNameExpandingLinks(fnamebuf);
      if (buf != NULL && buf->mem.mfile == NULL)
         buf = NULL;
      if (buf != NULL && doWasBookChanged(buf)) {
   ...
   fd = fopen((char *)fname, "r");
   if (fd) {
       while (!eeFgets(line, MAXWLEN * 2, fd)) {
      fpos = fpos_next;
      fpos_next = ftell(fd);
      if (fpos_next < 0)
   ...
   if (!undo) {
      ...
      if (fd == NULL && new_spf) {
         ...
         // already checked the runtime directory is writable in init_spellfile().
         if (!dir_of_file_exists(fname) && (p = gettail_sep(fname)) != fname) {
            Unt c = *p;

            // The directory doesn't exist.  Try creating it and opening the file again.
            *p = ZERO;
            eeMkdir(fname, 0755);
            *p = c;
            fd = fopen((char *)fname, "a");
   ...
   if (fd) {
      ...
      mkspell((ExpandMatch){.len = 1, .c = &fname}, FALSE, TRUE, TRUE);

      // If the .add file is edited somewhere, reload it.
      if (buf)
         buf_reload(buf, buf->origMode, FALSE);
...
init_spellfile(void) {
   ...
   Byte   *lend;
   int      aspath = FALSE;
   Byte   *lstart = curBook->syntax.spellLang;

   if (*curPor->ownSyntax->spellLang == ZERO || curPor->ownSyntax->b_langp.len <= 0)
      return;
   ...
   for (lend = curPor->ownSyntax->spellLang; *lend != ZERO
          && firstOccurrence((CS)",._", *lend) == NULL; ++lend) {
      if (*lend == '/') {
         aspath = TRUE;
         lstart = lend + 1;
      }
   ...
   while (*rtp != ZERO) {
      if (aspath)
         ...
         copySubstrToAllocation(
               builder, (Text){curBook->syntax.spellLang, lstart - curBook->syntax.spellLang - 1}
         );
      else
         // Copy the path from 'runtimepath' to builder[].
         copy_option_part(&rtp, builder, MAXPATHL, ",");
      if (filewritable(builder) == 2) {
         // Use the first language name from 'spelllang' and the
         // encoding used in the first loaded .spl file.
         if (aspath)
            copySubstrToAllocation(
                  builder, (Text){curBook->syntax.spellLang, lend - curBook->syntax.spellLang}
            );
         else {
            // Create the "spell" directory if it doesn't exist yet.
            l = (int)STRLEN(builder);
            eeSnprintf(builder + l, MAXPATHL - l, "/spell");
            if (filewritable(builder) != 2) {
               if (eeMkdir(builder, 0755) != 0) {
                  return;
               }
            }

            l = (int)STRLEN(builder);
            eeSnprintf(builder + l, MAXPATHL - l,
               "/%.*s", (int)(lend - lstart), lstart);
         }
         l = (int)STRLEN(builder);
         eeSnprintf(builder + l, MAXPATHL - l, ".%s.add", E);
         optChangeAndReportError(S"spellfile", optStr(builder), OPT_LOCAL);
         break;
      }
      aspath = FALSE;
   }
...
private int
set_spell_finish(SpellTab *new_st) {
   int      i;
...
private int
write_spell_prefcond(FILE *fd, ArrayList *gap, Unt *fwv) {
   Byte   *p;
   int      len;
   int      totlen;

   if (fd)
      put_bytes(fd, (Ulong)gap->len, 2);       // <prefcondcnt>

   totlen = 2 + gap->len; // length of <prefcondcnt> and <condlen> bytes
...
private void
set_map_str(SpellLang *lp, Byte *map) {
   Byte   *p;
   int headc = 0;
   int c;
   ...
   for (p = map; *p != ZERO; ) {
      ...
      if (c == '/')
         ...
         // the hash table.  Each entry is the char, a ZERO the headchar and a ZERO.
         if (c >= 256) {
            ...
            Text t = text(b);
            hash = calcHash(t);
            hi = hash_lookup(&lp->sl_map_hash, t, hash);
            if (HASHITEM_EMPTY(hi))
               hash_add_item(&lp->sl_map_hash, hi, t, hash);
            else {
...
typedef struct suginfo_S {
   ArrayList   su_ga;          // suggestions, contains "suggest_T"
   int      su_maxcount;       // max. number of suggestions displayed
   int      su_maxscore;       // maximum score for adding to su_ga
   int      su_sfmaxscore;       // idem, for when doing soundfold words
   ArrayList   su_sga;          // like su_ga, sound-folded scoring
   Byte   *su_badptr;       // start of bad word in line
   int      su_badlen;       // length of detected bad word in line
   int      su_badflags;       // caps flags for bad word
   Byte   su_badword[MAXWLEN]; // bad word truncated at su_badlen
   Byte   su_fbadword[MAXWLEN]; // su_badword case-folded
   Byte   su_sal_badword[MAXWLEN]; // su_badword soundfolded
   EeSet   su_banned;       // table with banned words
   SpellLang   *su_sallang;       // default language for sound folding
} Suggestion;

// One word suggestion.  Used in "si_ga".
typedef struct suggest_S {
   Byte   *st_word;   // suggested word, allocated string
   ...
   SpellLang   *st_slang;   // language used for sound folding
} suggest_T;

#define SUG(ga, i) (((suggest_T *)(ga).c)[i])
...
typedef enum {
   STATE_START = 0,   // At start of node check for ZERO bytes (goodword
        // ends); if badword ends there is a match, otherwise try splitting word.
   STATE_NOPREFIX,   // try without prefix
   STATE_SPLITUNDO,   // Undo splitting.
   STATE_ENDZERO,   // Past ZERO bytes at start of the node.
...
typedef struct trystate_S {
   ...
   Byte   ts_fidx;   // index in fword[], case-folded bad word
   Byte   ts_fidxtry;   // ts_fidx at which bytes may be changed
   Byte   ts_twordlen;   // valid length of tword[]
   Byte   ts_prefixdepth;   // stack depth for end of prefix or PFD_PREFIXTREE or PFD_NOPREFIX
   Byte   ts_flags;   // TSF_ flags
   Byte   ts_tcharlen;   // number of bytes in tword character
   Byte   ts_tcharidx;   // current byte index in tword character
   Byte   ts_isdiff;   // DIFF_ values
   Byte   ts_fcharstart;   // index in fword where badword char started
   Byte   ts_prewordlen;   // length of word in "preword[]"
   Byte   ts_splitoff;   // index in "tword" after last split
   Byte   ts_splitfidx;   // "ts_fidx" at word split
   Byte   ts_complen;   // nr of compound words used
   Byte   ts_compsplit;   // index for "compflags" where word was spit
   Byte   ts_save_badflags;   // su_badflags saved here
   Byte   ts_delidx;   // index in fword for char that was deleted,
...
private long spell_suggest_timeout = 5000;

private void spell_find_suggest(
      Byte *badptr, int badlen, Suggestion *su, int maxcount, int banbadword, 
      int interactive
);
private void spell_suggest_expr(Suggestion *su, Byte *expr);
private void spell_suggest_file(Suggestion *su, Byte *fname);
private void spell_suggest_intern(Suggestion *su, int interactive);
private void spell_find_cleanup(Suggestion *su);
private void suggest_try_special(Suggestion *su);
private void suggest_try_change(Suggestion *su);
private void suggest_trie_walk(Suggestion *su, Langp *lp, Byte *fword, int soundfold);
private void go_deeper(trystate_T *stack, int depth, int score_add);
private void find_keepcap_word(SpellLang *slang, Byte *fword, Byte *kword);
private void score_comp_sal(Suggestion *su);
private void score_combine(Suggestion *su);
private int stp_sal_score(suggest_T *stp, Suggestion *su, SpellLang *slang, Byte *badsound);
private void suggest_try_soundalike_prep(void);
private void suggest_try_soundalike(Suggestion *su);
private void suggest_try_soundalike_finish(void);
private void add_sound_suggest(Suggestion *su, Byte *goodword, int score, Langp *lp);
private int soundfold_find(SpellLang *slang, Byte *word);
private int similar_chars(SpellLang *slang, int c1, int c2);
private void add_suggestion(
      Suggestion *su, ArrayList *gap, Byte *goodword, int badlen, int score, int altscore, 
      int had_bonus, SpellLang *slang, int maxsf
);
private void check_suggestions(Suggestion *su, ArrayList *gap);
private void add_banned(Suggestion *su, Byte *word);
private void rescore_suggestions(Suggestion *su);
private void rescore_one(Suggestion *su, suggest_T *stp);
private int cleanup_suggestions(ArrayList *gap, int maxscore, int keep);
private int soundalike_score(Byte *goodsound, Byte *badsound);
private int spell_edit_score(SpellLang *slang, Byte *badword, Byte *goodword);
private int spell_edit_score_limit(SpellLang *slang, Byte *badword, Byte *goodword, int limit);
    ...
    int      flag)
{
   ...
   if (slang->sl_comprules != NULL && sp->ts_complen > sp->ts_compsplit) {
      ...
      compflags[sp->ts_complen + 1] = ZERO;
      v = match_compoundrule(slang, compflags + sp->ts_compsplit);
      compflags[sp->ts_complen] = ZERO;
      return v;
   }
   ...
   int      split)       // word was split, less bonus
{
   ...
   EeSetItem* hi = hash_find(&slang->sl_wordcount, mbText(word));
   if (HASHITEM_EMPTY(hi))
      return score;
...
// capital.  So that make_case_word() can turn WOrd into Word. Add ALLCAP for "WOrD".
private int
badword_captype(Byte *word, Byte *end) {
...
// "pp" points to the bytes and is advanced over it. Returns the offset.
private int
bytes2offset(Byte **pp) {
   Byte   *p = *pp;
   int      nr;
   int      c;
   ...
   if ((c & 0x80) == 0x00) {     // 1 byte
      nr = c - 1;
   } ei ((c & 0xc0) == 0x80) {  // 2 bytes
      nr = (c & 0x3f) - 1;
      nr = nr * 255 + (*p++ - 1);
   } ei ((c & 0xe0) == 0xc0) {  // 3 bytes
      nr = (c & 0x1f) - 1;
      nr = nr * 255 + (*p++ - 1);
      nr = nr * 255 + (*p++ - 1);
   } else {           // 4 bytes
...
private int sps_flags = SPS_BEST;   // flags from 'spellsuggest'
private int sps_limit = 9999;      // max nr of suggestions given

// Check the @spellsuggest option.  Return FAIL if it's wrong. Sets "sps_flags" and "sps_limit".
int
spell_check_sps(void) {
   ...
   for (p = p_sps; *p != ZERO; ) {
      ...
      if (EE_ISDIGIT(*buf)) {
         ...
         if (*s != ZERO && !EE_ISDIGIT(*s))
            f = -1;
      }
      // Note: Keep this in sync with p_sps_values.
      ei (STRCMP(buf, "best") == 0)
          f = SPS_BEST;
      ei (STRCMP(buf, "fast") == 0)
          f = SPS_FAST;
      ei (STRCMP(buf, "double") == 0)
          f = SPS_DOUBLE;
      ei (STRNCMP(buf, "expr:", 5) != 0
         && STRNCMP(buf, "file:", 5) != 0
         && (STRNCMP(buf, "timeout:", 8) != 0
             || (!EE_ISDIGIT(buf[8]) && !(buf[8] == '-' && EE_ISDIGIT(buf[9]))))
      )
          f = -1;
...
// "z=": Find badly spelled word under or after the cursor. Give suggestions for the properly 
// spelled word. In Visual mode use the highlighted word as the bad word.
// When "count" is non-zero use that suggestion.
void
spell_suggest(int count) {
   Byte   *line;
   Pos   prev_cursor = curPor->cursor;
   Byte   wcopy[MAXWLEN + 2];
   Byte   *p;
   int      i;
   int      c;
   Suggestion   sug;
   ...
   if (VIsual_active) {
      //Use the Visually selected text as the bad word. But reject a multi-line selection.
      if (curPor->cursor.lnum != VIsual.lnum) {
         return;
      }
      badlen = (int)curPor->cursor.col - (int)VIsual.col;
      if (badlen < 0)
          badlen = -badlen;
      else
          curPor->cursor.col = VIsual.col;
      ++badlen;
      end_visual_mode();
      // make sure we don't include the ZERO at the end of the line
      if (badlen > ml_get_curline_len() - (int)curPor->cursor.col)
          badlen = ml_get_curline_len() - (int)curPor->cursor.col;
   }
   // Find the start of the badly spelled word.
   ei (spell_move_to(curPor, FORWARD, SMT_ALL, TRUE, NULL) == 0
       || curPor->cursor.col > prev_cursor.col
   ){
      // No bad word or it starts after the cursor: use the word under the
      // cursor.
      curPor->cursor = prev_cursor;
      line = ml_get_curline();
      p = line + curPor->cursor.col;
      // Backup to before start of word.
      while (p > line && spell_iswordp_nmw(p))
         MB_PTR_BACK(line, p);
      // Forward to start of word.
      while (*p != ZERO && !spell_iswordp_nmw(p))
          MB_PTR_ADV(p);
      ...
      curPor->cursor.col = (ColNr)(p - line);
   }
   ...
   line = copySubstr(ml_get_curline(), ml_get_curline_len());

   // Get the list of suggestions.  Limit to 'lines' - 2 or the number in
   // 'spellsuggest', whatever is smaller.
   if (sps_limit > (int)visibleRowsG - 2)
      limit = (int)visibleRowsG - 2;
   else
      limit = sps_limit;
   spell_find_suggest(line + curPor->cursor.col, badlen, &sug, limit, TRUE, TRUE);

   if (sug.su_ga.len == 0)
      msg(_("Sorry, no suggestions"));
   ei (count > 0) {
   if (count > sug.su_ga.len)
       smsg(_("Sorry, only %ld suggestions"), (long)sug.su_ga.len);
   } else {
      ...
      eeSnprintf(IObuff, IOSIZE, _("Change \"%.*s\" to:"), sug.su_badlen, sug.su_badptr);
      msg_puts(IObuff);
      msg_clr_eos();
      msg_putchar('\n');

      msg_scroll = TRUE;
      for (i = 0; i < sug.su_ga.len; ++i) {
         int el;
         ...
         copySubstrToAllocation(wcopy, (Text){stp->st_word, MAXWLEN});
         el = sug.su_badlen - stp->st_orglen;
         if (el > 0 && stp->st_wordlen + el <= MAXWLEN) {
            copySubstrToAllocation(
                  wcopy + stp->st_wordlen, (Text){sug.su_badptr + stp->st_orglen, el}
            );
         } 
         eeSnprintf(IObuff, IOSIZE, "%2d", i + 1);
         msg_puts(IObuff);

         eeSnprintf(IObuff, IOSIZE, " \"%s\"", wcopy);
         msg_puts(IObuff);

         // The word may replace more than "su_badlen".
         if (sug.su_badlen < stp->st_orglen) {
            eeSnprintf(IObuff, IOSIZE, _(" < \"%.*s\""), stp->st_orglen, sug.su_badptr);
            msg_puts(IObuff);
         }
         ...
         if (p_verbose > 0) {
            ...
            if (sps_flags & (SPS_DOUBLE | SPS_BEST))
               eeSnprintf(
                  IObuff, IOSIZE, " (%s%d - %d)", stp->st_salscore ? "s " : "",
                  stp->st_score, stp->st_altscore
               );
            else
                eeSnprintf(IObuff, IOSIZE, " (%d)", stp->st_score);
            msg_advance(30);
            msg_puts(IObuff);
         }
         msg_putchar('\n');
   ...
   if (selected > 0 && selected <= sug.su_ga.len && u_save_cursor() == OK) {
      ...
      if (sug.su_badlen > stp->st_orglen) {
         // Replacing less than "su_badlen", append the remainder to repl_to.
         repl_from = copySubstr(sug.su_badptr, sug.su_badlen);
         eeSnprintf(IObuff, IOSIZE, "%s%.*s", stp->st_word,
             sug.su_badlen - stp->st_orglen, sug.su_badptr + stp->st_orglen);
         repl_to = copyStr(IObuff);
      } else {
         // Replacing su_badlen or more, use the whole word.
         repl_from = copySubstr(sug.su_badptr, stp->st_orglen);
         repl_to = copyStr(stp->st_word);
      }
      ...
      if (p) {
         ...
         changed_bytes(curPor->cursor.lnum, c);
         if (curBook->hasTextprop && len_diff != 0)
            adjustPropColumns(curPor->cursor.lnum, c, len_diff, APC_SUBSTITUTE);
      }
   } else
      curPor->cursor = prev_cursor;
...
// Find spell suggestions for "word". Return them in the growarray "*gap" as a list of allocated 
// strings.
void
spell_suggest_list(
   ArrayList   *gap,
   ...
   int      interactive)
{
   ...
   spell_find_suggest(word, 0, &sug, maxcount, FALSE, interactive);

   // Make room in "gap".
   ga_init2(gap, sizeof(CS), sug.su_ga.len + 1);
   if (ga_grow(gap, sug.su_ga.len) == OK) {
      for (i = 0; i < sug.su_ga.len; ++i) {
         stp = &SUG(sug.su_ga, i);

         // The suggested word may replace only part of "word", add the not replaced part.
         wcopy = alloc(stp->st_wordlen + (unsigned)STRLEN(sug.su_badptr + stp->st_orglen) + 1);
         if (wcopy == NULL)
            break;
         STRCPY(wcopy, stp->st_word);
         STRCPY(wcopy + stp->st_wordlen, sug.su_badptr + stp->st_orglen);
         ((Byte **)gap->c)[gap->len++] = wcopy;
      }
   }
...
//Return the suggestions in "su->su_ga". The maximum number of suggestions is "maxcount".
//Note: does use info for the current portal.
...
spell_find_suggest(
   ...
   if (badlen != 0)
      ...
      su->su_badlen = spell_check(curPor, su->su_badptr, &deco, NULL, FALSE);
   su->su_maxcount = maxcount;
   su->su_maxscore = SCORE_MAXINIT;

   if (su->su_badlen >= MAXWLEN)
      su->su_badlen = MAXWLEN - 1;   // just in case
   copySubstrToAllocation(su->su_badword, (Text){su->su_badptr, su->su_badlen});
   (void)spell_casefold(curPor, su->su_badptr, su->su_badlen, su->su_fbadword, MAXWLEN);
   ...
   su->su_fbadword[su->su_badlen] = ZERO;
   ...
   // languages (e.g., "pl,en").
   for (i = 0; i < curBook->syntax.b_langp.len; ++i) {
      lp = LANGP_ENTRY(curBook->syntax.b_langp, i);
      if (lp->lp_sallang) {
   ...
   sps_copy = copyStr(p_sps);
   spell_suggest_timeout = 5000;

   // Loop over the items in 'spellsuggest'.
   for (p = sps_copy; *p != ZERO; ) {
      copy_option_part(&p, buf, MAXPATHL, ",");
      ...
      } ei (STRNCMP(buf, "file:", 5) == 0)
          // Use list of suggestions in a file.
          spell_suggest_file(su, buf + 5);
      ei (STRNCMP(buf, "timeout:", 8) == 0)
...
private void
spell_suggest_expr(Suggestion *su, Byte *expr) {
   List   *list;
   ListItem   *li;
   int      score;
   Byte   *p;
   ...
   if (list != NULL) {
      ...
      FOR_ALL_LIST_ITEMS(list, li) {
         if (li->c.tag == VAR_LIST) {
            ...
            score = get_spellword(li->c.list, &p);
...
private void
spell_suggest_file(Suggestion *su, Byte *fname) {
   FILE   *fd;
   Byte   line[MAXWLEN * 2];
   Byte   *p;
   int      len;
   ...
   while (!eeFgets(line, MAXWLEN * 2, fd) && !gotInterruptG) {
      line_breakcheck();
      ...
      *p++ = ZERO;
      if (caseInsensitiveCompare(su->su_badword, line) == 0) {
...
private void
spell_suggest_intern(Suggestion *su, int interactive) {
   ...
   if ((sps_flags & SPS_FAST) == 0) {
      ...
      if (su->su_ga.len < SUG_CLEAN_COUNT(su)) {
   ...
   // gotInterruptG when using a command, not for spellsuggest().
   ui_breakcheck();
   if (interactive && gotInterruptG) {
      (void)vgetc();
      gotInterruptG = FALSE;
   }

   if ((sps_flags & SPS_DOUBLE) == 0 && su->su_ga.len != 0) {
      if (sps_flags & SPS_BEST)
...
private void
spell_find_cleanup(Suggestion *su) {
   int      i;

   // Free the suggestions.
   for (i = 0; i < su->su_ga.len; ++i)
      eeglFree(SUG(su->su_ga, i).st_word);
   ga_clear(&su->su_ga);
   for (i = 0; i < su->su_sga.len; ++i)
      eeglFree(SUG(su->su_sga, i).st_word);
   ga_clear(&su->su_sga);
...
private void
suggest_try_special(Suggestion *su) {
   Byte   *p;
   Unt   len;
   int      c;
   Byte   word[MAXWLEN];
...
private void
suggest_try_change(Suggestion *su) {
   Byte   fword[MAXWLEN];       // copy of the bad word, case-folded
   int      n;
   Byte   *p;
   int      lpi;
   Langp   *lp;
   ...
   (void)spell_casefold(curPor, p, (int)STRLEN(p), fword + n, MAXWLEN - n);
   ...
   for (lpi = 0; lpi < curPor->ownSyntax->b_langp.len; ++lpi) {
      ...
      // If reloading a spell file fails it's still in the list but everything has been cleared
...
suggest_trie_walk(
    ...
    int      soundfold)
{
    Byte   tword[MAXWLEN];       // good word collected so far
    trystate_T   stack[MAXWLEN];
    Byte   preword[MAXWLEN * 3]; // word found with proper case;
                  // concatenation of prefix compound
                  // words and split word.  ZERO terminated
                  // when going deeper but not when coming
                  // back.
    Byte   compflags[MAXWLEN];   // compound flags, one for each word
    trystate_T   *sp;
    int      newscore;
    int      score;
    Byte   *byts, *fbyts, *pbyts;
    ...
    SpellLang   *slang = lp->lp_slang;
    int      fword_ends;
    int      goodword_ends;
#ifdef DEBUG_TRIEWALK
    // Stores the name of the change made at each level.
    Byte   changename[MAXWLEN][80];
#endif
   ...
   // The loop may take an indefinite amount of time. Break out after some time.
   ...
   while (depth >= 0 && !gotInterruptG) {
      ...
      case STATE_START:
      case STATE_NOPREFIX:
          // Start of node: Deal with ZERO bytes, which means tword[] may end here.
          arridx = sp->ts_arridx;       // current node in the tree
          len = byts[arridx];          // bytes in this node
          arridx += sp->ts_curi;       // index of current byte

         if (sp->ts_prefixdepth == PFD_PREFIXTREE) {
            // Skip over the ZERO bytes, we use them later.
            for (n = 0; n < len && byts[arridx + n] == 0; ++n)
                ;
            sp->ts_curi += n;

            // Always past ZERO bytes now.
            n = (int)sp->ts_state;
            PROF_STORE(sp->ts_state)
            sp->ts_state = STATE_ENDZERO;
            sp->ts_save_badflags = su->su_badflags;

            // At end of a prefix or at start of prefixtree: check for
            // following word.
            if (depth < MAXWLEN - 1 && (byts[arridx] == 0 || n == (int)STATE_NOPREFIX)) {
               ...
               n = nofold_len(fword, sp->ts_fidx, su->su_badptr);
                ...
                tword[sp->ts_twordlen] = ZERO;
                make_case_word(tword + sp->ts_splitoff,
                       preword + sp->ts_prewordlen, flags);
                sp->ts_prewordlen = (Byte)STRLEN(preword);
         ...
         // Past bytes in node and/or past ZERO bytes.
         PROF_STORE(sp->ts_state)
         sp->ts_state = STATE_ENDZERO;
         sp->ts_save_badflags = su->su_badflags;
         break;
         ...
         break;
          ...
          fword_ends = (fword[sp->ts_fidx] == ZERO
               || (soundfold
                   ? SPACE_OR_TAB(fword[sp->ts_fidx])
                   : !spell_iswordp(fword + sp->ts_fidx, curPor)));
          tword[sp->ts_twordlen] = ZERO;
         ...
         len = pbyts[n++];
         for (c = 0; c < (Unt)len && pbyts[n + c] == 0; ++c)
             ;
         if (c > 0) {
         ...
         if (slang->nobreak) {
            ...
            // (splitting is tried for the word without compound flag).
            if (((unsigned)flags >> 24) == 0
                || sp->ts_twordlen - sp->ts_splitoff < slang->sl_compminlen)
            break;
            // For multi-byte chars check character length against
            // COMPOUNDMIN.
            if (slang->sl_compminlen > 0 && mb_charlen(tword + sp->ts_splitoff) < slang->sl_compminlen)
               break;

             compflags[sp->ts_complen] = ((unsigned)flags >> 24);
             compflags[sp->ts_complen + 1] = ZERO;
             copySubstrToAllocation(preword + sp->ts_prewordlen,
                (Text){tword + sp->ts_splitoff, sp->ts_twordlen - sp->ts_splitoff}
            );
         ...
         c = su->su_badflags;
         if ((c & WF_ALLCAP) && (Unt)su->su_badlen == utfCharLen(su->su_badptr))
             c = WF_ONECAP;
         c |= flags;
         ...
         if (!soundfold) {
            // Don't use a banned word.  It may appear again as a good word, thus remember it.
            if (flags & WF_BANNED) {
                add_banned(su, preword + sp->ts_prewordlen);
                break;
            }
            if (((sp->ts_complen == sp->ts_compsplit
                      && (WAS_BANNED(su, mbText(preword + sp->ts_prewordlen)))
                  )
                  || WAS_BANNED(su, mbText(preword)))
            ) {
               if (slang->sl_compprog == NULL)
                  break;
         ...
         if (soundfold) {
             ...
             p = fword + sp->ts_fidx;
             MB_PTR_BACK(fword, p);
             if (!spell_iswordp(p, curPor) && *preword != ZERO) {
         ...
         if ((sp->ts_fidx >= sp->ts_fidxtry || fword_ends)
            ...
            // For NOBREAK we never try splitting, it won't make any word valid.
            if (slang->nobreak && !slang->sl_nocompoundsugs)
                try_compound = TRUE;
               ...
               && (sp->ts_flags & TSF_DIDSPLIT) == 0)
            {
                ...
                --sp->ts_curi;       // do the same ZERO again
                compflags[sp->ts_complen] = ZERO;
            }
            else
                sp->ts_flags &= ~TSF_DIDSPLIT;
            ...
            if (try_split || try_compound) {
               ...
               // is only one word it must not have the NEEDCOMPOUND flag.
               ...
               if (TRY_DEEPER(su, stack, depth, newscore)) {
                  ...
                  // character when the word ends. But only when the good word can end.
                  if (((!try_compound && !spell_iswordp_nmw(fword + sp->ts_fidx)) || fword_ends)
                     && fword[sp->ts_fidx] != ZERO
                     && goodword_ends
                  ) {
                      int       l;

                      l = utfCharLen(fword + sp->ts_fidx);
                      if (fword_ends) {
      ...
      case STATE_SPLITUNDO:
          ...
          // Continue looking for ZERO bytes.
          PROF_STORE(sp->ts_state)
          sp->ts_state = STATE_START;
      ...
      case STATE_ENDZERO:
         // Past the ZERO bytes in the node.
         su->su_badflags = sp->ts_save_badflags;
         if (fword[sp->ts_fidx] == ZERO && sp->ts_tcharlen == 0) {
         ...
         if (sp->ts_fidx >= sp->ts_fidxtry)
               ...
               && TRY_DEEPER(su, stack, depth, newscore))
            {
               ...
               if (sp->ts_tcharlen == 0) {
                   // First byte.
                   sp->ts_tcharidx = 0;
                   sp->ts_tcharlen = utf8CharLens[c];
                   sp->ts_fcharstart = sp->ts_fidx - 1;
                   sp->ts_isdiff = (newscore != 0) ? DIFF_YES : DIFF_NONE;
               } ei (sp->ts_isdiff == DIFF_INSERT && sp->ts_fidx > 0)
                   // When inserting trail bytes don't advance in the bad word.
                   --sp->ts_fidx;
               if (++sp->ts_tcharidx == sp->ts_tcharlen) {
                  ...
                  if (sp->ts_isdiff == DIFF_YES) {
                     ...
                     sp->ts_fidx = sp->ts_fcharstart + utfCharLen(fword + sp->ts_fcharstart);
                     // For changing a composing character adjust
                     // the score from SCORE_SUBST to SCORE_SUBCOMP.
                     if (utf_iscomposing(mb_ptr2char(tword + sp->ts_twordlen - sp->ts_tcharlen))
                           && utf_iscomposing(mb_ptr2char(fword + sp->ts_fcharstart))
                     ) {
                  ...
                  ei (sp->ts_isdiff == DIFF_INSERT && sp->ts_twordlen > sp->ts_tcharlen) {
                     p = tword + sp->ts_twordlen - sp->ts_tcharlen;
                     c = mb_ptr2char(p);
                     if (utf_iscomposing(c)) {
                        // Inserting a composing char doesn't count that much.
      ...
      case STATE_DEL:
         ...
         if (soundfold && sp->ts_fidx == 0 && fword[sp->ts_fidx] == '*')
            ...
            newscore = SCORE_DEL;
         if (fword[sp->ts_fidx] != ZERO && TRY_DEEPER(su, stack, depth, newscore)) {
            go_deeper(stack, depth, newscore);
#ifdef DEBUG_TRIEWALK
            ...
            //a bit illogical for soundfold tree but it does give better results.
            c = mb_ptr2char(fword + sp->ts_fidx);
            stack[depth].ts_fidx += utfCharLen(fword + sp->ts_fidx);
            if (utf_iscomposing(c))
               stack[depth].ts_score -= SCORE_DEL - SCORE_DELCOMP;
            ei (c == mb_ptr2char(fword + stack[depth].ts_fidx))
      ...
      case STATE_INS_PREP:
         ...
         if (sp->ts_curi > byts[n]) {
             // Only ZERO bytes at this node, go to next state.
             PROF_STORE(sp->ts_state)
             sp->ts_state = STATE_SWAP;
             break;
         }
         if (byts[n + sp->ts_curi] != ZERO) {
      ...
      case STATE_INS:
          // Insert one byte.  Repeat this for each possible byte at this node.
          n = sp->ts_arridx;
          if (sp->ts_curi > byts[n]) {
      ...
      case STATE_SWAP:
         ...
         if (!soundfold && !spell_iswordp(p, curPor)) {
            PROF_STORE(sp->ts_state)
         ...
         if (p[n] == ZERO)
            c2 = ZERO;
         ei (!soundfold && !spell_iswordp(p + n, curPor))
            c2 = c; // don't swap non-word char
         else
            c2 = mb_ptr2char(p + n);

         // When the second character is ZERO we can't swap.
         if (c2 == ZERO) {
            PROF_STORE(sp->ts_state)
            sp->ts_state = STATE_REP_INI;
            break;
         ...
         if (c2 != ZERO && TRY_DEEPER(su, stack, depth, SCORE_SWAP)) {
      ...
      case STATE_UNSWAP:
         ...
         n = utfCharLen(p);
         c = mb_ptr2char(p + n);
         mch_memmove(p + utfCharLen(p + n), p, n);
         mb_char2bytes(c, p);
         // FALLTHROUGH
      ...
      case STATE_SWAP3:
         ...
         fl = MB_CPTR2LEN(p + n);
         c2 = mb_ptr2char(p + n);
         if (!soundfold && !spell_iswordp(p + n + fl, curPor))
      ...
      case STATE_REP:
         ...
         while (sp->ts_curi < gap->len) {
            ftp = (FromTo *)gap->c + sp->ts_curi++;
            if (*ftp->ft_from != *p) {
         ...
         if (sp->ts_curi >= gap->len && sp->ts_state == STATE_REP) {
            // No (more) matches.
            PROF_STORE(sp->ts_state)
            sp->ts_state = STATE_FINAL;
         }
...
private void
find_keepcap_word(SpellLang *slang, Byte *fword, Byte *kword) {
    Byte   uword[MAXWLEN];      // "fword" in upper-case
    int      depth;
    idx_T   tryidx;
    ...
    idx_T   lo, hi, m;
    Byte   *p;
    Byte   *byts = slang->sl_kbyts;    // array with bytes of the words
...
//Compute the sound-a-like score for suggestions in su->su_ga and add them to su->su_sga.
private void
score_comp_sal(Suggestion *su) {
    Langp   *lp;
    Byte   badsound[MAXWLEN];
    int      i;
    suggest_T   *stp;
    suggest_T   *sstp;
    int      score;
   ...
   if (ga_grow(&su->su_sga, su->su_ga.len) == FAIL)
      return;
   ...
   for (lpi = 0; lpi < curPor->ownSyntax->b_langp.len; ++lpi) {
      ...
      if (lp->lp_slang->sl_sal.len > 0) {
         ...
         for (i = 0; i < su->su_ga.len; ++i) {
            ...
            // Case-fold the suggested word, sound-fold it and compute the sound-a-like score.
            score = stp_sal_score(stp, su, lp->lp_slang, badsound);
            if (score < SCORE_MAXMAX) {
               // Add the suggestion.
               sstp = &SUG(su->su_sga, su->su_sga.len);
               sstp->st_word = copyStr(stp->st_word);
...
//Combine the list of suggestions in su->su_ga and su->su_sga. They are entwined.
private void
score_combine(Suggestion *su) {
    int      i;
    int      j;
    ArrayList   ga;
    ArrayList   *gap;
   ...
   stp->st_salscore = TRUE;
    ...
    ga_init2(&ga, sizeof(Suggestion), 1);
   if (ga_grow(&ga, su->su_ga.len + su->su_sga.len) == FAIL)
      return;

   stp = &SUG(ga, 0);
   for (i = 0; i < su->su_ga.len || i < su->su_sga.len; ++i) {
   ...
   if (ga.len > su->su_maxcount) {
   for (i = su->su_maxcount; i < ga.len; ++i)
       eeglFree(stp[i].st_word);
   ga.len = su->su_maxcount;
    }

    su->su_ga = ga;
}

//For the goodword in "stp" compute the soundalike score compared to the badword.
private int
stp_sal_score(
    suggest_T   *stp,
    Suggestion   *su,
    SpellLang   *slang,
    Byte   *badsound)   // sound-folded badword
{
    Byte   *p;
    Byte   *pbad;
    Byte   *pgood;
    Byte   badsound2[MAXWLEN];
    Byte   fword[MAXWLEN];
    Byte   goodsound[MAXWLEN];
    Byte   goodword[MAXWLEN];
    int      lendiff;
   ...
   (void)spell_casefold(curPor, su->su_badptr, stp->st_orglen, fword, MAXWLEN);

   // When joining two words the sound often changes a lot.  E.g., "t he"
   // sounds like "t h" while "the" sounds like "@".  Avoid that by
   // removing the space.  Don't do it when the good word also contains a
   // space.
   if (SPACE_OR_TAB(su->su_badptr[su->su_badlen]) && *skiptowhite(stp->st_word) == ZERO) {
      for (p = fword; *(p = skiptowhite(p)) != ZERO; ) 
         STRMOVE(p, p + 1);
    } 
   ...
   copySubstrToAllocation(goodword + stp->st_wordlen,
             (Text){su->su_badptr + su->su_badlen - lendiff, lendiff});
   pgood = goodword;
    } else
      pgood = stp->st_word;
...
typedef struct {
    ...
    Byte   sft_word[1];    // soundfolded word, actually longer
} SftWord;

private SftWord dumsft;
#define HIKEY2SFT(p)  ((SftWord *)((p) - (dumsft.sft_word - (CS)&dumsft)))
...
suggest_try_soundalike_prep(void) {
   ...
   lp = LANGP_ENTRY(curPor->ownSyntax->b_langp, lpi);
   slang = lp->lp_slang;
   if (slang->sl_sal.len > 0 && slang->sl_sbyts != NULL)
 ...
 */
private void
suggest_try_soundalike(Suggestion *su) {
    Byte   salword[MAXWLEN];
    Langp   *lp;
    int      lpi;
    SpellLang   *slang;
...
suggest_try_soundalike_finish(void) {
   ...
   if (slang->sl_sal.len > 0 && slang->sl_sbyts != NULL) {
       ...
       todo = (int)slang->sl_sounddone.count;
...
add_sound_suggest(
    ...
    Langp   *lp)
{
    SpellLang   *slang = lp->lp_slang;   // language for sound folding
    int      sfwordnr;
    Byte   *nrline;
    int      orgnr;
    Byte   theword[MAXWLEN];
    int      i;
    int      wlen;
    Byte   *byts;
    idx_T   *idxs;
    ...
    Text t = mbText(goodword);
    hash = calcHash(t);
    hi = hash_lookup(&slang->sl_sounddone, t, hash);
   if (HASHITEM_EMPTY(hi)) {
      sft = alloc(offsetof(SftWord, sft_word) + STRLEN(goodword) + 1);
      if (sft) {
         sft->sft_score = score;
         STRCPY(sft->sft_word, goodword);
         hash_add_item(&slang->sl_sounddone, hi, mbText(sft->sft_word), hash);
      }
   } else {
   sft = HI2SFT(hi);
   ...
   if (sfwordnr < 0) {
      internal_error((CS)"add_sound_suggest()");
      return;
    }

    // go over the list of good words that produce this soundfold word
    nrline = memGetLine(slang->sl_sugbuf, (LineNr)(sfwordnr + 1), FALSE);
    orgnr = 0;
    while (*nrline != ZERO) {
   ...
   for (wlen = 0; wlen < MAXWLEN - 3; ++wlen) {
       i = 1;
       if (wordcount == orgnr && byts[n + 1] == ZERO)
...
private int
soundfold_find(SpellLang *slang, Byte *word) {
    idx_T   arridx = 0;
    int      len;
    int      wlen = 0;
    int      c;
   ...
   // If the first possible byte is a zero the word could end here.
   // If the word ends we found the word.  If not skip the ZERO bytes.
   c = ptr[wlen];
   if (byts[arridx] == ZERO) {
       if (c == ZERO)
      break;
...
//Return TRUE if "c1" and "c2" are similar characters according to the MAP lines in the .aff file.
private int
similar_chars(SpellLang *slang, int c1, int c2) {
    int      m1, m2;
   ...
   hi = hash_find(&slang->sl_map_hash, text(buf));
   if (HASHITEM_EMPTY(hi))
       m1 = 0;
   else
            ...
            // su_sfmaxscore to the total score.
{
   ...
   // Add a suggestion.
   stp = &SUG(*gap, gap->len);
   stp->st_word = copySubstr(goodword, goodlen);
   if (stp->st_word != NULL) {
    ...
    ArrayList   *gap)          // either su_ga or su_sga
{
   ...
   return;
    stp = &SUG(*gap, 0);
    for (i = gap->len - 1; i >= 0; --i) {
   // Need to append what follows to check for "the the".
   copySubstrToAllocation(longword, (Text){stp[i].st_word, MAXWLEN});
   len = stp[i].st_wordlen;
   copySubstrToAllocation(longword + len, (Text){su->su_badptr + stp[i].st_orglen, MAXWLEN - len});
   deco = 0;
   (void)spell_check(curPor, longword, &deco, NULL, FALSE);
   if (deco != 0) {
      // Remove this entry.
      eeglFree(stp[i].st_word);
      --gap->len;
      if (i < gap->len)
         mch_memmove(stp + i, stp + i + 1, sizeof(suggest_T) * (gap->len - i));
   }
   }
}
...
private void
add_banned(Suggestion   *su, Byte   *word) {
   Byte   *s;
   Hash   hash;
   EeSetItem   *hi;

   hash = calcHash(mbText(word));
   hi = hash_lookup(&su->su_banned, mbText(word), hash);
   if (!HASHITEM_EMPTY(hi))      // already present
      return;
   s = copyStr(word);
   hash_add_item(&su->su_banned, hi, text(s), hash);
}

//Recompute the score for all suggestions if sound-folding is possible.  This
//is slow, thus only done for the final results.
private void
rescore_suggestions(Suggestion *su) {
   int      i;
...
private void
rescore_one(Suggestion *su, suggest_T *stp) {
    SpellLang   *slang = stp->st_slang;
    Byte   sal_badword[MAXWLEN];
    Byte   *p;

    // Only rescore suggestions that have no sal score yet and do have a language.
   if (slang != NULL && slang->sl_sal.len > 0 && !stp->st_had_bonus) {
...
private int sug_compare(const void *s1, const void *s2);
...
sug_compare(const void *s1, const void *s2) {
   ...
   if (n == 0)
       n = caseInsensitiveCompare(p1->st_word, p2->st_word);
    }
   return n;
}
    ...
    int      keep)      // nr of suggestions to keep
{
   ...
   return maxscore;
    ...
    // Sort the list.
    qsort(gap->c, (Unt)gap->len, sizeof(suggest_T), sug_compare);
...
private int
soundalike_score(
    Byte   *goodstart,   // sound-folded good word
    Byte   *badstart)   // sound-folded bad word
{
    Byte   *goodsound = goodstart;
    Byte   *badsound = badstart;
    int      goodlen;
    int      badlen;
    int      n;
    Byte   *pl, *ps;
   ...
   if ((badsound[0] == ZERO && goodsound[1] == ZERO)
       || (goodsound[0] == ZERO && badsound[1] == ZERO))
       // changing word with vowel to word without a sound
      return SCORE_DEL;
   if (badsound[0] == ZERO || goodsound[0] == ZERO)
...
{
    int      *cnt;
    int      badlen, goodlen;   // lengths including ZERO
    int      j, i;
    int      t;
    int      bc, gc;
    int      pbc, pgc;
   ...
   for (p = badword, badlen = 0; *p != ZERO; )
       wbadword[badlen++] = mb_cptr2char_adv(&p);
   wbadword[badlen++] = 0;
   for (p = goodword, goodlen = 0; *p != ZERO; )
...
spell_edit_score_limit(
   ...
   // Multi-byte characters require a bit more work
   ...
   Byte* p;
   for (p = badword; *p != ZERO; )
      wbadword[bi++] = mb_cptr2char_adv(&p);
   wbadword[bi++] = 0;
   gi = 0;
   for (p = goodword; *p != ZERO; )
   ...
   for (;;) {
      ...
      if (gc == ZERO){ // goodword ends, delete badword chars
         do {
            if ((score += SCORE_DEL) >= minscore)
               goto pop;       // do next alternative
         } while (wbadword[++bi] != ZERO);
         minscore = score;
      } ei (bc == ZERO) { // badword ends, insert badword chars
         do {
            if ((score += SCORE_INS) >= minscore)
                goto pop;       // do next alternative
         } while (wgoodword[++gi] != ZERO);
         minscore = score;
         ...
         // substitution is more expensive, thus there is no need to try both.
   ...
   pop:
    ...
    // Return a very large number to avoid going below the limit when giving a bonus.
   if (minscore > limit)
      return SCORE_MAXMAX;
   return minscore;
}
