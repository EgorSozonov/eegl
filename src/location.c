...
private Byte   *createMsg(int, int);
private int   getBufNrForPath(LocationList *ll, Byte *, Byte *);
private Arr(Byte) pushDir(Byte *, DirStack **, int is_file_stack);
private Byte   *popDir(DirStack **);
private Byte   *guessFilepath(LocationList *ll, Byte *);
...
getTempList(void) {
   ...
   // Reset the length to zero.  Retain c from previous use to avoid
...
nextStringLine(LocationState *state) {
   ...
   Byte* inputString = state->source.String.c;
   if (*inputString== ZERO) // Reached the end of the string
      return QF_END_OF_INPUT;

   p = firstOccurrence(inputString, '\n');
   if (p != NULL)
      len = (int)(p - inputString) + 1;
   else
...
nextListLine(LocationState* state) {
   ...
   len = (int)STRLEN(listItem->c.string);
   if (len > IOSIZE - 2) {
      state->linebuf = growLineBuffer(state, len);
      if (state->linebuf == NULL)
         return QF_NOMEM;
...
private int
copy_nonerror_line(Byte *linebuf, int linelen, Fields *fields) {
   Byte   *p;

   if (linelen >= fields->errmsglen) {
      // linelen + null terminator
      p = eeRealloc(fields->errmsg, linelen + 1);
      fields->errmsg = p;
...
private int
qf_parse_fmt_r(RegMatch *rmp, int midx, Byte **tail) {
   if (rmp->startp[midx] == NULL)
      return QF_FAIL;
...
parseErrorFormatMatch(
   ...
   if ((idx == 'C' || idx == 'Z') && !qf_multiline)
      return QF_FAIL;
   if (firstOccurrence((CS)"EWIN", idx) != NULL)
      fields->type = idx;
   else
      fields->type = 0;
...
private int
qf_parse_line_nomatch(Byte *linebuf, int linelen, Fields *fields) {
   ...
   Fields *fields)
{
   ...
   if (!ll->qf_multiignore) {
      ...
      if (!qfprev->fNum)
          qfprev->fNum = getBufNrForPath(ll,
             ll->dir,
...
restofline:
   ...
   status = qf_parse_get_fields(linebuf, linelen, fmt_ptr, fields,
            ll->qf_multiline, ll->qf_multiscan, &tail);
   if (status == QF_NOMEM)
       return status;
   if (status == QF_OK)
       break;
...
private void
storeTitle(LocationList *ll, Arr(Byte) title) {
   EE_CLEAR(ll->title);
...
private void
newLocList(LocationStack *stack, Arr(Byte) title) {
   LocationList   *ll;
...
private void
wipe_qf_buffer(LocationStack *stack) {
   Book   *qfbuf;

   if (stack->bufNum == INVALID_LL_BUFNR)
      return;

   qfbuf = bookFindFileByBookNr(stack->bufNum);
   if (qfbuf != NULL && qfbuf->countPortals == 0) {
      ...
      // but closeBook() calls CHECK_CURBOOK() macro and requires
      // curPor->book == curBook
      if (curPor->book == NULL) {
         curPor->book = curBook;
         buf_was_null = TRUE;
      ...
      closeBook(NULL, qfbuf, DOBOOK_WIPE, FALSE, FALSE);
...
// Get buffer number for file "directory/fname". Also sets the hasLocationEntry flag.
private int
getBufNrForPath(LocationList *ll, Byte *directory, Byte *fname) {
   Byte   *ptr = NULL;
   Book* book;
   Byte   *bufname;
...
pushDir(CS dirbuf, DirStack **stackptr, int is_file_stack) {
   ...
   ds_new = ALLOC_ONE_ID(DirStack, aid_ll_dirstack);
   if (ds_new == NULL)
      return NULL;
...
private Byte *
guessFilepath(LocationList *ll, Byte *filename) {
   DirStack     *ds_ptr;
   DirStack     *ds_tmp;
   Byte         *fullname;
...
getNthEntry(LocationList* ll, int errornr, int* new_qfidx) {
   ...
   while (errornr < ind && ind > 1 && curr->prev != NULL) {
      --ind;
      curr = curr->prev;
   }
   // New error number is greater than the current error number
   while (errornr > ind && ind < ll->count && curr->next != NULL) {
      ++ind;
      curr = curr->next;
   }
...
private int
open_new_file_port(LocationStack *ll_ref) {
   int flags = WSP_ABOVE;
   if (ll_ref)
      flags |= WSP_NEWLOC;
   if (splitPortal(0, flags) == FAIL)
...
jumpToUsablePortal(int fNum, int newPort, int *openedPortal) {
   ...
   if (!newPort)
      ll_ref = curPor->locationStackRef;
   ...
   if ((ONLY_ONE_PORTAL && isLocationListBook(curBook)) || !usablePort || newPort) {
      if (open_new_file_port(ll_ref) != OK)
         return FAIL;
...
private void
formatText(ArrayList *gap, Byte *text) {
   Byte* p = text;
   while (*p != ZERO) {
      if (*p == '\n') {
         ga_append(gap, ' ');
...
c_cPortal(Invocation *invo) {
   ...
   win = findPortalIntoLocList(stack);

    // If a location portal is open but we have no errors to display,
    // close the window.  If a location portal is not open, then open
...
openNewPortal(LocationStack *stack, int height) {
   ...
   if (qfBuf != NULL) {
      ...
      if (startEditingFile(qfBuf->fiNum, NULL, NULL, NULL, ECMD_ONE,
...
private Byte *
buildFullShellCommand(Arr(Byte) makecmd, Byte *fname) {
   unsigned   len;
   
   len = STRLEN(makecmd) + 1;
   if (*p_sp != ZERO)
...
c_make(Invocation* invo UNUSED) {
   if (apply_autocmds(EVENT_QUICKFIXCMDPRE, (CS)"make", curBook->currFileName, true, curBook) 
         && aborting()
   ) {
      return;
   }
...
/*
 * Returns the current index of the location list.
...
/*
 * Returns the current index in the location list (counting only valid
...
/*
 * Returns TRUE if the specified location entry is
...
cfile_get_auname(CommIndex id){
   switch (id) {
   case C_lfile:       return (CS)"lfile";
   case C_laddfile:  return (CS)"laddfile";
   default:       return NULL;
   }
}
...
private void
vgr_init_regmatch(RegMultilineMatch *regmatch, Byte *s) {
   // Get the search pattern: either white-separated or enclosed in //
...
qf_getprop_qfbufnr(LocationStack *stack, Bag *retBag) {
   ...
   if (stack != NULL && bookFindFileByBookNr(stack->bufNum) != NULL)
      bufnum = stack->bufNum;
...
private int
getPropertyDefaults(LocationStack *stack, int flags, OUT Bag *retBag) {
   int      status = OK;
...
exportContext(LocationList *ll, OUT Bag *retBag) {
   ...
   if (ll->qf_ctx != NULL) {
      di = dictitem_alloc(tConst("context"));
...
getProperties(LocationStack* stack, Bag* specifics, OUT Bag *retBag) {
   ...
   int flags = importKeysFromDict(specifics);
...
addEntry_from_dict(
   ...
   Byte   *filename, *module, *pattern, *text, *type;
   int      bufnum, status, col, end_col, vcol, nr;
   long   lnum, end_lnum;

   if (first_entry)
      did_bufnr_emsg = FALSE;

   filename = bagGetString(d,tConst("filename"), TRUE);
   module = bagGetString(d,tConst("module"), TRUE);
   bufnum = (int)bagGetNumber(d, tConst("bufnr"));
   lnum = (int)bagGetNumber(d, tConst("lnum"));
   end_lnum = (int)bagGetNumber(d, tConst("end_lnum"));
   col = (int)bagGetNumber(d, tConst("col"));
   end_col = (int)bagGetNumber(d, tConst("end_col"));
   vcol = (int)bagGetNumber(d, tConst("vcol"));
   nr = (int)bagGetNumber(d, tConst("nr"));
   type = bagGetString(d, tConst("type"), TRUE);
   pattern = bagGetString(d, tConst("pattern"), TRUE);
   text = bagGetString(d, tConst("text"), TRUE);
   if (text == NULL)
      text = copyStr(E);
   Var user_data;
...
private int
setProperties(LocationStack *stack, Bag *specifics, LocListAction action, Byte *title) {
   DictItem   *di;
   int      retval = FAIL;
   Boole      newlist = false;
   LocationList   *ll;

   if (action == LL_ACTION_NEW || isStackEmpty(stack))
      newlist = TRUE;
...
markReferencesInStack(LocationStack* st, int copyId) {
   return checkIfContextAndCallbackLocked(st, copyId)
         || checkIfUserDataLocked(st, copyId);
}
...
c_lbook(Invocation *invo) {
   ...
   if (processCbookArgs(invo, &book, &line1, &line2) == FAIL)
      return;
   ...
   idSave = getCurrent(stack)->id;
   if (auName) {
      Book *curbuf_old = curBook;

      apply_autocmds(EVENT_QUICKFIXCMDPOST, auName, curBook->currFileName, true, curBook);
      if (curBook != curbuf_old)
...
setLocationListInternal(
   ...
   ei (recursive != 0)
      ...
      if (newContent 
            ...
            && setLocationList(stack, newContent, action, (CS)":setloclist()", specific) 
               == OK
      )
         returnVar->number = 0;
      --recursive;
