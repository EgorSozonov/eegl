...
#define THISROWWIDTH(state) ROWWIDTH(state, (state)->pos.row)
  ...
  struct {
    VTermSelectionCallbacks* callbacks;
    void *user;
    CS buffer;
    Unt buflen;
  } selection;
...
private VTerm *
vterm_new_with_allocator(int rows, int cols, VTermAllocatorFunctions* funcs) {
   VTermBuilder builder;
   memset(&builder, 0, sizeof(builder));
   builder.rows = rows;
   builder.cols = cols;
...
vterm_allocator_malloc(VTerm* vt, Unt size) {
    return (*vt->allocator->malloc)(size);
}
...
private void
vterm_push_output_bytes(VTerm* vt, Arr(Byte) bytes, Unt len) {
   if (vt->outfunc) {
...
vterm_push_output_vsprintf(VTerm *vt, const char* format, va_list args) {
   Unt len = vsnprintf((char*)vt->tmpbuffer, vt->tmpbuffer_len, format, args);
...
private const Interval combining[] = {
   {0X0300, 0X036F},
   {0X0483, 0X0489},
   {0X0591, 0X05BD},
   {0X05BF, 0X05BF},
...
pub int IS_COMBINING_FUNCTION(uint32_t codepoint);
#else
# define IS_COMBINING_FUNCTION vterm_is_combining
private int
vterm_is_combining(uint32_t codepoint) {
   return bisearch(codepoint, combining, sizeof(combining) / sizeof(Interval) - 1);
}
#endif

#ifdef GET_SPECIAL_PTY_TYPE_FUNCTION
pub int GET_SPECIAL_PTY_TYPE_FUNCTION(void);
#else
...
vterm_state_getpen_color(VTermColor col, int argi, long args[], Boole isFg) {
   if (col < 8) {
       ...
       args[argi++] = (col - 8 + (isFg ? 90 : 100));
   } else {
       args[argi++] = CSI_ARG_FLAG_MORE | (isFg ? 38 : 48);
...
private ScreenCell *
getcell(VTermScreen *screen, int row, int col) {
...
sb_pushline_from_row(VTermScreen* screen, int row) {
   ...
   for (pos.col = 0; pos.col < screen->cols; pos.col++)
      vterm_screen_get_cell(screen, pos, OUT screen->sb_buffer + pos.col);
...
scrollrect(VTermRect rect, int downward, int rightward, void *user) {
   ...
   if (screen->damaged.start_row != SHORT && !rect_intersects(&rect, &screen->damaged)) {
      vterm_screen_flush_damage(screen);
   }
...
setpenattr(VTermAttr attr, VTermValue* val, void* user) {
   ...
   case VTERM_ATTR_BOLD:
      screen->pen.flags |= val->boolean*DECO_BOLD;
      return 1;
   case VTERM_ATTR_UNDERLINE:
      screen->pen.flags |= val->number*DECO_UNDERLINE;
      return 1;
   case VTERM_ATTR_ITALIC:
      screen->pen.flags |= val->boolean*DECO_ITALIC;
      return 1;
...
#define PUT(c)                                            \
   ...
   if (buffer && outpos + thislen <= len)                 \
      outpos += fill_utf8((c), (Byte *)buffer + outpos);  \
   else                                                   \
      outpos += thislen;
...
vterm_screen_flush_damage(VTermScreen *screen) {
  if (screen->pending_scrollrect.start_row != SHORT) {
    ...
    screen->pending_scrollrect.start_row = SHORT;
  }
  ...
  if (screen->damaged.start_row != SHORT) {
     ...
     screen->damaged.start_row = SHORT;
   }
}
...
stateErase(VTermState* state, VTermRect rect) {
   if (rect.end_col == state->cols) {
      ...
      for(Short row = rect.start_row + 1; row < rect.end_row + 1 && row < state->rows; row++)
...
scroll(VTermState* state, VTermRect rect, int downward, int rightward) {
   ...
   if (state->callbacks && state->callbacks->scrollrect
       && (*state->callbacks->scrollrect)(rect, downward, rightward, state->cbdata)
   )
      return;
...
private int
is_cursor_in_scrollregion(VTermState* state) {
...
on_text(CS bytes UNUSED, Unt len UNUSED, void* user) {
   ...
   if (state->at_phantom || state->pos.col + width > (int)THISROWWIDTH(state)) {
      linefeed(state);
      state->pos.col = 0;
      state->at_phantom = 0;
   ...
   if (state->pos.col + width >= (int)THISROWWIDTH(state)) {
...
#define INTERMED(i,b) ((i << 16) | b)
   ...
   case 0x62: { // REP - ECMA-48 8.3.103
      ...
      if (state->pos.col + state->combine_width >= (int)row_width && state->mode.autowrap) {
         state->at_phantom = 1;
         cancel_phantom = 0;
      }
      break;
...
private void
vterm_state_get_mousestate(VTermState* state, VTermMouseState* mousestate) {
...
private void
vterm_state_set_callbacks(VTermState* state, VTermStateCallbacks* callbacks, void* user) {
   if (callbacks) {
...
update_snapshot(Terminal* term) {
   ...
   for (pos.row = 0; pos.row < term->rows; ++pos.row) {
      ...
      for (pos.col = 0; pos.col < term->cols; ++pos.col)
         if (vterm_screen_get_cell(screen, pos, OUT &cell) != 0 && cell.chars[0] != ZERO) {
            len = pos.col + 1;
...
may_move_terminal_to_buffer(Terminal* term, int redraw) {
   ...
   vterm_state_get_default_colors(
      vterm_obtain_state(term->vterm), OUT &term->cellDeco.fg, OUT &term->cellDeco.bg
   );
      ...
      for (pos.col = 0; 
         ...
         if (same_chars && sameFlags && !is_cursor_pos) {
            ...
            if (!sameFlags) {
               ...
               if (cell.deco.fg == prev_cell.deco.fg)
                  fputs("&", fd);
               else {
                  fputs("#", fd);
                  dump_term_color(fd, cell.deco.fg);
               }
               if (cell.deco.bg == prev_cell.deco.bg)
                  fputs("&", fd);
               else {
                  fputs("#", fd);
                  dump_term_color(fd, cell.deco.bg);
               }
            }
...
read_dump_file(FILE *fd, VTermPos* cursor_pos) {
   ...
   cursor_pos->row = SHORT;
   cursor_pos->col = SHORT;
         ...
         for (col = 0; col < width && *p1 != ZERO && *p2 != ZERO; ++col) {
            ...
            ei (cellattr1 != 0 && cellattr2 != 0) {
               if (cellattr1[col].fg != cellattr2[col].fg)
                  textline[col] = 'f';
               ei (cellattr1[col].bg != cellattr2[col].bg)
                  textline[col] = 'b';
...
f_term_getline(Arr(Var) argvars, Var* returnVar) {
   ...
   if (term->vterm == NULL) {
      ...
      rect.start_row = row;
      rect.end_row = row + 1;
      p[vterm_screen_get_text(screen, p, len, rect)] = ZERO;
    }
}
...
f_term_scrape(Arr(Var) argvars, Var* returnVar) {
   ...
   for (pos.col = 0; pos.col < term->cols; ) {
      ...
      eeSnprintf(colorBuf, 3, "%d", fg);
      bagAddString(dcell, S"fg", colorBuf);
      eeSnprintf(colorBuf, 3, "%d", bg);
      bagAddString(dcell, S"bg", colorBuf);

      bagAddNumber(dcell, S"deco", cellToDecoration(deco, fg, bg).flags);
